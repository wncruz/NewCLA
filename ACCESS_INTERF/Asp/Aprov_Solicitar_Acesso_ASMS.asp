...
'objXmlDadosForm.save(Server.MapPath("../TesteConectividade-RealidadeIP.xml"))

		strxml = objXmlDadosForm.xml
		Aprovisi_ID = 0		
		
		strxmlResp = 	"<resposta-cla><codigo>10</codigo>"
		strxmlResp = strxmlResp  & 	"<mensagem>Inicio</mensagem></resposta-cla>"
		...
		Vetor_Campos(4)="adVarchar,20,adParamInput," 	& ID_Tarefa_Acesso
		Vetor_Campos(5)="adVarchar,20,adParamInput, ASMS " 	
		Vetor_Campos(6)="adVarchar,10,adParamInput, ATV " 
		Vetor_Campos(7)="adInteger,4,adParamInput," 	& Aprovisi_ID
		Vetor_Campos(8)="adVarchar,200,adParamInput," 	& strxmlResp
		Vetor_Campos(9)="adVarchar,8000,adParamInput," 	& strxml
...
If objXmlDadosForm.parseError.errorCode < 1 Then
	...
	if Acao = "CAN" and ( OriSol_Descricao = "ASMS" or OriSol_Descricao = "ASMS\REENVIO" ) then
		ID_Tarefa_Can = objXmlDadosForm.selectSingleNode("//requisicao-cla/id-tarefa-can").text
	end if
	'ID_Tarefa_Can = ""
	
	'if Acao = "CAN" and OriSol_Descricao = "ASMS\REENVIO" then
	'	ID_Tarefa_Can = objXmlDadosForm.selectSingleNode("//requisicao-cla/id-tarefa-can").text
	'end if
	
	Id_Logico =  objXmlDadosForm.selectSingleNode("//requisicao-cla/id-logico").text
	
	if OriSol_Descricao = "ASMS" or OriSol_Descricao = "ASMS\REENVIO" then 
		
		Header_Original 	=  objXmlDadosForm.selectSingleNode("//requisicao-cla/header-original").xml 
		
		'Header_Original 	=  objXmlDadosForm.selectSingleNode("//requisicao-cla/header-original/standardHeader/serviceState/stateCode").xml 
		External_Reflist	=  objXmlDadosForm.selectSingleNode("//requisicao-cla/external-reflist").xml
		Id_Logico_Temp 		=  objXmlDadosForm.selectSingleNode("//requisicao-cla/id-logico-tmp").text
		Id_Acesso 		=  objXmlDadosForm.selectSingleNode("//requisicao-cla/id-acesso").text
		
	end if 
	
	' Serviço/Order Entry
	if ( OriSol_Descricao <> "ASMS" and OriSol_Descricao <> "ASMS\REENVIO" ) then
	...
	If ( Trim(Acao) = "ATV"  or Trim(Acao) = "ALT"  or Trim(Acao) = "DES" or (OriSol_Descricao = "ASMS" and Trim(Acao) = "CAN")  or ( Acao = "CAN" and  OriSol_Descricao = "ASMS\REENVIO" ) )  Then
		...
		'if PABX_VIRTUAL = "2" then	
		'	PABX_VIRTUAL = "SIM"
		'elseif PABX_VIRTUAL = "1" then
		'	PABX_VIRTUAL = "NÃO"
		'else 
		'	PABX_VIRTUAL = "NULL"
		'end if	
		 ...
		 if ( OriSol_Descricao = "ASMS" or OriSol_Descricao = "ASMS\REENVIO" ) then 
			...
			Indicador_Alterar 	=  trim(objXmlDadosForm.selectSingleNode("//requisicao-cla/ind-provide-cease").text)
			Id_Servico 			=  trim(objXmlDadosForm.selectSingleNode("//requisicao-cla/id-servico").text)
			Velocidade_Total 	=  trim(objXmlDadosForm.selectSingleNode("//requisicao-cla/servico/order-entry/velocidade-total").text)
		end if
		 
		tecnologia 			=  trim(objXmlDadosForm.selectSingleNode("//requisicao-cla/acessos-fisicos/acesso/tecnologia").text)
		predial 			=  trim(objXmlDadosForm.selectSingleNode("//requisicao-cla/acessos-fisicos/acesso/predial").text)
		cnl_estacao			=  trim(objXmlDadosForm.selectSingleNode("//requisicao-cla/acessos-fisicos/acesso/cnl").text)
		compl_estacao 		=  trim(objXmlDadosForm.selectSingleNode("//requisicao-cla/acessos-fisicos/acesso/compl-estacao").text)
	...
	Vetor_Campos(2)="addouble,18,adParamInput,"& ID_Tarefa_Acesso
	Vetor_Campos(3)="addouble,18,adParamInput,"& ID_Tarefa_Can
	...
	'Gpon
	Vetor_Campos(50)="adWChar,1,adParamInput,"
	Vetor_Campos(51)="adWChar,30,adParamInput,null"
	Vetor_Campos(52)="adWChar,1,adParamInput,"
	...
	Vetor_Campos(62)="adWChar,200,adParamInput,null"
	
	Vetor_Campos(63)="adWChar,50,adParamInput,null"
	
	'Vetor_Campos(64)="adWChar,10,adParamInput,"& PABX_VIRTUAL
	Vetor_Campos(64)="adWChar,10,adParamInput,null"
	
	'ASMS:
	Vetor_Campos(65)="adWChar,20,adParamInput,"& tecnologia
	Vetor_Campos(66)="adWChar,1,adParamInput,"& predial
	Vetor_Campos(67)="adWChar,4,adParamInput,"& cnl_estacao
	Vetor_Campos(68)="adWChar,3,adParamInput,"& compl_estacao
	
	Call APENDA_PARAM("CLA_sp_ins_Aprovisionador",68,Vetor_Campos)
	'response.write APENDA_PARAMSTR("CLA_sp_ins_Aprovisionador2",68,Vetor_Campos)
	'response.end
	...
	If trim(DBAction) <> "0" and trim(DBAction) <> "" Then
		...
		'objXmlRetorno.save(Server.MapPath("SolicitaAcessoRetorno.xml"))
