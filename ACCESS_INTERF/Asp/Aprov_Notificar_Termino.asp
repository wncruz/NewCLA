...
If objXmlDadosForm.parseError.errorCode < 1 Then
	...
	strsolid = trim(objXmlDadosForm.selectSingleNode("//requisicao-cla/id-solicitacao").text)
	
	rede_contratada = trim(objXmlDadosForm.selectSingleNode("//requisicao-cla/rede_contratada").text)
	IMSI = trim(objXmlDadosForm.selectSingleNode("//requisicao-cla/IMSI").text)
	...
	Vetor_Campos(9)="adWChar,200,adParamOutput,a "
	Vetor_Campos(10)="adInteger,8,adParamOutput,0"
	
	Vetor_Campos(11)="adInteger,4,adParamInput,"
	Vetor_Campos(12)="adWChar,30,adParamInput,"
	
	Vetor_Campos(13)="adWChar,30,adParamInput,"& rede_contratada
	Vetor_Campos(14)="adWChar,30,adParamInput,"& IMSI

	Call APENDA_PARAM("CLA_sp_ins_Encerrar_AcessoAprov",14,Vetor_Campos)
...
'********* JCARTUS@ - Jan/2012 CH-54677YFT : Garantir Atualização do Status 55 - CONCLUIDA (para casos em que o SOL_ID não for enviado pelo aprovisionador)
		if isnull(strSolid) or trim(strSolid) = "" then
			strSQL = "select max(Sol_ID) as SOL_IDMax from cla_solicitacao where Acl_IDAcessologico = " & stridLogico
			Set objRSSol = db.Execute(strSQL)
			if Not objRSSol.Eof and not objRSSol.Bof then			 
					strSolid = objRSSol("SOL_IDMax")
			end if			
		end if
'********* Fim
