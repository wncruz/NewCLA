...
'IF strLoginRede <> "PRSSILV" and strLoginRede <> "EDAR" THEN
'	msg = "<p align=center><b><font color=#000080 face=Arial Black size=6>Sistema NewCLA</font></b></p>"
'	msg = msg & "<p align=center><b><font color=#000080 face=Arial Black size=4>Em Manutenção - BACKUP</font></b></p>"
'	Response.write msg
'	response.end
'END IF
...
Function ConectarCLA()
		...
		db.ConnectionString = "file name=c:\inetpub\ConexaoSQL\NewCLA.udl"
		End if
...
Function ConectarSSA()
	...
	objConnSSA.ConnectionString = "file name=c:\inetpub\ConexaoSQL\SSA.udl"
	
	objConnSSA.ConnectionTimeout = 0
	objConnSSA.CommandTimeout = 0
...
'--- Função APENDA_PARAM ---
Sub APENDA_PARAM_chrome(strProcedure, intQtdParams, arrParams)
    On Error Resume Next

    If Not IsObject(db) Then Call ConectarCla()

    Set ObjCmd = Server.CreateObject("ADODB.Command")
    Set ObjCmd.ActiveConnection = db
    ObjCmd.CommandType = 4
    ObjCmd.CommandText = strProcedure

    Dim i, arrTemp, strTipo, intTamanho, intDirecao
    Dim strBruto, strNomeParam, strConfig
    Dim varValor, vFinal, p

    For i = 1 To intQtdParams

        strBruto = CStr(arrParams(i))

        If InStr(strBruto, "|") > 0 Then
            strNomeParam = Trim(Split(strBruto, "|")(0))
            strConfig    = Trim(Split(strBruto, "|")(1))
        Else
            strNomeParam = ""
            strConfig    = strBruto
        End If

        arrTemp = Split(strConfig, ",")

        If UBound(arrTemp) >= 3 Then

            strTipo    = Trim(arrTemp(0))
            intTamanho = CInt(Trim(arrTemp(1)))
            intDirecao = GetParamDirection(Trim(arrTemp(2)))
            varValor   = Trim(arrTemp(UBound(arrTemp)))

            ' --- Valor padrão ---
            If Trim(varValor) = "" Then
                If LCase(strTipo) = "adnumeric" Or LCase(strTipo) = "adinteger" Then
                    varValor = "0"
                Else
                    varValor = ""
                End If
            ...
            ' --- Conversão segura ---
            If LCase(strTipo) = "adnumeric" Then
                If IsNumeric(varValor) Then
                    vFinal = CDbl(varValor)
                Else
                    vFinal = 0
                End If
            ElseIf LCase(strTipo) = "adinteger" Then
                If IsNumeric(varValor) Then
                    vFinal = CLng(varValor)
                Else
                    vFinal = 0
                End If
            Else
            ...
            ' --- Criar parâmetro ---
            If InStr(LCase(strConfig), "output") > 0 Then

                Set p = ObjCmd.CreateParameter(strNomeParam, GetDataType(strTipo), intDirecao, intTamanho)
            ...
            Else
                ...
                Set p = ObjCmd.CreateParameter(strNomeParam, GetDataType(strTipo), intDirecao, intTamanho, vFinal)
    ...
    If Err.Number <> 0 Then
        Response.Write "<!-- Erro APENDA_PARAM: " & Err.Description & " -->"
        Err.Clear
    End If
...
Function GetDataType_chrome(strTipo)
    Select Case LCase(strTipo)
        Case "adinteger": GetDataType = 3      ' adInteger
        Case "adnumeric": GetDataType = 131    ' adNumeric
        Case "advarchar": GetDataType = 200    ' adVarChar
        Case "adwchar":   GetDataType = 130    ' adWChar
        Case Else:        GetDataType = 200
    End Select
End Function

Function GetParamDirection_chrome(strDir)
    Select Case LCase(strDir)
        Case "adparaminput" : GetParamDirection = 1
        Case "adparamoutput": GetParamDirection = 2
        Case Else:            GetParamDirection = 1
    End Select
End Function
...
'Apenda parametros para o command
Sub APENDA_PARAMSSA(Nome_Proc_Server, Num_Param, Vetor_Campos)

	Dim i,Tipo, Tamanho, Valor, Direcao, intCountRet

	Set ObjCmdSSA = Server.CreateObject ("ADODB.Command")
	Set ObjParamSSA = Server.CreateObject ("ADODB.Parameter")

	intCountRet = 0

	ObjCmdSSA.CommandText = Nome_Proc_Server		'Nome procedimento no servidor
	ObjCmdSSA.CommandType  = adCmdStoredProc		'Tipo comando
	Set ObjCmdSSA.ActiveConnection = objConnSSA			'Associa o cammand com a conecção corrente
	...
	for i=1 to Num_Param
		...
		Select Case lcase(Tipo) 'Isso é porque ela não aceita a variável diretamente dentro de CreateParameter
			...
			Case "adinteger"
				if Direcao = "adParamInput" then
					Set ObjParamSSA = ObjCmdSSA.CreateParameter(Nome_Proc_Server,adInteger,adParamInput,Tamanho,Valor)
					ObjCmdSSA.Parameters.Append ObjParamSSA
				Else
					intCountRet = intCountRet + 1
					Select Case intCountRet
						Case 1
							Set ObjParamSSA = ObjCmdSSA.CreateParameter("RET",adInteger,adParamReturnValue)
							ObjCmdSSA.Parameters.Append ObjParamSSA
						Case 2
							Set ObjParamSSA = ObjCmdSSA.CreateParameter("RET2",adInteger,adParamReturnValue)
							ObjCmdSSA.Parameters.Append ObjParamSSA
						Case 3
							Set ObjParamSSA = ObjCmdSSA.CreateParameter("RET3",adInteger,adParamReturnValue)
							ObjCmdSSA.Parameters.Append ObjParamSSA
					End Select
				End IF
			Case "adwchar"
				if Direcao = "adParamInput" then
					Set ObjParamSSA = ObjCmdSSA.CreateParameter(Nome_Proc_Server,adWChar,adParamInput,Tamanho,Valor)
					ObjCmdSSA.Parameters.Append ObjParamSSA
				Else
					Set ObjParamSSA = ObjCmdSSA.CreateParameter("RET1",adWChar,adParamOutput,Tamanho) 'Quem usua essa saida é somente a PROC_INICIO2
					ObjCmdSSA.Parameters.Append ObjParamSSA
				End If
			Case "adlongvarchar"
				Set ObjParamSSA = ObjCmdSSA.CreateParameter(Nome_Proc_Server,adLongVarChar,adParamInput,Tamanho,Valor)
				ObjCmdSSA.Parameters.Append ObjParamSSA
			Case "addate"
				Set ObjParamSSA = ObjCmdSSA.CreateParameter(Nome_Proc_Server,adDate,adParamInput,Tamanho,Valor)
				ObjCmdSSA.Parameters.Append ObjParamSSA
			Case "addouble"
				if Direcao = "adParamInput" then
					Set ObjParamSSA = ObjCmdSSA.CreateParameter(Nome_Proc_Server,adDouble,adParamInput,Tamanho,Valor)
					ObjCmdSSA.Parameters.Append ObjParamSSA
				Else
					intCountRet = intCountRet + 1
					Select case intCountRet
						Case 1
							Set ObjParamSSA = ObjCmdSSA.CreateParameter("RET",adDouble,adParamReturnValue)
							ObjCmdSSA.Parameters.Append ObjParamSSA
						Case 2
							Set ObjParamSSA = ObjCmdSSA.CreateParameter("RET2",adDouble,adParamReturnValue)
							ObjCmdSSA.Parameters.Append ObjParamSSA
						Case 3
							Set ObjParamSSA = ObjCmdSSA.CreateParameter("RET3",adDouble,adParamReturnValue)
							ObjCmdSSA.Parameters.Append ObjParamSSA
					End Select
				End if
		End Select
...
ForXMLAutoQuery = "<root>" & adoStream.ReadText & "</root>"
...
'''novo
'''EnviarEntregarAprovASMSAuto()
Function EnviarEntregarAprovASMSAuto()
		...
		'Set objRSDadosCla = db.Execute(strSqlRet)
		 ...
		 Set objRSDadosCla = db.execute("CLA_sp_sel_RetornoAsms")
 		 if Not objRSDadosCla.Eof And Not objRSDadosCla.Bof then
		
		'If not objRSDadosCla.Eof and  not objRSDadosCla.Bof Then
		
			header 			= trim(objRSDadosCla("header_desc"))
			external 		= trim(objRSDadosCla("externalRef_desc")	)					
		...
		'	Strxml1 = Strxml1 & 	"]]> " & vbnewline
			...
			Strxml1 = Strxml1 & 	"		<dt-desativacao></dt-desativacao>" & vbnewline
			...
			Strxml1 = Strxml1 & 	"		<id-logico-tmp>"& objRSDadosCla("Sigla_Acl_idacessologico") &"</id-logico-tmp>" & vbnewline
    		...
    		xmlAcesso = xmlAcesso & 	"		<velocidade></velocidade>" & vbnewline
			...
			xmlAcesso = xmlAcesso & 	"		<interface></interface>" & vbnewline
    		
			xmlAcesso = xmlAcesso & 	"		<numero-acesso></numero-acesso>" & vbnewline
			xmlAcesso = xmlAcesso & 	"		<distribuidor></distribuidor>" & vbnewline
			xmlAcesso = xmlAcesso & 	"		<rede></rede>" & vbnewline
			xmlAcesso = xmlAcesso & 	"		<cabo></cabo>" & vbnewline ' tipo de acesso ADE fac_tronco e o cabo
			xmlAcesso = xmlAcesso & 	"		<par></par>" & vbnewline
			xmlAcesso = xmlAcesso & 	"		<tp-cabo></tp-cabo>" & vbnewline
			xmlAcesso = xmlAcesso & 	"		<prop-modem></prop-modem>" & vbnewline
			
			' Verificar
			xmlAcesso = xmlAcesso & 	"		<derivacao></derivacao>" & vbnewline
			xmlAcesso = xmlAcesso & 	"		<tipo-modem></tipo-modem>" & vbnewline
			xmlAcesso = xmlAcesso & 	"		<pade></pade>" & vbnewline
		  ...
		  '      xmlhttp.Open "POST", "http://nthor028:9333/barramento/services/SolicitarAcesso", false
		   '    xmlhttp.Open "POST", "http://nthor028:9333/barramento/services/SolicitarAcesso", false
			 ...
			 If Ucase(Request.ServerVariables("SERVER_NAME")) = "NTSPO913" or  Ucase(Request.ServerVariables("SERVER_NAME")) = "10.100.1.21" or Ucase(Request.ServerVariables("SERVER_NAME")) = "NTSPO912" or  Ucase(Request.ServerVariables("SERVER_NAME")) = "10.100.1.17" then
  					...
  					StrSQL = "select Conn_Desc from CLA_ConexaoInterf where Conn_Tipo = 'DSV1' and OriSol_ID = " & OriSol_ID
				end if
...
Function Verificar_Enviar_Retorno_Auto_APG
	Set RS_APG = db.execute("select Conn_Tipo from CLA_ConexaoInterf (nolock) where conn_Sistema='APG'")    
  if Not RS_APG.Eof And Not RS_APG.Bof then
		If RS_APG("Conn_Tipo")="S" Then
...
ResgatarLogin()
Verificar_Enviar_Retorno_Auto_APG()
'Enviar_Retorno_Auto_APG()
...
'GPON
Function Interface_SGAV_SAIP(id_tarefa,origem,status,id_Logico,Sol_ID,Aprovisi_ID)
	Strxml2 = ""
	OriSol_ID = origem
	...
	if origem = "7" then 'SGAV
		ConSGA.CommandText = "SGAV_VIPS.SP_INSERE_DADOS_SAIP"
		If trim(strLoginRede) = "EDAR" or trim(strLoginRede) = "PRSSILV" Then
			response.write "<script>alert('Retorno de SGAV SAIP')</script>"
		End if
	end if
	...
	Set objParam = ConSGA.CreateParameter("p2", adWChar, adParamInput, 100, "N") 'N = NET
	ConSGA.Parameters.Append objParam
