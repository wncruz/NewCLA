...
<!--JCARTUS-->
	<tr class="clsSilver">
		...
		<td colspan="8">
		  ...
		  &nbsp;<input type="text" class="text" name="txtComplLocalEntrega" value=<%=objRS("Esc_Sigla")%> maxlength="3" size="6" onKeyUp="ValidarTipo(this,7)" onblur="CompletarCampo(this);CheckEstacaoUsuFac2(document.Form2.txtCNLLocalEntrega,document.Form2.txtComplLocalEntrega,<%=dblUsuId%>,1);" TIPO="A" >
		</td> 
		<td colspan="1">&nbsp;</td>
	</tr>
	<tr class="clsSilver">
		...
		<td colspan="8">
		  ...
		  &nbsp;<input type="text" class="text" name="txtComplLocalConfig" value="<%=objRS("Esc_Sigla")%>" maxlength="3" size="6" onKeyUp="ValidarTipo(this,7)" onblur="CompletarCampo(this);CheckEstacaoUsuFac2(document.Form2.txtCNLLocalConfig,document.Form2.txtComplLocalConfig,<%=dblUsuId%>,2);" TIPO="A">
		</td>
		<td colspan="1">&nbsp;</td>
	</tr>
<!--JCARTUS-->
	...
	<%
		set objRS = db.execute("CLA_sp_sel_newconsultaTecnologiaFacilidade_good " & dblSolId & ", null," & dblAcfId)
	...
	<tr class=clsSilver>
		...
		<td colspan="9" >
			<%
				...
				'set objRS2 = db.execute("select distinct cla_newtecnologia.newTec_id, cla_newtecnologia.newTec_Nome from cla_assoc_tecnologiaFacilidade inner join cla_newtecnologia	on cla_assoc_tecnologiaFacilidade.newtec_id = cla_newtecnologia.newtec_id inner join cla_estrutura_tecnologiaFacilidade	on cla_assoc_tecnologiaFacilidade.assoc_tecfac_id = cla_estrutura_tecnologiaFacilidade.assoc_tecfac_id and cla_estrutura_tecnologiaFacilidade.status = 'S' 	where  cla_newtecnologia.newtec_ativo = 'S' and cla_assoc_tecnologiaFacilidade.newfac_id =  " & newfac_id   )
			
				set objRS2 = db.execute("select distinct cla_newtecnologia.newTec_id, cla_newtecnologia.newTec_Nome from cla_assoc_tecnologiaFacilidade inner join cla_newtecnologia	on cla_assoc_tecnologiaFacilidade.newtec_id = cla_newtecnologia.newtec_id where  assoc_tecfac_inativo is null and cla_newtecnologia.newtec_ativo = 'S'  and cla_assoc_tecnologiaFacilidade.newfac_id =  " & newfac_id   )
...
%>					
	<table cellspacing=0 cellpadding=0 width=760 border=0>
		...
		<tr class=clsSilver2>
			...
			<td colspan="9" >
				<input type=text class=text name='ser_PE'  size='40' maxlength='15' value="<%=Trim(objRS("newfacservico_pe"))%>" >
			</td>
		</tr>
		<tr class=clsSilver> 
			<td width="170px" nowrap><font class="clsObrig">:: </font><%=strPorta %></td>
			<td colspan="9" >
				<input type=text class=text name='ser_portaOLt'  size='40' maxlength='30' value="<%=Trim(objRS("newfacservico_porta"))%>"> 
			</td>
		</tr>
		...
		<tr class=clsSilver2>
			...
			<td colspan="9" >
			<input type=text class=text name='ser_Vlan'  size='5' maxlength='4'  onKeyUp="ValidarTipo(this,0)" value="<%=objRS("newfacservico_vlan")%>">
			</td>
		</tr>
		...
		<tr class=clsSilver>
			...
			<td colspan="9" >
				<input type=text class=text name='ser_SVLAN'  size='5' maxlength='4' onKeyUp="ValidarTipo(this,0)" value="<%=Trim(objRS("newfacservico_svlan"))%>">
			</td>
		</tr>
	...
	While Not objRS.Eof
		...
		<tr class=<%=strClass%>>
			...
			<% else %>
				...
				<% else %>
				    <input type=text class=text name='campo_<%=intCount%>' onkeyup='ValidarTipo(this,<%=trim(strTipo)%> )' size="95" maxlength="<%=trim(objRS("tamanho"))%>" value="<%=trim(objRS("conteudo"))%>" >
				<% end if %>					
				</td>
