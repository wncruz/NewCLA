...
function GravarPrevisaoProvedor()
{
	with (document.forms[0])
	{
		...
		if (!ValidarCampos(Form1.cboProjetoEspecial,"Projeto Especial")) return
		
		if (cboProjetoEspecial.value == "SIM")
		  {
...
If	Trim(Request.form("hdnAcao")) = "GravarEILD" And Request.Form("hdnAcf_ID") > 0 Then 
...
<%End IF%>	
	...
	<td colspan="2"><font class="clsObrig">:: </font> Projeto Especial de Acesso:  
		<select name="cboProjetoEspecial" >
			<option value=""></option>
			<option value="SIM" <%if strProjetoEspecial = "SIM" then Response.Write " selected " End If%> >Sim </option>
			<option value="NAO" <%if strProjetoEspecial = "NAO" then Response.Write " selected " End If%> >Não </option>
		</select> &nbsp;&nbsp;Prazo: <input type="text" class="text" size="5" name="txtProjEsp_Prazo" VALUE="<%=StrProjEsp_Prazo%>" maxlength="3" size="3" onKeyPress="OnlyNumbers()" > (Dias)</td>
 ...
 <tr class=clsSilver>
	...
	<td width=110px><font class="clsObrig">:: </font> Custo (R$):</td>
...
<tr class=clsSilver>
	...
	<td width=110px> Número da Proposta:</td>

	<td > 

		<input type="text" class="text" size="20" name="txtProjEsp_NumeroProposta" VALUE="<%=StrProjEsp_NumeroProposta%>" maxlength="20" size="20" >
