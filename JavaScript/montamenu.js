...
function makeTop()
{
	...
	{
		topZ = 0;
		//alert('teste'+menuLoc.document.all.length)
		//return
	...
	while(eval("window.aMenu" + topCount)) 
	{
...
{
	tempArray = eval("aMenu" + menuCount);
	if (!isChild) 
	{
		tempWidth = tempArray[0] ? tempArray[0] : menuWidth;
		menu = makeElement("eMenu" + menuCount,tempWidth,null,null);
	}
	else 
	{
		menu = makeElement("eMenu" + menuCount,null,parMenu,null);
	}
...
{
	menu = makeElement("eMenu" + menuCount);
	menu.array = eval("aMenu" + menuCount);
	menu.setMenuTree = setMenuTree;
	menu.setMenuTree(isChild,parMenu);
	menu.itemStr = "";
	while (menu.itemCount < menu.maxItems) 
...
function itemSetup(whichItem,whichArray) 
{
	...
	if (IE4 && this.hasMore) 
	{
		this.child = eval("eMenu" + this.id.substr(4));
...
function popMenu(e)
{
	...
	if (!isFrames) 
	{
		xPos = (currentMenu.menuLeft) ? currentMenu.menuLeft : 10;	//(NS4) ? e.pageX : (event.clientX + 152 - event.offsetX);
		yPos = (currentMenu.menuTop) ? currentMenu.menuTop : (NS4) ? e.pageY : (event.clientY - event.offsetY);
	}
	else 
	{
		switch (navFrLoc) 
		{
			case "left":
				...
				break;
			case "top":
				xPos = (currentMenu.menuLeft) ? currentMenu.menuLeft : (NS4) ? (e.pageX-pageXOffset)+menuLoc.pageXOffset -115 : event.clientX - event.offsetX -1;
				yPos = (currentMenu.menuTop) ? currentMenu.menuTop : (NS4) ? menuLoc.pageYOffset : menuLocBod.scrollTop;
				if (parent.frames.length == 3) xPos = xPos -194;
				break;
			case "bottom":
...
function hideTree() 
{
	window.status = ':: CLA - Controle Local de Acesso - Usuário: ' + hdnUserHerder.value
	allTimer = null;
