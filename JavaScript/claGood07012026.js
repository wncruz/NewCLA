...
function ValidarTipo(Campo,intTipo)
{
	...
	switch (intTipo)
	{
		...
		case 8:
			...
			break
		case 9:
			var checkOK = "0123456789.-+" //int,smallint,bit - SNOA Latitude / Longitude
			break
		//Incluido para aceitar os caracteres (+-/.()#$%&) no fabricante e modelo
		case 10:
			var checkOK = "QWERTYUIOPASDFGHJKLÇZXCVBNMqwertyuiopasdfghjklçzxcvbnm01234546789*+-/.()#$%& "
			break
		default:
...
//Validação para tamanho dos campos obrigatórios
function ValidarTamCampos(obj,plSize,strMsg)
{

	if (obj.value.length > plSize)
	{
		alert(strMsg + " excedeu o tamanho de " + plSize + " caracteres.")
		obj.focus()
		return false
	}
	return true
...
{
	//<!--Alterado por Fabio Pinho em 5/05/2016 - ver 1.0 - Inicio-->
	/**
	...
	**/
	...
	return (true);
	//<!--Alterado por Fabio Pinho em 5/05/2016 - ver 1.0 - Fim-->
}
...
function ConfirmarDSLAM()
{
	...
	//else
	//{
		if (window.confirm('Deseja incluir os registros selecionados ?') == false)
		{
			return false;
		}
		else
