...
'	- Descri��o			: Fun��es que atende a Grava��o/Altera��o da Solicitação

'Retira Chr(10) e Chr(13) do objeto xml para enviar p/ o JS/SQL
Function FormatarXml(objXml)

	Dim strXmlDadosAux
	'Retira a quebra de linha que tem no final XML e passa para a vari�vel que vai para o HTML
...
Function GravarSolicitacao(intTipoProc,intTipoProcAlt)
	...
	''Response.Write "<script language=javascript>alert('Iniciando Gravar ');</script>"


	Set objXmlDadosDB = Server.CreateObject("Microsoft.XMLDOM")

	if intTipoProc = 1 then
		'Recupera dados do xml da p�gina anterior
	...
	'Response.Write "<script language=javascript>alert('Montar XML');</script>"

	'Salva no disco ...Tempor�rio
	objXmlDadosForm.save(Server.MapPath("form.xml"))
	objXmlDadosDB.save(Server.MapPath("DadosDB.xml"))
	...
	'Response.Write " objXmlDadosForm: " & strXml
	'Response.Write " strXmlDataBase: " & strXmlDataBase
...
Function AprovarAcesso()
    ...
    'Response.Write "<script language=javascript>alert('Cheguei aqui... " & Request.Form("hdnSolId") & " ');</script>"
   ...
   dblSolId = Request.Form("hdnSolId")
	...
	Vetor_Campos(4)="adWChar,1,adParamInput,"  'Atualiza Data Devolu��o p/ GIC. Caso S, atualiza com data atual. Caso "N", a data � removida(Null).
	...
	if DBAction = 1 then
			'Response.Write "<script language=javascript>alert('Motivo da ped�ncia gravado com sucesso.')</script>"
	...
	Else
			Response.Write "<script language=javascript>alert('não foi enviar email.')</script>"
			Response.End
	End if


	Response.Write "<script language=javascript>alert('Avaliação Conclu�da ...');window.location.replace('gerenciamento_main.asp');</script>"
...
Function DevolverAcesso()
	...
	Vetor_Campos(4)="adWChar,1,adParamInput,S"  'Atualiza Data Devolu��o p/ GIC (S/N)
	Vetor_Campos(5)="adInteger,4,adParamOutput,0"
	...
	if DBAction = 1 then
			...
			Response.Write "<script language=javascript>alert('Avaliação Devolvida com Sucesso 1... ');window.location.replace('gerenciamento_main.asp');</script>"

	Else

			Response.Write "<script language=javascript>alert('não foi poss�vel gravar o motivo da pend�ncia.')</script>"
			Response.End
...
Function AlterarInfoAcesso(intTipoProc,intTipoProcAlt)
	...
	strXml =  ForXMLAutoQuery(strSqlRet)
	if err.number <> 0 then
		'Response.Write "<script language=javascript>alert('Executei Alt.');</script>"
		Response.Write "<script language=javascript>alert('" & err.Description & "');</script>"
	end if


	'Response.Write "<script language=javascript>alert('Executei antes do XML Alt.');</script>"
	Response.Write "<script language=javascript>var objXmlRet = new ActiveXObject(""Microsoft.XMLDOM"");objXmlRet.loadXML('" & strXml &"');parent.Message(objXmlRet);</script>"
	'Response.Write "<script language=javascript>alert('Executei Pos XML Alt.');</script>"
	...
	If StrPossuiAvaliador = "1" and not isnull(strDtDevolucaoGIC) Then
		Vetor_Campos(1)="adInteger,4,adParamInput,"	& dblSolId
		Vetor_Campos(2)="adInteger,4,adParamInput,203" 'GIC envia para Avaliador
		Vetor_Campos(3)="adWChar,30,adParamInput,"	& Trim(strLoginRede)
		Vetor_Campos(4)="adDate,10,adParamInput,"

		Vetor_Campos(5)="adWChar,300,adParamInput,"	& "Solicitação Alterada GIC"
		Vetor_Campos(6)="adWChar,1,adParamInput,D" 'Status detalhado
		Vetor_Campos(7)="adInteger,2,adParamOutput,0"
		Vetor_Campos(8)="adInteger,2,adParamInput, " & IntPed_ID 'Trim(Request.Form("hdnPedId"))
		...
		Call EnviarEmailAlteracaoStatus(dblSolId, 203, "Solicitação Alterada e Devolvida ao Avaliador")
		...
		Vetor_Campos(1)="adInteger,4,adParamInput,"	& dblSolId
		Vetor_Campos(2)="adInteger,4,adParamInput,202"
		Vetor_Campos(3)="adWChar,30,adParamInput,"	& Trim(strLoginRede)
		Vetor_Campos(4)="adDate,10,adParamInput,"

		Vetor_Campos(5)="adWChar,300,adParamInput,"	& "Solicitação Alterada"
		Vetor_Campos(6)="adWChar,1,adParamInput,D" 'Status detalhado
		Vetor_Campos(7)="adInteger,2,adParamOutput,0"
		Vetor_Campos(8)="adInteger,2,adParamInput, " & IntPed_ID 'Trim(Request.Form("hdnPedId"))
		...
		Call EnviarEmailAlteracaoStatus(dblSolId, 202, "Solicitação Alterada")
	End If

	'Response.Write "<script language=javascript>alert('Retorno status:" & DBAction & "');</script>"
	If DBAction <> 1 Then
		Response.Write "<script language=javascript>parent.resposta(" & Cint("0" & DBAction) & ",'');</script>"
		Response.End
	End If
	...
	Vetor_Campos(2)="adWChar,1,adParamInput,N"  'Atualiza Data Inicio Avaliacao (S/N)
	Vetor_Campos(3)="adWChar,1,adParamInput,N"  'Atualiza Data Fim Avaliacao (S/N)
	Vetor_Campos(4)="adWChar,1,adParamInput,N"  'Atualiza Data Devolu��o p/ GIC p/ Null
	Vetor_Campos(5)="adInteger,4,adParamOutput,0"
...
'		'Response.Write	"<script language=javascript>alert('Motivo da ped�ncia gravado com sucesso.')</script>"
'		'Response.Write	"<script language=javascript>parent.IFrmLista.location.href	= 'ProcessoMotivoPendencia.asp?strAcao=ResgatarLista&dblSolId="	& Request.Form("hdnSolId") & "&dblPedId=" &	Request.Form("hdnPedId") & "'</script>"
'		Call EnviarEmailAlteracaoStatus(Request.Form("hdnSolId"),202, "Solicitação Alterada")
'	Else
'		Response.Write "<script	language=javascript>alert('não foi poss�vel	gravar o motivo	da pend�ncia.')</script>"
'	End	if
...
Function VerificaRedir()
	...
	Set objRS = db.execute("CLA_sp_sel_ConfigCtf null,null,'IM'," & CidSigla)
	...
	Response.Write "<script language=javascript>alert('Obriga GLA: " &  strObrigaGla & "');</script>"
...
'''Procura o GLA respos�vel pelo Cliente(Raz�o Social)
Function ResgatarGLA()

	Dim strHtmlGla


	'@@JKNUP: Altera��o para tratar se tem redirecionamento.
	...
	'-- @@Davif - Melhoria a realizar (usar Cidade do Endere�o do Acesso como ref.)
	'Dim CidSigla
	'CidSigla = Trim(request.form("hdnCNLCliente"))
	...
	Response.Write "<script language=javascript>alert('Letra: " & Left(Trim(Request.Form("hdnRazaoSocial")),1) & "  hdnCtfcId:" & Request.Form("hdnCtfcId") & "');</script>"
...
'Resgata a descri��o da cidade pela UF,CNL,Perfil do usu�rio
...
Function ResgatarSev(dblNroSev)
	...
	'Localiza Informações da solu��o SSA
...
'Resgata o usu�rio a partir do perfil
Function ResgatarUserCoordenacao(strNomeObj,strTipoUser)

	Vetor_Campos(1)="adInteger,4,adParamInput," 'Id do usu�rio
	Vetor_Campos(2)="adWChar,30,adParamInput," & Request.Form(strNomeObj) 'UserName
	Vetor_Campos(3)="adWChar,10,adParamInput," & strTipoUser 'Tipo do Usu�rio
...
Function ProcurarCliente()
	...
	if Trim(Request.Form("txtRazaoSocial")) <> "" and Trim(Request.Form("cboClienteSel")) = "" and Trim(Request.Form("cboEndCliSel")) = "" then
		...
		if Not objRS.Eof then
			...
			if objRS("Tot_Cli") > 3000 then
				Response.Write "<script language=javascript>parent.alert('Detalhe a raz�o social para localiza��o do cliente.')</script>"
				Response.end
			end if
	...
	if Trim(Request.Form("cboClienteSel")) <> "" then  'Monta combo com endere�os para o cliente atual
		...
		if Not objRS.Eof then
			...
			Response.Write "<script language=javascript>parent.spnLabelCliente.innerHTML = '&nbsp;&nbsp;&nbsp;&nbsp;Endere�os(s)'</script>"
			Response.Write "<script language=javascript>parent.spnCliente.innerHTML = '" & strCboRet & "'</script>"
			Response.End
	...
	Else
		'Seta o Informações do Cliente e do Endere�o esse se não tivermos SEV selecionada
...
Select Case Trim(Request.Form("hdnAcao"))
	...
	Case "AprovarAcesso"
		...
		if Not ObjRSAvaliacao.EOF and not ObjRSAvaliacao.BOF then
			...
			If not isnull(ObjRSAvaliacao("Sol_DtFimAvaliacao")) and ObjRSAvaliacao("Sol_DtFimAvaliacao") <> "" then
				''Response.Write "<script language=javascript>alert('Avaliação já Aprovada..')</script>"
			...
			If not isnull(ObjRSAvaliacao("Sol_DtDevolucaoGIC")) and ObjRSAvaliacao("Sol_DtDevolucaoGIC") <> "" then
				''Response.Write "<script language=javascript>alert('Solicitação não pode ser Aprovada. Foi Devolvida ao GIC. ')</script>"
	...
	Case "DevolverAcesso"
		...
		''Response.Write "<script language=javascript>alert('obter pedid');</script>"
		Vetor_Campos(1)="adInteger,4,adParamInput," & Trim(Request.Form("hdnSolID"))
		strSqlRet = APENDA_PARAMSTRSQL("CLA_sp_Sel_Avaliacao",1,Vetor_Campos)
		Set objDicProp = Server.CreateObject("Scripting.Dictionary")
		...
		if Not ObjRSAvaliacao.EOF and not ObjRSAvaliacao.BOF then
			...
			If not isnull(ObjRSAvaliacao("Sol_DtDevolucaoGIC")) AND ObjRSAvaliacao("Sol_DtDevolucaoGIC") <> "" then
				''Response.Write "<script language=javascript>alert('Avaliação já Devolvida ao GIC..')</script>"
			...
			If not isnull(ObjRSAvaliacao("Sol_DtFimAvaliacao")) and ObjRSAvaliacao("Sol_DtFimAvaliacao") <> "" then
				''Response.Write "<script language=javascript>alert('Avaliação já Aprovada, não pode ser devolvida ao GIC.')</script>"
	...
	Case "AlterarInfoAcesso"
		...
		if Not ObjRSAvaliacao.EOF and not ObjRSAvaliacao.BOF then
			...
			If not isnull(ObjRSAvaliacao("Sol_DtFimAvaliacao"))	and	ObjRSAvaliacao("Sol_DtFimAvaliacao") <>	"" then
				''Response.Write "<script	language=javascript>alert('Avaliação já Aprovada, não pode ser Alterada!')</script>"
	...
	Case "Alteracao"
		...
		Set objXmlDadosForm  = AdicionarNode("intTipoProc",objXmlDadosForm,3) 'Tipo de Processo Altera��o/Ativa��o/Cancelamento
...
''		Set objXmlDadosForm  = AdicionarNode("hdnPossuiGeren",objXmlDadosForm,Request.Form("hdnPossuiGeren"))
	...
	Case "ResgatarEnderecoEstacao"
		'Endere�o do local de instala��o
		Set objRS = db.execute("CLA_sp_sel_estacao " & Trim(Request.Form("hdnEstacaoAtual")))

		if Not objRS.Eof And Not objRS.Bof then
			strEndereco = Trim(Cstr("" & objRS("Tpl_Sigla"))) & " " &  Trim(Cstr("" & objRS("Esc_NomeLogr"))) & " n� " & Trim(Cstr("" & objRS("Esc_NroLogr"))) & " - " & Trim(Cstr("" & objRS("Est_Sigla"))) & " - " & Trim(Cstr("" & objRS("Cid_Sigla")))
	...
	Case "ResgatarEstacaoOrigem"
		...
		Else
			Response.Write "<script language=javascript>alert('Esta��o não encontrada.');</script>"
			Response.Write "<script language=javascript>with (parent.document.forms[1]){" & _
			"cboLogrEnd.value = '';" & _
			"txtEnd.value = '';" & _
			"txtNroEnd.value = '';" & _
	...
	Case "ResgatarEstacaoDestino"
		...
		if Not objRS.Eof And Not objRS.Bof then
			...
			if Not objRSEsc.Eof And Not objRSEsc.Bof then
				...
				Response.Write "<script language=javascript>alert('Usu�rio não possui acesso a esta esta��o.');</script>"
				Response.Write "<script language=javascript>with (parent.document.forms[1]){" & _
	...
	Case "ResgatarAcessoFisComp"
		...
		'Itens que foram populados no compartilhamento do acesso f�sico e dever estar com seus valores atualizados
