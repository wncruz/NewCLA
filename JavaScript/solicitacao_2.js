...
{
	//alert('cboProvedor='+document.getElementById('cboProvedor').value)
	//alert('strAcessoTipoRede='+document.getElementById('strAcessoTipoRede').value)
	if (
		    (document.getElementById('cboProvedor').value=="136" && (document.getElementById('strAcessoTipoRede').value==null || document.getElementById('strAcessoTipoRede').value!="4")) || document.getElementById('cboProvedor').value=="137"
	)
	{
		alert('Estão bloqueadas novas ativações para provedores \nHFC BSoD VIA e HFC BSoD NET. \nEm caso de dúvidas, contatar a equipe do SOE.')
		return
	}

	//alert (document.getElementById('cboTecnologia').value)
	if (document.forms[0].cboOrigemSol.value == "9") {

		if (document.getElementById('cboProvedor').value == "145" || document.getElementById('cboTecnologia').value == "7"){
			scboInterFaceEndFis = document.getElementById('cboInterFaceEndFis').value
			scboInterFaceEnd = document.getElementById('cboInterFaceEnd').value

			if (scboInterFaceEnd !="GIGABIT ETHERNET E" && scboInterFaceEnd !="GIGABIT ETHERNET O" && scboInterFaceEnd !="FAST ETHERNET E" && scboInterFaceEnd !="FAST ETHERNET O"){
				alert('Informar Interface Válida. Opções: \n\n - FAST ETHERNET E \n - FAST ETHERNET O \n - GIGABIT ETHERNET E \n - GIGABIT ETHERNET O.')
				document.getElementById('cboInterFaceEnd').focus()
				return
			}

			if (scboInterFaceEndFis !="GIGABIT ETHERNET E" && scboInterFaceEndFis !="GIGABIT ETHERNET O" && scboInterFaceEndFis !="FAST ETHERNET E" && scboInterFaceEndFis !="FAST ETHERNET O"){
				alert('Informar Interface Válida. Opções: \n\n - FAST ETHERNET E \n - FAST ETHERNET O \n - GIGABIT ETHERNET E \n - GIGABIT ETHERNET O.')
				document.getElementById('cboInterFaceEndFis').focus()
				return
			}

		}
		//alert (document.getElementById('cboTecnologia').value)
		if ( document.getElementById('cboTecnologia').value == "5"){
			scboInterFaceEndFis = document.getElementById('cboInterFaceEndFis').value
			scboInterFaceEnd = document.getElementById('cboInterFaceEnd').value

			if (scboInterFaceEnd !="VDSL" ){
				alert('Informar Interface Válida. Opções: \n\n - VDSL.')
				document.getElementById('cboInterFaceEnd').focus()
				return
			}

			if (scboInterFaceEndFis !="VDSL" ){
				alert('Informar Interface Válida. Opções: \n\n - VDSL.')
				document.getElementById('cboInterFaceEndFis').focus()
				return
			}
	...
	with (document.forms[1])
	{
		...
		if (document.forms[0].txtNroSev.value != "" || document.forms[0].cboOrigemSol.value == "3" || document.forms[0].hd_exigesev.value == "nao")
		{
			...
			/** RAIO X if (rdoPropAcessoFisico[2].checked)
			{
...
function VerificaProvedorAcesso(strProvedor){
	...
	//Refaz a lista de Ids no IFRAME
	for (var intIndex=0;intIndex<objNode.length;intIndex++){
		strProvedorAcesso = RequestNodeAcesso(objXmlGeral,"cboProvedor",objNode[intIndex].childNodes[0].text)
...
function ReanaliseSEV()
{
	with (document.forms[0])
	{
		...
		var intRet=alertbox('Deseja realmente solicitar a Reanálise da SEV ?','Sim','Não','Sair')
...
function SistemaOrderEntry()
{
	...
	{
     //CH-55350FJO
     if(cboOrigemSol.value != "3" && cboOrigemSol.value != "9" )//Povoamento
     {//cboSistemaOrderEntry.text == "SGA VOZ VIP'S" ||
        if(cboSistemaOrderEntry.value == "SGA VOZ VIP'S" ||cboSistemaOrderEntry.value == "SGA PLUS" || cboSistemaOrderEntry.value == "APG"){
           alert("Somente são permitidos os sistemas APG, SGA VOZ VIP'S e SGA PLUS para Origem Solicitação 'POVOAMENTO'.");
		...
		if(hdnstrPOP.value == "0" && cboSistemaOrderEntry.value == "SGA PLUS"){
			alert("Desculpe, você não tem permissão para criar POVOAMENTO do sistema SGA PLUS. \nFavor, solicite ao seu Gestor que efetue a solicitação de acesso via sistema GATI.");
			cboSistemaOrderEntry.value = "";
			cboOrigemSol.focus();
			return
		}

		if(hdnstrPOV.value == "0" && cboSistemaOrderEntry.value == "SGA VOZ VIP'S"){
			alert("Desculpe, você não tem permissão para criar POVOAMENTO do sistema SGA VOZ VIP'S. \nFavor, solicite ao seu Gestor que efetue a solicitação de acesso via sistema GATI.");
			cboSistemaOrderEntry.value = "";
            cboOrigemSol.focus();
			return
		}
	...
	with (document.forms[1])
	{
		...
		if (strValue != "")
		{
			if (strValue == '11')
			{
				...
				if ( rdoPropAcessoFisico[0].checked )
				{
					alert('Não é possível selecionar EMBRATEL para tipo Terceiro/Cliente')
					obj.value = 0
					strValue = 0
				}
			}
...
function ProcurarIDFis_(intID)
{
	...
	with (document.forms[0])
	{
		...
		**/
				...
				//hdnIdAcessoFisico.value = ""
				target = "IFrmIDFis1"
				action = "AcessoCompartilhadoSol.asp?intEnd=1&strtipo=T&hdnStrAcfIDAcessoFisico= " + intID
...
{ //Verifica se há solicitação já criada para o Aprov-ID (Às vezes o usuário clica mais de uma vez no botão gravar, gerando duplicidade de solicitação.
	...
	if (document.forms[1].strOrigemAPG.value == "Aprov"){//Somente para solicitações vindas dos Aprovisionadores
		if(document.forms[3].hdnTipoAcao.value == "Alteracao" || document.forms[3].hdnTipoAcao.value == "Ativacao"){//Somente para casos de ATV e ALT.
			...
			if(strXML!=""){
  				alert("Solicitação já criada.\n\n  - Número do ID-Lógico: " + strIDLog + ",\n  - Número da Solicitação: " + strSolID + ".")
				return
			}
	}
  }
	...
	{
		alert("A gravação da solicitação não poderá ser efetuada enquanto a área 'Informações do Acesso' estiver preenchida. Apague as INFORMAÇÕES do ACESSO, clicando em LIMPAR.")
		return
	}
...
//LP
		...
		}else if (cboSistemaOrderEntry.value == "CFD" && cboOrigemSol.value != 10){
			hdnOrderEntry.value = txt_variavel.value + txt_ss.value + txt_num_sol.value + txt_ano_sol.value
		}else{
			hdnOrderEntry.value = cboSistemaOrderEntry.value + txtOrderEntry[0].value + txtOrderEntry[1].value + txtOrderEntry[2].value
		}

		if (document.forms[1].strOrigemAPG.value == "APG"){
			if ( document.forms[2].rdoEmiteOTS[0].checked == false && document.forms[2].rdoEmiteOTS[1].checked == false ){
...
function ContinuarGravacao()
{
	...
	with (document.forms[2]) //Form3
	{
		...
		if (document.forms[1].strOrigemAPG.value == "Aprov"){
			...
			if (!ValidarCampos(txtCNLLocalEntrega,"Estação do Local de Entrega")) return
			if (!ValidarCampos(txtComplLocalEntrega,"Estação do Local de Entrega")) return
			if (!ValidarCampos(txtCNLLocalConfig,"Estação do Local de Configuração")) return
			if (!ValidarCampos(txtComplLocalConfig,"Estação do Local de Configuração")) return
		}
...
//--VBL VOZ E BANDA LARGA,IP DIRETO,INTERNET,BUSINESS LINK DIRECT,IP VPN,BUSINESS IP SAT,RUD MPLS,VIP UNICO.
	if ((document.forms[0].hdnCboServico.value==27 || document.forms[0].hdnCboServico.value==46 || document.forms[0].hdnCboServico.value==106 || document.forms[0].hdnCboServico.value==120 ||
		   document.forms[0].hdnCboServico.value==126 || document.forms[0].hdnCboServico.value==138 || document.forms[0].hdnCboServico.value==139 || document.forms[0].hdnCboServico.value==140)
		   && document.forms[0].cboOrigemSol.value!="6" && document.forms[0].cboOrigemSol.value!="7" && document.forms[0].cboOrigemSol.value!="9") {

		  document.forms[0].hdnDesigServ.value = document.forms[0].hdnDesigServ.value.replace(" /IP/","/IP/")
	...
	//alert('Designação do Serviço é um campo obrigatório para esse serviço.');
	...
	if (document.forms[1].strOrigemAPG.value == "APG"){
	  ...
	  if ( (document.forms[2].rdoEmiteOTS[1].checked == true) || ( document.Form3.hdnCompartilhaRota.value == "S" || document.Form3.hdnCompartilhaTronco2M.value == "S" )){
		...
		if (document.Form1.txtdesignacaoServico.value != "")	AdicionarNode(objXmlGeral,"txtdesignacaoServico",document.Form1.txtdesignacaoServico.value)
	}
	...
	with (document.Form4){
		  ...
		  var_check = document.forms[0].rdoAntAcesso;

		  for (var i = 0; i < var_check.length; i ++)
		  {
		  	if (var_check[i].checked)
		      {

			   if (i == 0)
			     {
			       hdnAntAcesso.value = 'S'
				   var_check[i].value = 'S'
				   var_ckeck_ok = 'S'
				 }
			   else
			     {
				   hdnAntAcesso.value = 'N'
				   var_check[i].value = 'N'
				   var_ckeck_ok = 'N'
				 }

			   hdnAntAcesso.value = var_check[i].value

				  if(var_check[i].value = 'N'){
				    if (!MontarDesigServico(true)) return false
					if ( InStr(1,document.forms[0].hdnDesigServ.value,"$") != 0 ){
						alert('Designação do Serviço é um campo obrigatório para esse serviço.');
						return
					}
				 }

			 AdicionarNode(objXmlGeral,"AntAcesso",var_ckeck_ok);
		      }
		  }
			...
			if (objNode.length >= 0){
				...
				for (var intIndex=1;intIndex<=treeList.length;intIndex++){
					...
					if( document.forms[1].hdnAcfId.value == hdnAcfId_var){
						alert("Não é possível inserir o mesmo Físico na Solicitação....zz");
						return;
					}
...
function PopupProcessoAtivoAnt(){
	with (document.Form1){
		strRet = window.showModalDialog('PopupAcessosFisicosAnterior.asp?hdnSolId='+hdnSolId.value,"", "dialogHeight: 300px; dialogWidth: 630px; dialogTop: px; dialogLeft: px; edge: Raised; center: Yes; help: No; resizable: No; status: No;");
			if (strRet != undefined)
			{
...
function LimparInfoAcesso(){
	...
	for (var intIndex=0;intIndex<document.Form2.elements.length;intIndex++)
	{
		var elemento = document.Form2.elements[intIndex];
		//alert(elemento.type)
		if (elemento.type != 'button' && elemento.type != 'hidden' && elemento.type != 'radio'){
...
function ResgatarContrato()
{
	with (document.forms[0])
	{
		...
		{
			solicPedSnoaCboContrFornec.innerHTML = "<Select name=CboContrFornec style='width:200px'></select>"
		}
	}
}
...
function EditarAcessoFisComp(intChave,objXmlAcessoFisComp)
{
	...
	//else{
		...
		document.forms[1].txtCNLSiglaCentroCliDest.disabled = true
		document.forms[1].txtComplSiglaCentroCliDest.disabled = true
...
function CompartilhaTronco2M(AcfID){
	...
	with (document.forms[1]){
		if (strOrigemAPG.value == "APG"){
			if ( document.Form3.hdnCompartilhaRota.value == "S" || document.Form3.hdnCompartilhaTronco2M.value == "S" ){
				target = "IFrmTronco2M"
...
function AdicionarAcessoListaAprov(){
		...
		if (objNode.length != 0 )
		{
			...
			for (var intIndex=0;intIndex<objNode.length;intIndex++){
				...
				strcboProvedor = RequestNodeAcesso(objXmlGeral,"cboProvedor",objNode[intIndex].childNodes[0].text)
				...
				/** retirada 09072015 **/
				/**
				if (strcboTecnologia == "6" && ( document.Form2.btnAddAcesso.value == "Adicionar" || document.Form2.btnAddAcesso.value == "Alterar" )  ) {
					alert("A tecnologia GPON não aceita acesso misto!")
				    return
				}
				
				if (strcboTecnologia == "7" && (  document.Form2.btnAddAcesso.value == "Adicionar" || document.Form2.btnAddAcesso.value == "Alterar" ) ){
					alert("A tecnologia FO EDD não aceita acesso misto!")
				    return
				}
				
				if (strcboProvedor == "145" && ( document.Form2.btnAddAcesso.value == "Adicionar" || document.Form2.btnAddAcesso.value == "Alterar") ){
					alert("A tecnologia Ethernet não aceita acesso misto!")
				    return
				}
				
				if (strcboProvedor == "154" && ( document.Form2.btnAddAcesso.value == "Adicionar" || document.Form2.btnAddAcesso.value == "Alterar" ) ){
					alert("A tecnologia HFC não aceita acesso misto!")
				    return
				}
				**/
	...
	with (document.forms[1]) //Form2
	{
		...
		//GPON
		try{
			if (cboTecnologia[cboTecnologia.selectedIndex].innerText == "GPON" || cboTecnologia[cboTecnologia.selectedIndex].innerText == "FO EDD")
				{
				var objNode = objXmlGeral.selectNodes("//xDados/Acesso")
				/** retirada 09072015
				if (objNode.length != 0 )
				...
				**/
				...
				if (!ValidarCampos(cboFabricanteONT,"Fabricante")) return false
				if (!ValidarCampos(cboTipoONT,"Modelo")) return false
				}
			}
		catch(e){}
		...
		//BSOD:
		//if (cboProvedor.value == 138)
		if (cboProvedor.value == 136 && hdnOriSol_ID.value!=7)
		{
...
function GravarSolicPedSNOA1()
{
	with (document.forms[0])
	{
		...
		if (cboTipoAcao.value != "2" && cboTipoAcao.value != "6" )  //CANCELAMENTO //DESATIVACAO
		{

			//alert('ok')


			if (rdoEntrCanalizada_A[0].checked)  // Sim
			{
				if (!ValidarCampos(txtTimeSlot_A,"Time Slot")) return
				if (!ValidarCampos(txtE1Canalizado_A,"E1 Canalizado")) return
			}

			if (rdoEntrCanalizada_B[0].checked)  // Sim
			{
			...
			if (!ValidarCampos(cboInterfFisica_A,"Interface Física da Ponta A")) return
			if (!ValidarCampos(cboInterfEletr_A,"Interface Elétrica da Ponta A")) return
			...
			if (!ValidarCampos(cboInterfFisica_B,"Interface Física da Ponta B")) return
			if (!ValidarCampos(cboInterfEletr_B,"Interface Elétrica da Ponta B")) return
			...
			if (!ValidarTamCampos(txtPontoRefencia_A,50,"Ponto de Referência da Ponta A")) return

			if (!ValidarTamCampos(txtEmailTec_A,50,"Email Tecnico da Ponta A")) return
			...
			if (!ValidarTamCampos(txtPontoRefencia_B,50,"Ponto de Referência da Ponta B")) return

			if (!ValidarTamCampos(txtUsuario_B,50,"Usuário da Ponta B")) return

			if (!ValidarTamCampos(txtComplEnd_A,120,"Complemento da Ponta A")) return
			if (!ValidarTamCampos(txtComplEnd_B,120,"Complemento da Ponta B")) return

			if (!ValidarTamCampos(txtE1Canalizado_A,50,"E1 Canalizado da Ponta A")) return

			if (!ValidarTamCampos(txtTimeSlot_A,50,"TimeSlot da Ponta A")) return

			if (!ValidarTamCampos(txtE1Canalizado_B,50,"E1 Canalizado da Ponta B")) return

			if (!ValidarTamCampos(txtTimeSlot_B,50,"TimeSlot da Ponta B")) return

			if (!ValidarTamCampos(txtEmailTec_B,50,"Email Tecnico da Ponta B")) return
			...
			//<!-- CH-28482YPU - Inicio -->
			if (IsEmpty(txtCproInscricaoEstadual.value)){alert('Inscrição Estadual é um campo obrigatório.');return}
			//<!-- CH-28482YPU - Fim -->

		}


		if (cboTipoAcao.value == "2")  //CANCELAMENTO
		{
		...
		if (cboTipoAcao.value == "6")  //DESATIVACAO
		{
			if (!ValidarCampos(txtMotivo,"Justificativa")) return
			if (!ValidarCampos(cboVelocidade,"Taxa de Transmissão")) return
			...
			if (!ValidarCampos(txtDesignacaoFornecedora,"Designação da Fornecedora")) return
		/**	
			with(document.forms[0]){
			...
			**/
			...
			strXML = strXML + "<acfid>" +  document.Form2.hdnAcfIdRadio.value + "</acfid>"
			
			strXML = strXML + "<Velocidade>" +  cboVelocidade.value + "</Velocidade>"
			strXML = strXML + "<ContatoEnd_A>" +  txtContatoEnd_A.value + "</ContatoEnd_A>"
			strXML = strXML + "<TelEnd_A>" +  txtTelEnd_A.value + "</TelEnd_A>"
			strXML = strXML + "<DesignacaoFornecedora>" +  txtDesignacaoFornecedora.value + "</DesignacaoFornecedora>"
		...
		//<!-- CH-83646VWR - Inicio -->
		if (cboTipoAcao.value == "3")  //MUDANÇA DE VELOCIDADE
		{
			if (!ValidarCampos(txtNumPedSnoaAnt,"Número do Pedido SNOA Anterior")) return
		}

		if (cboTipoAcao.value == "4")  //MUDANÇA DE ENDEREÇO
		{
			if (!ValidarCampos(txtNumPedSnoaAnt,"Número do Pedido SNOA Anterior")) return
		}
		//<!-- CH-83646VWR - Fim -->
		...
		//<!-- CH-83646VWR - Inicio -->
		if (cboTipoAcao.value != "1")  // ATIVAÇÃO
		{

			if (IsEmpty(txtNumPedSnoaAnt.value)){alert('Número do Pedido Anterior do SNOA é um campo obrigatório.');return}
