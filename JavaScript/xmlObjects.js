/*
=====================================================================================
Objetivo		: Retorna o valor de um node se n�o existir retorna ""
Premissas		: Enviar objeto XML 
Entradas		: objXml - objeto XML
				  strNomeNode - Nome do node
				  intChave - Indentificador do no	
Retorno			: Valor do node ou ""
=====================================================================================
*/
function RequestNodeAcesso(objXML, strNomeNode, intChave) {
  objNode = objXML.selectSingleNode(
    "//xDados/Acesso[intIndice=" + parseInt(intChave) + "]"
  );
  try {
    var objNodeRequest = objNode.getElementsByTagName(strNomeNode);

    if (objNodeRequest.length != 0) {
      return objNodeRequest.item(0).text;
    } else {
      return "";
    }
  } catch (e) {
    //alert("RequestNodeAcesso= " + strNomeNode);
    return "";
  }
}
/*
=====================================================================================
Objetivo		: Cria/Atualiza nodes no Xml de Acessos
Premissas		: Existir objeto XML na p�gina que faz o include
Entradas		: blnLimpar - booleana se limpa os campos do acesso ou n�o
Retorno			: Atualiza lista com item adicionado
=====================================================================================
*/
function xmlUpd_OLD(blnLimpar) {
  for (
    var intIndex = 0;
    intIndex < document.Form2.elements.length;
    intIndex++
  ) {
    var elemento = document.Form2.elements[intIndex];

    if (intIndex != 58 || intIndex != 59) {
      //@@JKNUP: guardar o CNL cliente para recuperar REDIRECIONAMENTO
      if (elemento.type == "text" && elemento.name == "txtEndCid") {
        document.Form2.hdnCNLCliente.value = elemento.value;
      }
      //--------------------------------------------------------------
    }

    if (elemento.type == "radio") {
      if (elemento.checked) {
        if (document.Form2.hdnIntIndice.value != "") {
          AdicionarNodeAcesso(
            objXmlGeral,
            elemento.name,
            elemento.value,
            document.Form2.hdnIntIndice.value
          );
          AdicionarNodeAcesso(
            objXmlGeral,
            elemento.name + "Index",
            elemento.Index,
            document.Form2.hdnIntIndice.value
          );
        } else {
          AdicionarNodeAcesso(
            objXmlGeral,
            elemento.name,
            elemento.value,
            intIndice
          );
          AdicionarNodeAcesso(
            objXmlGeral,
            elemento.name + "Index",
            elemento.Index,
            intIndice
          );
        }
      }
    } else {
      if (document.Form2.hdnIntIndice.value != "") {
        AdicionarNodeAcesso(
          objXmlGeral,
          elemento.name,
          elemento.value,
          document.Form2.hdnIntIndice.value
        );
      } else {
        AdicionarNodeAcesso(
          objXmlGeral,
          elemento.name,
          elemento.value,
          intIndice
        );
      }
    }
    //Cria um node com o text dos combos
    if (elemento.type == "select-one") {
      if (document.Form2.hdnIntIndice.value != "") {
        AdicionarNodeAcesso(
          objXmlGeral,
          elemento.name + "Text",
          elemento[elemento.selectedIndex].text,
          document.Form2.hdnIntIndice.value
        );
      } else {
        AdicionarNodeAcesso(
          objXmlGeral,
          elemento.name + "Text",
          elemento[elemento.selectedIndex].text,
          intIndice
        );
      }
    }

    if (blnLimpar) {
      //N�o pode limpar o conte�do do radio somente fazer checked=false
      if (
        elemento.type != "button" &&
        elemento.type != "hidden" &&
        elemento.type != "radio"
      )
        elemento.value = "";
      if (elemento.type == "radio") {
        elemento.checked = false;
      }
    }
  }
  if (blnLimpar) {
    try {
      spnListaIdFis.innerHTML = "";
    } catch (e) {}
  }
  //Limpar os controle de busca de esta��o Destino/Origem e de Cidade(CNL)
  document.Form2.hdnCNLAtual.value = "";
  document.Form2.hdnCNLAtual1.value = "";
  document.Form2.hdnEstacaoOrigem.value = "";
  document.Form2.hdnEstacaoDestino.value = "";

  try {
    document.Form2.btnAddAcesso.value = "Adicionar";
  } catch (e) {}

  intIndice += 1;
  document.Form2.hdnIntIndice.value = "";
}

function getTextWithNameAndValue(selectName, optionValue) {
  var selects = document.getElementsByName(selectName);

  for (var i = 0; i < selects.length; i++) {
    var selectElem = selects[i];
    //       if (selectElem.tagName && selectElem.tagName.toLowerCase() === "select") {
    var options = selectElem.options;
    for (var j = 0; j < options.length; j++) {
      if (options[j].value == optionValue) {
        return options[j].text;
      }
      //         }
    }
  }
  return null; // Not found
}

function xmlUpd(blnLimpar) {
  var intNode = "";
  var objNodeRequest4 = "";
  var node = "";
  if (!document.Form2.hdnIntIndice.value) {
    node = objXmlGeral.selectSingleNode("//xDados");

    objNodeRequest4 = node.getElementsByTagName("intIndice");
    intNode = RequestNodeAcesso(
      objXmlGeral,
      "intIndice",
      objNodeRequest4.length
    );

    if (intNode) {
      document.Form2.hdnIntIndice.value = parseInt(intNode) + 1;
    } else {
      document.Form2.hdnIntIndice.value = 1;
    }
  }

  for (
    var intIndex = 0;
    intIndex < document.Form2.elements.length;
    intIndex++
  ) {
    var elemento = document.Form2.elements[intIndex];
    if (intIndex != 58 || intIndex != 59) {
      //@@JKNUP: guardar o CNL cliente para recuperar REDIRECIONAMENTO
      if (elemento.type == "text" && elemento.name == "txtEndCid") {
        document.Form2.hdnCNLCliente.value = elemento.value;
      }
      //--------------------------------------------------------------
    }

    if (elemento.type == "radio") {
      if (elemento.checked) {
        if (document.Form2.hdnIntIndice.value != "") {
          AdicionarNodeAcesso(
            objXmlGeral,
            elemento.name,
            elemento.value,
            document.Form2.hdnIntIndice.value
          );
          AdicionarNodeAcesso(
            objXmlGeral,
            elemento.name + "Index",
            elemento.Index,
            document.Form2.hdnIntIndice.value
          );
        } /**else {
          AdicionarNodeAcesso(
            objXmlGeral,
            elemento.name,
            elemento.value,
            intIndice
          );
          AdicionarNodeAcesso(
            objXmlGeral,
            elemento.name + "Index",
            elemento.Index,
            intIndice
          );
        } **/
      }
    } else {
      if (document.Form2.hdnIntIndice.value != "") {
        AdicionarNodeAcesso(
          objXmlGeral,
          elemento.name,
          elemento.value,
          document.Form2.hdnIntIndice.value
        );
      } /**else {
        AdicionarNodeAcesso(
          objXmlGeral,
          elemento.name,
          elemento.value,
          intIndice
        );
      } **/
    }

    //Cria um node com o text dos combos
    var txtFac = "";
    var txtTec = "";
    var nameValue = "";
    var htx = "";
    if (elemento.type == "select-one") {
      nameValue = elemento.name;
      if (document.Form2.hdnIntIndice.value != "") {
        if (elemento.name == "txtFacilidade") {
          if (document.Form2.hdnIntIndice.value == "1") {
            htx = document.Form1.hdntxtFacilidade1.value;
            txtFac = getTextWithNameAndValue("txtFacilidade", htx);
          } else {
            var cpo = document.getElementsByName("txtFacilidade");
            htx = cpo[0].value; //document.Form1.hdntxtFacilidade2.value;
            txtFac = getTextWithNameAndValue("txtFacilidade", htx);
          }
          AdicionarNodeAcesso(
            objXmlGeral,
            elemento.name + "Text",
            txtFac,
            document.Form2.hdnIntIndice.value
          );
          AdicionarNodeAcesso(
            objXmlGeral,
            elemento.name,
            txtFac,
            document.Form2.hdnIntIndice.value
          );
          AdicionarNodeAcesso(
            objXmlGeral,
            "hdn" + elemento.name,
            txtFac,
            document.Form2.hdnIntIndice.value
          );
        } else if (elemento.name == "cboTecnologia") {
          if (document.Form2.hdnIntIndice.value == "1") {
            htx = document.Form1.hdnTecnologia1.value;
            txtTec = getTextWithNameAndValue("cboTecnologia", htx);
          } else {
            cpo = document.getElementsByName("cboTecnologia");
            htx = cpo[0].value;
            txtTec = getTextWithNameAndValue("cboTecnologia", htx);
          }
          AdicionarNodeAcesso(
            objXmlGeral,
            elemento.name + "Text",
            txtTec,
            document.Form2.hdnIntIndice.value
          );
        } else {
          AdicionarNodeAcesso(
            objXmlGeral,
            elemento.name + "Text",
            elemento[elemento.selectedIndex].text,
            document.Form2.hdnIntIndice.value
          );
        }
      } else {
        if (elemento.name == "txtFacilidade") {
          if (intIndice == "0") {
            htx = document.Form1.hdntxtFacilidade1.value;
            txtFac = getTextWithNameAndValue("txtFacilidade", htx);
          } else {
            htx = document.Form1.hdntxtFacilidade2.value;
            txtFac = getTextWithNameAndValue("txtFacilidade", htx);
          }

          AdicionarNodeAcesso(
            objXmlGeral,
            elemento.name + "Text",
            txtFac,
            intIndice
          );
          AdicionarNodeAcesso(objXmlGeral, elemento.name, txtFac, intIndice);
          AdicionarNodeAcesso(
            objXmlGeral,
            "hdn" + elemento.name,
            txtFac,
            intIndice
          );
        } else if (elemento.name == "cboTecnologia") {
          if (intIndice == "0") {
            htx = document.Form1.hdnTecnologia1.value;
            txtTec = getTextWithNameAndValue("cboTecnologia", htx);
          } else {
            htx = document.Form1.hdnTecnologia2.value;
            txtTec = getTextWithNameAndValue("cboTecnologia", htx);
          }

          AdicionarNodeAcesso(
            objXmlGeral,
            elemento.name + "Text",
            txtTec,
            intIndice
          );
        } else {
          AdicionarNodeAcesso(
            objXmlGeral,
            elemento.name + "Text",
            elemento[elemento.selectedIndex].text,
            intIndice
          );
        }
      }
    }

    if (blnLimpar) {
      //N�o pode limpar o conte�do do radio somente fazer checked=false
      if (
        elemento.type != "button" &&
        elemento.type != "hidden" &&
        elemento.type != "radio"
      )
        elemento.value = "";
      if (elemento.type == "radio") {
        elemento.checked = false;
      }
    }

    if (document.Form2.hdnIntIndice.value != "") {
      if (
        RequestNodeAcesso(
          objXmlGeral,
          "Acf_Id",
          document.Form2.hdnIntIndice.value
        ) != ""
      ) {
        AdicionarNodeAcesso(
          objXmlGeral,
          "TipoAcao",
          "A",
          document.Form2.hdnIntIndice.value
        );
      } else {
        AdicionarNodeAcesso(
          objXmlGeral,
          "TipoAcao",
          "N",
          document.Form2.hdnIntIndice.value
        );
      }
    } else {
      if (document.forms[1].strOrigemAPG.value == "APG") {
        AdicionarNodeAcesso(
          objXmlGeral,
          "TipoAcao",
          "A",
          document.Form2.hdnIntIndice.value
        );
      } else {
        AdicionarNodeAcesso(
          objXmlGeral,
          "TipoAcao",
          "N",
          document.Form2.hdnIntIndice.value
        );
      }
    }
  }

  if (blnLimpar) {
    try {
      spnListaIdFis.innerHTML = "";
    } catch (e) {}
  }

  //Limpar os controle de busca de esta��o Destino/Origem e de Cidade(CNL)
  document.Form2.hdnCNLAtual.value = "";
  document.Form2.hdnCNLAtual1.value = "";
  document.Form2.hdnEstacaoOrigem.value = "";
  document.Form2.hdnEstacaoDestino.value = "";

  var objNode = objXmlGeral.selectNodes("//Acesso");
  if (objNode.length == 1) {
    for (var intIndex = 0; intIndex < objNode.length; intIndex++) {
      intChave = objNode[intIndex].childNodes[0].text;
      if (intChave == 1) {
        if (document.Form1.hdnTecnologia1.value) {
          AdicionarNodeAcesso(
            objXmlGeral,
            "hdnTecnologia1",
            document.Form1.hdnTecnologia1.value,
            intChave
          );
          AdicionarNodeAcesso(
            objXmlGeral,
            "hdncboTecnologia",
            document.Form1.hdnTecnologia1.value,
            intChave
          );
        }
        if (document.Form1.hdnTecnologia1.value) {
          AdicionarNodeAcesso(
            objXmlGeral,
            "hdntxtFacilidade1",
            document.Form1.hdntxtFacilidade1.value,
            intChave
          );
          AdicionarNodeAcesso(
            objXmlGeral,
            "hdncboFacilidade",
            document.Form1.hdntxtFacilidade1.value,
            intChave
          );
        }
        var tds = document.getElementsByName("txtPadrao")[0];
        if (tds) {
          AdicionarNodeAcesso(
            objXmlGeral,
            "txtdesignacaoServico",
            tds.value,
            intChave
          );
        }
      }
    }
  } else {
    intChave = objNode[objNode.length - 1].childNodes[0].text;

    if (document.Form1.hdnTecnologia1.value) {
      AdicionarNodeAcesso(
        objXmlGeral,
        "hdnTecnologia2",
        document.Form1.hdnTecnologia2.value,
        intChave
      );
      AdicionarNodeAcesso(
        objXmlGeral,
        "hdncboTecnologia",
        document.Form1.hdnTecnologia2.value,
        intChave
      );
    }
    if (document.Form1.hdnTecnologia1.value) {
      AdicionarNodeAcesso(
        objXmlGeral,
        "hdntxtFacilidade2",
        document.Form1.hdntxtFacilidade2.value,
        intChave
      );
      AdicionarNodeAcesso(
        objXmlGeral,
        "hdncboFacilidade",
        document.Form1.hdntxtFacilidade2.value,
        intChave
      );
    }
    var tds2 = document.getElementsByName("txtPadrao")[0];
    if (tds2) {
      AdicionarNodeAcesso(
        objXmlGeral,
        "txtdesignacaoServico",
        tds2.value,
        intChave
      );
    }
  }
  btnAddAcesso = document.getElementsByName("btnAddAcesso")[0];
  if (btnAddAcesso) {
    btnAddAcesso.value = "Adicionar";
  }
  intIndice += 1;
  document.Form2.hdnIntIndice.value = "";
}

/*
=====================================================================================
Objetivo		: Adiciona item a lista que acessos adicionados
Premissas		: Existir objeto XML na p�gina que faz o include
Entradas		: 
Retorno			: Mostra Html no Iframe de acessos adicionados
=====================================================================================
*/
//'*******************************************
//' Good Início
//'*******************************************
function AtualizarLista_OLD() {
  if (arguments.length > 0) {
    var intOptSel = arguments[0];
  } else {
    var intOptSel = 9999;
  }
  var objNode = objXmlGeral.selectNodes(
    "//xDados/Acesso[TipoAcao='N' || TipoAcao='A']"
  );

  var strAcesso = new String(
    "<table border=0 width=740 cellspacing=1 cellpadding=0>"
  );
  //Refaz a lista de Ids no IFRAME
  for (var intIndex = 0; intIndex < objNode.length; intIndex++) {
    if (intIndex % 2 != 0) {
      strAcesso += "<tr class=clsSilver>";
    } else {
      strAcesso += "<tr class=clsSilver2>";
    }
    intChave = objNode[intIndex].childNodes[0].text;
    strAcesso +=
      "<td width=15>" +
      RequestNodeAcesso(objXmlGeral, "intOrdem", intChave) +
      "</td>";
    if (intOptSel == intIndex) {
      strAcesso +=
        "<td width=35 align=center><input type=radio name=rdoAcesso value=" +
        intChave +
        " onClick='parent.EditarAcessoLista(this)' checked ></td>";
    } else {
      strAcesso +=
        "<td width=35 align=center><input type=radio name=rdoAcesso value=" +
        intChave +
        " onClick='parent.EditarAcessoLista(this)'></td>";
    }
    strAcesso +=
      "<td width=50>" +
      RequestNodeAcesso(objXmlGeral, "rdoPropAcessoFisico", intChave) +
      "</td>";
    strAcesso +=
      "<td width=185>" +
      RequestNodeAcesso(objXmlGeral, "cboProvedorText", intChave) +
      "</td>";
    if (RequestNodeAcesso(objXmlGeral, "cboTipoVel", intChave) != "") {
      strAcesso +=
        "<td width=200>" +
        RequestNodeAcesso(objXmlGeral, "cboVelAcessoText", intChave) +
        " " +
        RequestNodeAcesso(objXmlGeral, "cboTipoVelText", intChave) +
        "</td>";
    } else {
      strAcesso +=
        "<td width=200>" +
        RequestNodeAcesso(objXmlGeral, "cboVelAcessoText", intChave) +
        "</td>";
    }
    //Endere�o
    strAcesso +=
      "<td width=255>" +
      RequestNodeAcesso(objXmlGeral, "cboLogrEnd", intChave) +
      " " +
      RequestNodeAcesso(objXmlGeral, "txtEnd", intChave);
    strAcesso += ", " + RequestNodeAcesso(objXmlGeral, "txtNroEnd", intChave);
    strAcesso += " " + RequestNodeAcesso(objXmlGeral, "txtComplEnd", intChave);
    strAcesso += " " + RequestNodeAcesso(objXmlGeral, "txtCepEnd", intChave);
    strAcesso += " " + RequestNodeAcesso(objXmlGeral, "txtEndCid", intChave);
    strAcesso +=
      " " + RequestNodeAcesso(objXmlGeral, "txtEndCidDesc", intChave);
    strAcesso += "</td></tr>";
  }
  strAcesso += "</table>";

  //alert(strAcesso)
  parent.IFrmAcessoFis.spnAcessoFis.innerHTML = strAcesso;
}
function AtualizarLista() {
  if (arguments.length > 0) {
    var intOptSel = arguments[0];
  } else {
    var intOptSel = 9999;
  }

  var objNode = objXmlGeral.selectNodes(
    "//xDados/Acesso[TipoAcao='N' || TipoAcao='A']"
  );
  var strAcesso = new String(
    "<table border=0 width=760 cellspacing=1 cellpadding=0>"
  );
  //Refaz a lista de Ids no IFRAME

  for (var intIndex = 0; intIndex < objNode.length; intIndex++) {
    if (intIndex % 2 != 0) {
      strAcesso += "<tr class=clsSilver>";
    } else {
      strAcesso += "<tr class=clsSilver2>";
    }
    intChave = objNode[intIndex].childNodes[0].text;

    strAcesso +=
      "<td width=15>" +
      RequestNodeAcesso(objXmlGeral, "intOrdem", intChave) +
      "</td>";
    if (intOptSel == intIndex) {
      strAcesso +=
        "<td width=35 align=center><input type=radio id=rdoAcesso name=rdoAcesso value=" +
        intChave +
        " onClick='parent.EditarAcessoLista(this)' checked ></td>";
    } else {
      strAcesso +=
        "<td width=35 align=center><input type=radio id=rdoAcesso name=rdoAcesso value=" +
        intChave +
        " onClick='parent.EditarAcessoLista(this)'></td>";
    }
    strAcesso +=
      "<td width=50>" +
      RequestNodeAcesso(objXmlGeral, "rdoPropAcessoFisico", intChave) +
      "</td>";
    strAcesso +=
      "<td width=185>" +
      RequestNodeAcesso(objXmlGeral, "cboProvedorText", intChave) +
      "</td>";
    //----------------------------------
    //  Good início
    //----------------------------------
    switch (intChave) {
      case 1:
        document.Form1.hdnTecnologia1.value = RequestNodeAcesso(
          objXmlGeral,
          "hdnTecnologia1",
          intChave
        );
        document.Form12.hdntxtFacilidade1.value = RequestNodeAcesso(
          objXmlGeral,
          "hdntxtFacilidade1",
          intChave
        );

        break;
      case 2:
        document.Form1.hdnTecnologia2.value = RequestNodeAcesso(
          objXmlGeral,
          "hdnTecnologia2",
          intChave
        );
        document.Form1.hdntxtFacilidade2.value = RequestNodeAcesso(
          objXmlGeral,
          "hdntxtFacilidade1",
          intChave
        );
        break;
    }
    //----------------------------------
    //  Good fim
    //----------------------------------
    document.Form2.hdnCboProvedor.value = RequestNodeAcesso(
      objXmlGeral,
      "cboProvedorText",
      intChave
    );
    document.Form2.hdnPropIdFisico.value = RequestNodeAcesso(
      objXmlGeral,
      "rdoPropAcessoFisico",
      intChave
    );

    if (RequestNodeAcesso(objXmlGeral, "cboTipoVel", intChave) != "") {
      strAcesso +=
        "<td width=200>" +
        RequestNodeAcesso(objXmlGeral, "cboVelAcessoText", intChave) +
        " " +
        RequestNodeAcesso(objXmlGeral, "cboTipoVelText", intChave) +
        "</td>";
    } else {
      strAcesso +=
        "<td width=200>" +
        RequestNodeAcesso(objXmlGeral, "cboVelAcessoText", intChave) +
        "</td>";
    }
    //Endere�o
    strAcesso +=
      "<td width=255>" +
      RequestNodeAcesso(objXmlGeral, "cboLogrEnd", intChave) +
      " " +
      RequestNodeAcesso(objXmlGeral, "txtEnd", intChave);
    strAcesso += ", " + RequestNodeAcesso(objXmlGeral, "txtNroEnd", intChave);
    strAcesso += " " + RequestNodeAcesso(objXmlGeral, "txtComplEnd", intChave);
    strAcesso += " " + RequestNodeAcesso(objXmlGeral, "txtCepEnd", intChave);
    strAcesso += " " + RequestNodeAcesso(objXmlGeral, "txtEndCid", intChave);
    strAcesso +=
      " " + RequestNodeAcesso(objXmlGeral, "txtEndCidDesc", intChave);
    strAcesso += "</td></tr>";
  }
  strAcesso += "</table>";

  parent.IFrmAcessoFis.spnAcessoFis.innerHTML = strAcesso;
}
//'*******************************************
//' Good fim
//'*******************************************
/*
Eduardo Araujo   Analista Programador 
02/04/2007
=====================================================================================
Objetivo		: Adiciona item a lista que acessos adicionados do APG 
Premissas		: Existir objeto XML na p�gina que faz o include
Entradas		: 
Retorno			: Mostra Html no Iframe de acessos adicionados
=====================================================================================
*/
function AtualizarListaApg() {
  if (arguments.length > 0) {
    var intOptSel = arguments[0];
  } else {
    var intOptSel = 9999;
  }
  var objNode = objXmlGeral.selectNodes(
    "//xDados/Acesso[TipoAcao='N' || TipoAcao='A']"
  );

  var strAcesso = new String(
    "<table border=0 width=740 cellspacing=1 cellpadding=0>"
  );
  //Refaz a lista de Ids no IFRAME
  for (var intIndex = 0; intIndex < objNode.length; intIndex++) {
    if (intIndex % 2 != 0) {
      strAcesso += "<tr class=clsSilver>";
    } else {
      strAcesso += "<tr class=clsSilver2>";
    }
    intChave = objNode[intIndex].childNodes[0].text;
    strAcesso +=
      "<td width=15>" +
      RequestNodeAcesso(objXmlGeral, "intOrdem", intChave) +
      "</td>";
    if (intOptSel == intIndex) {
      strAcesso +=
        "<td width=35 align=center><input type=radio name=rdoAcesso value=" +
        intChave +
        " onClick='parent.EditarAcessoLista(this)' checked ></td>";
    } else {
      strAcesso +=
        "<td width=35 align=center><input type=radio name=rdoAcesso value=" +
        intChave +
        " onClick='parent.EditarAcessoLista(this)'></td>";
    }
    strAcesso +=
      "<td width=50>" +
      RequestNodeAcesso(objXmlGeral, "rdoPropAcessoFisico", intChave) +
      "</td>";
    strAcesso +=
      "<td width=185>" +
      RequestNodeAcesso(objXmlGeral, "cboProvedorText", intChave) +
      "</td>";
    if (RequestNodeAcesso(objXmlGeral, "cboTipoVel", intChave) != "") {
      strAcesso +=
        "<td width=200>" +
        RequestNodeAcesso(objXmlGeral, "cboVelAcessoText", intChave) +
        " " +
        RequestNodeAcesso(objXmlGeral, "cboTipoVelText", intChave) +
        "</td>";
    } else {
      strAcesso +=
        "<td width=200>" +
        RequestNodeAcesso(objXmlGeral, "cboVelAcessoText", intChave) +
        "</td>";
    }
    //Endere�o
    strAcesso +=
      "<td width=255>" +
      RequestNodeAcesso(objXmlGeral, "cboLogrEnd", intChave) +
      " " +
      RequestNodeAcesso(objXmlGeral, "txtEnd", intChave);
    strAcesso += ", " + RequestNodeAcesso(objXmlGeral, "txtNroEnd", intChave);
    strAcesso += " " + RequestNodeAcesso(objXmlGeral, "txtComplEnd", intChave);
    strAcesso += " " + RequestNodeAcesso(objXmlGeral, "txtCepEnd", intChave);
    strAcesso += " " + RequestNodeAcesso(objXmlGeral, "txtEndCid", intChave);
    strAcesso +=
      " " + RequestNodeAcesso(objXmlGeral, "txtEndCidDesc", intChave);
    strAcesso += "</td></tr>";
  }
  strAcesso += "</table>";

  parent.IFrmAcessoFisApg.spnAcessoFisApg.innerHTML = strAcesso;
}

/*
function xmlUpdApg(blnLimpar)
{
	for (var intIndex=0;intIndex<document.Form2.elements.length;intIndex++)
	{
		var elemento = document.Form2.elements[intIndex];
		
		

		if(intIndex != 58 || intIndex != 59){
			//@@JKNUP: guardar o CNL cliente para recuperar REDIRECIONAMENTO
			if (elemento.type == 'text' && elemento.name == 'txtEndCid'){
				document.Form2.hdnCNLCliente.value = elemento.value
			}
			//--------------------------------------------------------------
		}

		
		
		if (elemento.type == "radio"){
			if (elemento.checked){
				if (document.Form2.hdnIntIndice.value != ""){
					AdicionarNodeAcesso(objXmlApg,elemento.name,elemento.value,document.Form2.hdnIntIndice.value)
					AdicionarNodeAcesso(objXmlApg,elemento.name+"Index",elemento.Index,document.Form2.hdnIntIndice.value)
				}
				else{
					AdicionarNodeAcesso(objXmlApg,elemento.name,elemento.value,intIndice)
					AdicionarNodeAcesso(objXmlApg,elemento.name+"Index",elemento.Index,intIndice)
				}	
			}	
		}
		else{
			if (document.Form2.hdnIntIndice.value != ""){
				AdicionarNodeAcesso(objXmlApg,elemento.name,elemento.value,document.Form2.hdnIntIndice.value)
			}
			else{
				AdicionarNodeAcesso(objXmlApg,elemento.name,elemento.value,intIndice)
			}	
		}	
		//Cria um node com o text dos combos
		if (elemento.type == "select-one"){
			if (document.Form2.hdnIntIndice.value != ""){
				AdicionarNodeAcesso(objXmlApg,elemento.name+"Text",elemento[elemento.selectedIndex].text,document.Form2.hdnIntIndice.value)
			}
			else{	
				AdicionarNodeAcesso(objXmlApg,elemento.name+"Text",elemento[elemento.selectedIndex].text,intIndice)
			}
		}

		if (blnLimpar){
			//N�o pode limpar o conte�do do radio somente fazer checked=false
			if (elemento.type != 'button' && elemento.type != 'hidden' && elemento.type != 'radio')	elemento.value = ""
			if (elemento.type == 'radio')
			{
				elemento.checked = false
			}
		}	
	}
	if (blnLimpar)
	{
		try{
			spnListaIdFis.innerHTML = ""
		}catch(e){}	
	}	
	//Limpar os controle de busca de esta��o Destino/Origem e de Cidade(CNL)
	document.Form2.hdnCNLAtual.value = ""
	document.Form2.hdnCNLAtual1.value = ""
	document.Form2.hdnEstacaoOrigem.value = ""
	document.Form2.hdnEstacaoDestino.value = ""

	if (document.forms[1].hdnIntIndice.value != ""){
		if (RequestNodeAcesso(objXmlApg,"Acf_Id",document.forms[1].hdnIntIndice.value) != ""){
			AdicionarNodeAcesso(objXmlApg,'TipoAcao','A',document.forms[1].hdnIntIndice.value)
		}else{
			AdicionarNodeAcesso(objXmlApg,'TipoAcao','N',document.forms[1].hdnIntIndice.value)
		}	
	}
	else{	
		AdicionarNodeAcesso(objXmlApg,'TipoAcao','N',intIndice)
	}

	try{
		document.Form2.btnAddAcesso.value = "Adicionar"
	}catch(e){}

	intIndice += 1
	document.Form2.hdnIntIndice.value = ""
}

*/

/*
Eduardo Araujo   Analista Programador 
02/04/2007
=====================================================================================
Objetivo		:  lista o Ponto de Instala��o do processo ativo anterior 
Premissas		: Existir objeto XML na p�gina que faz o include
Entradas		: 
Retorno		: Mostra Html no Iframe de acessos adicionados
=====================================================================================
*/
function ListaAnterior() {
  if (arguments.length > 0) {
    var intOptSel = arguments[0];
  } else {
    var intOptSel = 9999;
  }
  var objNode = objXmlFisicoAntigo.selectNodes(
    "//xDados/Acesso[TipoAcao='N' || TipoAcao='A']"
  );

  var strAcesso = new String(
    "<table border=0 width=740 cellspacing=1 cellpadding=0>"
  );
  //Refaz a lista de Ids no IFRAME
  for (var intIndex = 0; intIndex < objNode.length; intIndex++) {
    if (intIndex % 2 != 0) {
      strAcesso += "<tr class=clsSilver>";
    } else {
      strAcesso += "<tr class=clsSilver2>";
    }
    intChave = objNode[intIndex].childNodes[0].text;
    strAcesso +=
      "<td width=15>" +
      RequestNodeAcesso(objXmlFisicoAntigo, "intOrdem", intChave) +
      "</td>";
    strAcesso +=
      "<td width=35 align=center><input type=radio name=rdoAcesso value=" +
      intChave +
      " onClick='parent.PopupProcessoAtivoAnt(this)'></td>";
    strAcesso +=
      "<td width=50>" +
      RequestNodeAcesso(objXmlFisicoAntigo, "rdoPropAcessoFisico", intChave) +
      "</td>";
    strAcesso +=
      "<td width=185>" +
      RequestNodeAcesso(objXmlFisicoAntigo, "cboProvedorText", intChave) +
      "</td>";
    if (RequestNodeAcesso(objXmlFisicoAntigo, "cboTipoVel", intChave) != "") {
      strAcesso +=
        "<td width=200>" +
        RequestNodeAcesso(objXmlFisicoAntigo, "cboVelAcessoText", intChave) +
        " " +
        RequestNodeAcesso(objXmlFisicoAntigo, "cboTipoVelText", intChave) +
        "</td>";
    } else {
      strAcesso +=
        "<td width=200>" +
        RequestNodeAcesso(objXmlFisicoAntigo, "cboVelAcessoText", intChave) +
        "</td>";
    }
    //Endere�o
    strAcesso +=
      "<td width=255>" +
      RequestNodeAcesso(objXmlFisicoAntigo, "cboLogrEnd", intChave) +
      " " +
      RequestNodeAcesso(objXmlFisicoAntigo, "txtEnd", intChave);
    strAcesso +=
      ", " + RequestNodeAcesso(objXmlFisicoAntigo, "txtNroEnd", intChave);
    strAcesso +=
      " " + RequestNodeAcesso(objXmlFisicoAntigo, "txtComplEnd", intChave);
    strAcesso +=
      " " + RequestNodeAcesso(objXmlFisicoAntigo, "txtCepEnd", intChave);
    strAcesso +=
      " " + RequestNodeAcesso(objXmlFisicoAntigo, "txtEndCid", intChave);
    strAcesso +=
      " " + RequestNodeAcesso(objXmlFisicoAntigo, "txtEndCidDesc", intChave);
    strAcesso += "</td></tr>";
  }
  strAcesso += "</table>";

  parent.IFrmAcessoFisAnterior.spnAcessoFisAnterior.innerHTML = strAcesso;
}

/*
=====================================================================================
Objetivo		: Popula os campos de acesso f�sico
Premissas		: Existir objeto XML na p�gina que faz o include
Entradas		: intChave -  Chave identificadora do item editado
Retorno			: Atualiza campos do formul�rio com inform��o
=====================================================================================
*/

function EditarAcesso(intChave) {
  var objNode = objXmlGeral.selectNodes(
    "//xDados/Acesso[intIndice=" + parseInt(intChave) + "]"
  );
  //Refaz a lista de Ids no IFRAME
  for (var intIndex = 0; intIndex < objNode.length; intIndex++) {
    for (
      var intIndexII = 0;
      intIndexII < objNode[intIndex].childNodes.length;
      intIndexII++
    ) {
      try {
        //Caso de radio button
        if (
          objNode[intIndex].childNodes[intIndexII].nodeName ==
          "rdoPropAcessoFisico"
        ) {
          eval(
            "document.Form2." +
              objNode[intIndex].childNodes[intIndexII].nodeName +
              "[" +
              RequestNodeAcesso(
                objXmlGeral,
                "rdoPropAcessoFisicoIndex",
                objNode[intIndex].childNodes[0].text
              ) +
              "].checked = true"
          );
        } else {
          //eval("document.Form2."+objNode[intIndex].childNodes[intIndexII].nodeName+".value='"+objNode[intIndex].childNodes[intIndexII].text+"'")

          var objChildForm = new Object(
            eval(
              "document.Form2." +
                objNode[intIndex].childNodes[intIndexII].nodeName
            )
          );
          //alert(objNode[intIndex].childNodes[intIndexII].text + " = " + objNode[intIndex].childNodes[intIndexII].nodeName)
          if (
            objChildForm.type != "button" &&
            objChildForm.type != "select-one"
          ) {
            objChildForm.value = objNode[intIndex].childNodes[intIndexII].text;
          }
        }
      } catch (e) {}
    }
  }

  //**********************
  //Good início
  //**********************
  //  if ( document.Form1.hdnAcaoMain.value != 'ALT' && RequestNodeAcesso(objXmlGeral, "TipoAcao", objChave.value) != "A" ){
  //     document.getElementsByName("txtFacilidade")[0].value = document.Form1.hdntxtFacilidade.value;
  // }
  //**********************
  //Good fim
  //**********************
  if (objNode.length > 0) {
    var intChaveFis = RequestNodeAcesso(objXmlGeral, "Aec_Id", intChave);
    if (intChaveFis == "") intChaveFis = 0;
    var objNodeFis = objXmlGeral.selectNodes(
      "//xDados/Acesso/IdFisico[Aec_Id=" + intChaveFis + "]"
    );
    var strAcessoIdFis = new String(
      "<table border=0 width=759 cellspacing=0 cellpadding=0>"
    );
    //Refaz a lista de Ids no IFRAME
    for (var intIndex = 0; intIndex < objNodeFis.length; intIndex++) {
      var intAcfId = objNodeFis[intIndex].childNodes[0].text;
      var intAecId = objNodeFis[intIndex].childNodes[2].text;
      var objNodePed = objXmlGeral.selectNodes(
        "//xDados/Acesso/Pedido[Acf_Id=" + intAcfId + "]"
      );
      strAcessoIdFis += "<tr class=clsSilver>";
      strAcessoIdFis += "<td >&nbsp;Pedido</td>";
      strAcessoIdFis +=
        "<td>&nbsp;" + objNodePed[0].childNodes[1].text + "</td>";
      strAcessoIdFis += "<td >&nbsp;ID Físico</td>";
      strAcessoIdFis += "<td >";
      try {
        strAcessoIdFis += objNodeFis[intIndex].childNodes[3].text;
      } catch (e) {}
      strAcessoIdFis += "</td>";
      strAcessoIdFis += "<td >&nbsp;N� Acesso</td>";
      strAcessoIdFis += "<td >";
      try {
        strAcessoIdFis += objNodeFis[intIndex].childNodes[4].text;
      } catch (e) {}
      strAcessoIdFis += "</td>";
      strAcessoIdFis += "</tr>";
      strAcessoIdFis += "<tr></tr>";
    }
    strAcessoIdFis += "</table>";
    //Qtde circuitos
    var objNodeCircuitos = objXmlGeral.selectNodes(
      "//xDados/Acesso/Pedido[Aec_Id=" + intAecId + "]"
    );
    if (objNodeCircuitos.length > 0) {
      var intQtdeCir = 1;
      var intQtdeCirAtual = RequestNodeAcesso(
        objXmlGeral,
        "txtQtdeCircuitos",
        intChave
      );
      if (intQtdeCirAtual == "") {
        intQtdeCirAtual = 1;
      }
      var intPedId = objNodeCircuitos[0].childNodes[0].text;
      for (var intIndex = 0; intIndex < objNodeCircuitos.length; intIndex++) {
        if (intPedId != objNodeCircuitos[intIndex].childNodes[0].text) {
          intQtdeCir += 1;
        }
      }
      if (document.Form2.txtQtdeCircuitos) {
        if (intQtdeCirAtual == intQtdeCir) {
          document.Form2.txtQtdeCircuitos.value = intQtdeCir;
        } else {
          document.Form2.txtQtdeCircuitos.value = intQtdeCirAtual;
        }
      }
    }
  } else {
    strAcessoIdFis = "";
  }

  document.Form2.hdnIntIndice.value = intChave; //Chave Atual no Html
  //Acerta o disabled para o provedor quando temos EBT
  if (
    RequestNodeAcesso(objXmlGeral, "rdoPropAcessoFisico", intChave) != "EBT"
  ) {
    document.forms[1].cboProvedor.disabled = false;
  } else {
    document.forms[1].cboProvedor.disabled = true;
  }

  DesabilitarCamposAlt(true, intChave);

  parent.spnListaIdFis.innerHTML = strAcessoIdFis;
}

function EditarAcessoOLD(intChave) {
  var objNode = objXmlGeral.selectNodes(
    "//xDados/Acesso[intIndice=" + parseInt(intChave) + "]"
  );
  //Refaz a lista de Ids no IFRAME

  for (var intIndex = 0; intIndex < objNode.length; intIndex++) {
    for (
      var intIndexII = 0;
      intIndexII < objNode[intIndex].childNodes.length;
      intIndexII++
    ) {
      try {
        //Caso de radio button
        if (
          objNode[intIndex].childNodes[intIndexII].nodeName ==
          "rdoPropAcessoFisico"
        ) {
          eval(
            "document.Form2." +
              objNode[intIndex].childNodes[intIndexII].nodeName +
              "[" +
              RequestNodeAcesso(
                objXmlGeral,
                "rdoPropAcessoFisicoIndex",
                objNode[intIndex].childNodes[0].text
              ) +
              "].checked = true"
          );
        } else {
          //eval("document.Form2."+objNode[intIndex].childNodes[intIndexII].nodeName+".value='"+objNode[intIndex].childNodes[intIndexII].text+"'")
          var objChildForm = new Object(
            eval(
              "document.Form2." +
                objNode[intIndex].childNodes[intIndexII].nodeName
            )
          );
          //alert(objNode[intIndex].childNodes[intIndexII].text + " = " + objNode[intIndex].childNodes[intIndexII].nodeName)
          if (objChildForm.type != "button") {
            objChildForm.value = objNode[intIndex].childNodes[intIndexII].text;
          }
        }
      } catch (e) {}
    }
  }

  document.getElementsByName("txtFacilidade")[0].value =
    document.Form1.hdntxtFacilidade.value;

  if (objNode.length > 0) {
    var intChaveFis = RequestNodeAcesso(objXmlGeral, "Aec_Id", intChave);
    if (intChaveFis == "") intChaveFis = 0;
    var objNodeFis = objXmlGeral.selectNodes(
      "//xDados/Acesso/IdFisico[Aec_Id=" + intChaveFis + "]"
    );
    var strAcessoIdFis = new String(
      "<table border=0 width=759 cellspacing=0 cellpadding=0>"
    );
    //Refaz a lista de Ids no IFRAME
    for (var intIndex = 0; intIndex < objNodeFis.length; intIndex++) {
      var intAcfId = objNodeFis[intIndex].childNodes[0].text;
      var intAecId = objNodeFis[intIndex].childNodes[2].text;
      var objNodePed = objXmlGeral.selectNodes(
        "//xDados/Acesso/Pedido[Acf_Id=" + intAcfId + "]"
      );
      strAcessoIdFis += "<tr class=clsSilver>";
      strAcessoIdFis += "<td >&nbsp;Pedido</td>";
      strAcessoIdFis +=
        "<td>&nbsp;" + objNodePed[0].childNodes[1].text + "</td>";
      strAcessoIdFis += "<td >&nbsp;ID F�sico</td>";
      strAcessoIdFis += "<td >";
      try {
        strAcessoIdFis += objNodeFis[intIndex].childNodes[3].text;
      } catch (e) {}
      strAcessoIdFis += "</td>";
      strAcessoIdFis += "<td >&nbsp;N� Acesso</td>";
      strAcessoIdFis += "<td >";
      try {
        strAcessoIdFis += objNodeFis[intIndex].childNodes[4].text;
      } catch (e) {}
      strAcessoIdFis += "</td>";
      strAcessoIdFis += "</tr>";
      strAcessoIdFis += "<tr></tr>";
    }
    strAcessoIdFis += "</table>";
    //Qtde circuitos
    var objNodeCircuitos = objXmlGeral.selectNodes(
      "//xDados/Acesso/Pedido[Aec_Id=" + intAecId + "]"
    );
    if (objNodeCircuitos.length > 0) {
      var intQtdeCir = 1;
      var intQtdeCirAtual = RequestNodeAcesso(
        objXmlGeral,
        "txtQtdeCircuitos",
        intChave
      );
      if (intQtdeCirAtual == "") {
        intQtdeCirAtual = 1;
      }
      var intPedId = objNodeCircuitos[0].childNodes[0].text;
      for (var intIndex = 0; intIndex < objNodeCircuitos.length; intIndex++) {
        if (intPedId != objNodeCircuitos[intIndex].childNodes[0].text) {
          intQtdeCir += 1;
        }
      }
      if (intQtdeCirAtual == intQtdeCir) {
        document.Form2.txtQtdeCircuitos.value = intQtdeCir;
      } else {
        document.Form2.txtQtdeCircuitos.value = intQtdeCirAtual;
      }
    }
  } else {
    strAcessoIdFis = "";
  }

  document.Form2.hdnIntIndice.value = intChave; //Chave Atual no Html
  //Acerta o disabled para o provedor quando temos EBT
  if (
    RequestNodeAcesso(objXmlGeral, "rdoPropAcessoFisico", intChave) != "EBT"
  ) {
    document.forms[1].cboProvedor.disabled = false;
  } else {
    document.forms[1].cboProvedor.disabled = true;
  }

  DesabilitarCamposAlt(true, intChave);
  parent.spnListaIdFis.innerHTML = strAcessoIdFis;
}
/*
=====================================================================================
Objetivo		: Remove acesso do XML
Premissas		: Existir objeto XML na p�gina que faz o include
Entradas		: intIndice - Chave o item a ser removido
				  blnLimpar - booleana para limpar os campos do formul�rio depois de remover
Retorno			: Atualiza lista do Iframe de acessos adicionados
=====================================================================================
*/
function RemoverAcesso(intIndice, blnLimpar) {
  //alert(intIndice)
  //alert(blnLimpar)
  if (RequestNodeAcesso(objXmlGeral, "Acf_Id", intIndice) != "") {
    //alert("TipoAcao")
    AdicionarNodeAcesso(objXmlGeral, "TipoAcao", "R", intIndice);
  } else {
    //alert("remover")
    var objNode = objXmlGeral.selectNodes(
      "//xDados/Acesso[intIndice=" + parseInt(intIndice) + "]"
    );
    //alert(objNode.length)
    //alert(objNode[0])
    if (objNode.length > 0) {
      objNode[0].parentNode.removeChild(objNode[0]);
      document.Form2.hdnIntIndice.value = "";
    } else {
      alert("Item n�o encontrado.");
      return;
    }
  }

  for (
    var intIndex = 0;
    intIndex < document.Form2.elements.length;
    intIndex++
  ) {
    var elemento = document.Form2.elements[intIndex];
    if (blnLimpar) {
      if (
        elemento.type != "button" &&
        elemento.type != "hidden" &&
        elemento.type != "radio"
      )
        elemento.value = "";
      if (elemento.type == "radio") {
        elemento.checked = false;
      }
    }
  }
  AtualizarLista();
  if (blnLimpar) {
    parent.spnListaIdFis.innerHTML = "";
  }
}
/*
=====================================================================================
Objetivo		: Adiciona um Node a um objeto XML. Se existir atualiza
Premissas		: Enviar o objeto XML
Entradas		: objXMLProcesso - objeto XML
				  strNomeNode - Nome do Node
				  strValorNode - Valor do node 
				  intChave - Chave para identifica��o/cria��o do node
Retorno			: 
=====================================================================================
*/
function AdicionarNodeAcesso(
  objXMLProcesso,
  strNomeNode,
  strValorNode,
  intChave
) {
  var objElemento;
  var objNodeFilho;
  var intIndex;
  var objNodeList;
  var objNode;
  var objXML = objXMLProcesso;
  if (objXML.xml == "") {
    objXML.loadXML("<xDados></xDados>");
  }
  //Verifica se j� existe
  objNodeList = objXML.selectNodes(
    "//xDados/Acesso[intIndice=" + parseInt(intChave) + "]"
  );
  if (objNodeList.length == 0) {
    //Cria o Node Chave
    objNode = objXML.createNode("element", "Acesso", "");
    objXML.documentElement.appendChild(objNode);

    objNodeFilho = objXML.createNode("element", "intIndice", "");
    objNodeFilho.text = parseInt(intChave);
    objNode.appendChild(objNodeFilho);
  } else {
    objNode = objXML.selectSingleNode(
      "//xDados/Acesso[intIndice=" + parseInt(intChave) + "]"
    );
  }

  var objNodeListAtual = objNode.getElementsByTagName(strNomeNode);
  if (objNodeListAtual.length == 0) {
    //Cria
    if (strValorNode != "") {
      objNodeFilho = objXML.createNode("element", strNomeNode, "");
      objNodeFilho.text = strValorNode;
      objNode.appendChild(objNodeFilho);
    }
  } else {
    //Atualiza
    if (strValorNode != "") {
      objNodeListAtual.item(0).text = strValorNode;
    } else {
      //Se esta vazio remove do XML
      objNodeListAtual.item(0).parentNode.removeChild(objNodeListAtual.item(0));
    }
  }
}

function AdicionarNodeTronco2M(
  objXMLProcesso,
  strNomeNode,
  strValorNode,
  intChave
) {
  var objElemento;
  var objNodeFilho;
  var intIndex;
  var objNodeList;
  var objNode;
  var objXML = objXMLProcesso;

  objNode = objXML.createNode("element", "tronco2m", "");
  objXML.documentElement.appendChild(objNode);

  //Cria
  if (strValorNode != "") {
    objNodeFilho = objXML.createNode("element", strNomeNode, "");
    objNodeFilho.text = strValorNode;
    objNode.appendChild(objNodeFilho);
  }
}

//Verifica se o acesso foi alterado e n�o foi recolocado a lista
function AcessoAlteradoNaoAtualizado() {
  if (document.Form2.hdnIntIndice.value != "") {
    intChave = document.Form2.hdnIntIndice.value;
  } else {
    return false;
  }
  var objAry = new Array(
    "cboTecnologia",
    "cboVelAcesso",
    "cboTipoVel",
    "cboProvedor",
    "cboRegimeCntr",
    "cboPromocao",
    "cboUFEnd",
    "txtEndCid",
    "txtEndCidDesc",
    "cboLogrEnd",
    "txtEnd",
    "txtNroEnd",
    "txtBairroEnd",
    "txtCepEnd",
    "txtContatoEnd",
    "txtTelEnd",
    "txtCNPJ",
    "txtIE",
    "txtIM",
    "txtComplEnd",
    "txtPropEnd",
    "txtCodSAP",
    "txtNroPI",
    "cboInterFaceEnd",
    "cboTipoPonto",
    "hdnCompartilhamento",
    "hdnIdAcessoFisico",
    "hdnNovoPedido",
    "txtCNLSiglaCentroCliDest",
    "txtComplSiglaCentroCliDest",
    "cboInterFaceEndFis"
  );
  var intIndexRb = RequestNodeAcesso(
    objXmlGeral,
    "rdoPropAcessoFisicoIndex",
    intChave
  );
  if (intIndexRb != "") {
    if (!document.forms[1].rdoPropAcessoFisico[intIndexRb].checked) {
      return true;
    }
  }
  for (var intIndex = 0; intIndex < objAry.length; intIndex++) {
    var objChildForm = new Object(eval("document.Form2." + objAry[intIndex]));
    //alert(eval("document.Form2."+objAry[intIndex]+".name") + " : " + objChildForm.value + ' = ' + RequestNodeAcesso(objXmlGeral,objAry[intIndex],intChave))
    if (
      objChildForm.value !=
      RequestNodeAcesso(objXmlGeral, objAry[intIndex], intChave)
    ) {
      return true;
    }
  }
  return false;
}

function AddTaxaServico(objXMLProcesso, objXmlTaxa) {
  var objElemento;
  var objNodeFilho;
  var intIndex;
  var objNodeList;
  var objNode;
  var objXML = objXMLProcesso;
  if (objXML.xml == "") {
    objXML.loadXML("<xDados></xDados>");
  }
  //Verifica se j� existe
  var objNodeTaxa = objXmlTaxa.selectNodes("//TaxaServico");
  if (objNodeTaxa.length > 0) {
    for (var intIndex = 0; intIndex < objNodeTaxa.length; intIndex++) {
      intAcfId = objNodeTaxa[intIndex].childNodes[0].text;
      objNodeList = objXML.selectNodes(
        "//xDados/TaxaServico[Acf_Id=" + parseInt(intAcfId) + "]"
      );
      if (objNodeList.length == 0) {
        //Cria o Node Chave
        objNode = objXML.createNode("element", "TaxaServico", "");
        objXML.documentElement.appendChild(objNode);
      } else {
        objNode = objXML.selectSingleNode(
          "//xDados/TaxaServico[Acf_Id=" + parseInt(intAcfId) + "]"
        );
      }

      for (
        var intIndexII = 0;
        intIndexII < objNodeTaxa[intIndex].childNodes.length;
        intIndexII++
      ) {
        var strNomeNode = objNodeTaxa[intIndex].childNodes[intIndexII].nodeName;
        var strValorNode = objNodeTaxa[intIndex].childNodes[intIndexII].text;

        var objNodeListAtual = objNode.getElementsByTagName(strNomeNode);
        if (objNodeListAtual.length == 0) {
          //Cria
          if (strValorNode != "") {
            objNodeFilho = objXML.createNode("element", strNomeNode, "");
            objNodeFilho.text = strValorNode;
            objNode.appendChild(objNodeFilho);
          }
        } else {
          //Atualiza
          if (strValorNode != "") {
            objNodeListAtual.item(0).text = strValorNode;
          } else {
            //Se esta vazio remove do XML
            objNodeListAtual
              .item(0)
              .parentNode.removeChild(objNodeListAtual.item(0));
          }
        }
      }
    }
  }
}

//Verifica se o acesso foi alterado e n�o foi recolocado a lista
function ValidarPontoInstalacao() {
  var objAry = new Array(
    "cboUFEnd",
    "txtEndCid",
    "txtEndCidDesc",
    "cboLogrEnd",
    "txtEnd",
    "txtNroEnd",
    "txtBairroEnd",
    "txtCepEnd"
  );
  var objNode = objXmlGeral.selectNodes("//xDados/Acesso");

  for (var intIndexNode = 0; intIndexNode < objNode.length; intIndexNode++) {
    var blnAchou = true;
    intChave = objNode[intIndexNode].childNodes[0].text;
    for (var intIndex = 0; intIndex < objAry.length; intIndex++) {
      var objChildForm = new Object(eval("document.Form2." + objAry[intIndex]));
      //alert(eval("document.Form2."+objAry[intIndex]+".name") + " : " + objChildForm.value + ' = ' + RequestNodeAcesso(objXmlGeral,objAry[intIndex],intChave))
      if (
        objChildForm.value !=
        RequestNodeAcesso(objXmlGeral, objAry[intIndex], intChave)
      ) {
        blnAchou = false;
      }
    }
    if (blnAchou) return true;
  }
  return false;
}
