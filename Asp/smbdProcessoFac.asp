...
Function AlocaFacilidade(dblRecId)
	...
	Response.Write "aaaa"
	Response.Write "dblRecId = " & dblRecId
	Response.Write "dblUsuId = " & dblUsuId
	...
	if objNode.length > 0 then
		...
		if objNode.length = 0 then
			Response.Write "<script language=javascript>parent.document.forms[0].cboRede.disabled=false;</script>"
		Else	
			Response.Write "<script language=javascript>parent.document.forms[0].cboRede.disabled=true;</script>"
		End if
	End if
...
Select Case Trim(Request.Form("hdnAcao"))
	...
	Case "GravarFacilidade"
		...
		strSqlRet = APENDA_PARAMSTR("CLA_sp_check_recurso2",5,Vetor_Campos)
