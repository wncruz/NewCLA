...
If Trim(Request.Form("hdnxml")) <> "" Then
	...
	'Adicionado PRSS 16/04/2007
	Acao = objXmlDadosForm.selectSingleNode("//requisicao-cla/acao").text
	
	'Checa se serviço é 0800.
	Oe_numero = objXmlDadosForm.selectSingleNode("//requisicao-cla/servico/order-entry/numero").text
	Oe_ano = objXmlDadosForm.selectSingleNode("//requisicao-cla/servico/order-entry/ano").text
	Oe_item = objXmlDadosForm.selectSingleNode("//requisicao-cla/servico/order-entry/item").text
	...
	call check_servico(Oe_numero,Oe_ano,Oe_item,Id_logico,processo,acao,1,strxml)
	...
	If 	Len(Trim(objXmlDadosForm.parseError.errorCode)) > 1 Then
		...
		call check_servico(Oe_numero,Oe_ano,Oe_item,Id_logico,processo,acao,1,strxmlResp)
	...
	'Requisicao CLA
	 ...
	 Acao = objXmlDadosForm.selectSingleNode("//requisicao-cla/acao").text
	 processo = objXmlDadosForm.selectSingleNode("//requisicao-cla/processo").text
	 ID_Tarefa_APG = objXmlDadosForm.selectSingleNode("//requisicao-cla/id-tarefa-apg").text
	 Id_Logico =  objXmlDadosForm.selectSingleNode("//requisicao-cla/id-logico").text
	 'on error resume next
	 if trim(Acao) = "ATV" then
	 	Rollback			        = trim(objXmlDadosForm.selectSingleNode("//requisicao-cla/rollback").text)
	 end if 
	 
	 If Trim(Acao) = "CAN" Then
		...
		Telefone_Cadastrador 	                = trim(objXmlDadosForm.selectSingleNode("//requisicao-cla/servico/order-entry/cadastrador/telefone").text)
	 	...
	 	if (  trim(Rollback) = "N" and trim(Id_Logico) = "" ) then
			ID_Tarefa_APG_ATV		        = trim(objXmlDadosForm.selectSingleNode("//requisicao-cla/id-tarefa-apg-ant").text)
		end if
		
		if trim(Rollback) = "S" then
			Observacao				= trim(objXmlDadosForm.selectSingleNode("//requisicao-cla/observacao").text)
		end if 
	 end if 
	 ...
	 If Trim(Acao) = "DES" Then

		Dt_Desativacao 	= objXmlDadosForm.selectSingleNode("//requisicao-cla/desativacao/data").text
		Reaproveitar_Fisico =  objXmlDadosForm.selectSingleNode("//requisicao-cla/desativacao/reaproveitar").text		

	 End If

	 ' Adicionado 09/05/2007 por EDUARDO ARAUJO
	 ' Serviço/Order Entry
	...
	OE_Item = trim(objXmlDadosForm.selectSingleNode("/requisicao-cla/servico/order-entry/item").text)
	 ...
	 'If Trim(Acao) = "ATV" or Trim(Acao) = "ALT" Then
	 If (Trim(Acao) = "ATV" and Trim(Processo) <> "DES") or (Trim(Acao) = "ALT" and Trim(Processo) <> "DES") Then 'PRSS 14/11/2007
		...
		Rota					= trim(objXmlDadosForm.selectSingleNode("/requisicao-cla/servico/order-entry/endereco-instalacao/compartilhaRota").text)							
		Distribuicao			= trim(objXmlDadosForm.selectSingleNode("/requisicao-cla/servico/order-entry/endereco-instalacao/compartilhaDistribuicao").text)							

		'Alterado PRSS 05/11/2007
		if trim(Rollback) = "S" then
			Observacao				= trim(objXmlDadosForm.selectSingleNode("/requisicao-cla/observacao").text)
		end if

		if Trim(Rota) = "S" then
			set	 objNodeList = objXmlDadosForm.getElementsByTagName("grupoCompartRota")
		end if
		
		if Trim(Distribuicao) = "S" then 
			set	 objNodeList = objXmlDadosForm.getElementsByTagName("grupoCompartDistribuicao")
		end if 
		
		
		if Trim(Rota) = "S" or Trim(Distribuicao) = "S" then
		
			Tronco_Interface = ""
			Tronco_Inicio = ""
			Tronco_Fim = ""
			Servico_Compartilhado = ""
	
			For i = 0 To objNodeList.Length - 1
	
				Set subLst = objNodeList.item(i)
	
			
				Tronco_Interface =  Tronco_Interface & subLst.childNodes(1).text & ","
				Tronco_Inicio = Tronco_Inicio & subLst.childNodes(3).text & ","
				Tronco_Fim = Tronco_Fim & subLst.childNodes(5).text & ","
				Servico_Compartilhado = Servico_Compartilhado & subLst.childNodes(7).text & ","
			
			next
			
	
			Tronco_Interface = mid(Tronco_Interface,1,Len(Tronco_Interface)-1)   	
			Tronco_Inicio = mid(Tronco_Inicio,1,Len(Tronco_Inicio)-1)
			Tronco_Fim = mid(Tronco_Fim,1,Len(Tronco_Fim)-1)
			Servico_Compartilhado = mid(Servico_Compartilhado,1,Len(Servico_Compartilhado)-1)
	...
	Vetor_Campos(3)="adinteger,5,adParamInput,"& ID_Tarefa_APG
	...
	Vetor_Campos(16)="adWChar,30,adParamInput,"& Contato_Tecnico_Cliente
	...
	Vetor_Campos(34)="adWChar,30,adParamInput,"& Complemento
	Vetor_Campos(35)="adWChar,60,adParamInput,"& Nome_Logradouro
	...
	Vetor_Campos(44)="adWChar,1,adParamInput,"& Rota
	Vetor_Campos(45)="adWChar,1,adParamInput,"& Distribuicao
	Vetor_Campos(46)="adWChar,800,adParamInput,"& Tronco_Interface
	Vetor_Campos(47)="adWChar,800,adParamInput,"& Tronco_Inicio
	Vetor_Campos(48)="adWChar,800,adParamInput,"& Tronco_Fim
	Vetor_Campos(49)="adWChar,2000,adParamInput,"& Servico_compartilhado	
	Vetor_Campos(50)="adWChar,800,adParamInput,"& Observacao

    Vetor_Campos(51)="adinteger,5,adParamInput,"& ID_Tarefa_APG_ATV
	Vetor_Campos(52)="adWChar,1,adParamInput,"& Rollback

	Vetor_Campos(53)="adinteger,12,adParamInput,"& Codigo_Sap
	Vetor_Campos(54)="adWChar,12,adParamInput,"& Estacao_Cliente

	Vetor_Campos(55)="adInteger,2,adParamOutput,0"
	Vetor_Campos(56)="adWChar,100,adParamOutput,a "

	'strLoginRede = ucase(mid(Request.ServerVariables("Logon_User"), Instr(Request.ServerVariables("Logon_User"),"\")+1))
	
	Call APENDA_PARAM("CLA_sp_ins_Solicita_Acesso",56,Vetor_Campos)
    ObjCmd.Execute'pega dbaction
	...
	If trim(DBAction) <> "0" and trim(DBAction) <> "" Then
		...
		strxmlResp = "Erro ao gravar na Interface APG => CLA - Ação: Solicitar_Acesso" & Trim(DBAction) & Trim(DBDescricao)
		
		call check_servico(Oe_numero,Oe_ano,Oe_item,Id_logico,processo,acao,1,strxmlResp)

		Vetor_Campos(1)="adInteger,6,adParamInput, 0"	 							'Solicitação
		Vetor_Campos(2)="adInteger,6,adParamInput, " & ID_Tarefa_APG				'Identificação do APG
		Vetor_Campos(3)="addouble,10,adParamInput, 0" 								'ID Logico
		Vetor_Campos(4)="adWChar,255,adParamInput," & Trim(strxmlResp)				'Descrição do Erro
		Vetor_Campos(5)="adInteger,4,adParamOutput,0"
