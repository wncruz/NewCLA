...
function ConsultarPedidosPend()
{
	with (document.forms[0])
	{
		...
		hdnAcao.value = "SEL"
		target = self.name
		action = "DesalocacaoNew_main.asp?Consulta=1"
...
<form action="DesalocacaoNew_main.asp" name="Form1" method="post" >
...
<tr >
	<th colspan=2 ><p align=center>Desalocação</p></th>
</tr>
...
function ContinuaDesalocacao(dblSolId,dblPedId)
{
	with (document.forms[0])
	{
		PopularXml()
		...
		action = "DesalocacaoNew.asp"
		submit()
	}
}
//ContinuaAlocacao2
function ContinuaDesalocacao2(dblSolId)
{
	//alert(dblSolId)
	with (document.forms[0])
	{
...
strPedido = Request.Form("txtPedido")
	...
	strSql = APENDA_PARAMSTR("CLA_sp_view_DesalocacaoAceite",2,Vetor_Campos)
...
''LP
	...
	'	if intPedId <> objRSPag("Ped_Id") then 'Usado para controle de exibição de pedidos duplicados (Devido a qtde. de acso. fisicos)
			...
			'response.write "<script>alert('"&objRSPag("Sts_Desc") &"')</script>"
			'response.write "<script>alert('"&objRSPag("Sol_id") &"')</script>"
			'response.write "<script>alert('"&objRSPag("Cli_Nome") &"')</script>"
			'response.write "<script>alert('"&AcaoPedido(objRSPag("Tprc_ID")) &"')</script>"
			'response.write "<script>alert('"&objRSPag("Acl_NContratoServico") &"')</script>"
			'response.write "<script>alert('"&objRSPag("Pro_Nome") &"')</script>"
			'response.write "<script>alert('"&strStatusDet &"')</script>"
					...
					strHtml = strHtml &   "<tr class='" & strClass & "'>" & _
									...
									"<td ><a href='javascript:ContinuaDesalocacao2(" & objRSPag("Sol_id") & ")'>" & objRSPag("Sol_id") & "</a></td>" & _
