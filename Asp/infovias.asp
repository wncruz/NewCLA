...
strParmProc = dblProId + "|" + dblPedId  + "|" + intTipoProcesso + "|" + strUserName 
	...
	if not isnull(objRSped1("esc_nomelogr")) then 
		...
		strXML = strXML &	"<chkendereço></chkendereço>"
			...
			select case trim(objRSped1("Tct_Meses"))
				case "12"
						blnind =  "12 Meses" 
				case "24" 
						blnind = "24 Meses" 
				...
				case "60"
						blnind = "60 Meses" 
				case else 
						blnind = "de " &  objRSped1("Acl_DtIniAcessoTemp")  & " até " & objRSped1("Acl_DtFimAcessoTemp")
			end select 
...
'		strXML = strXML &	"<chkind>" & blnIndT &  "</chkind>"
		...
		'strXML = strXML &	"<chkind>" & objRSped1("Tct_Meses") &  "</chkind>"
		strXML = strXML &	"<chkind>" & blnind &  "</chkind>"
		...
		strXML = strXML &	"<pontaAnome>EMPRESA BRASILEIRA DE TELECOMUNICAÇÕES - EMBRATEL - S/A  </pontaAnome>"
		
		strXML = strXML &	"<quantidade>1</quantidade>"
		
		strXML = strXML &	"<pontaAend>" & trim(objRSped1("TPL_SIGLA")) & " " & trim(objRSped1("esc_nomelogr")) & " " & objRSped1("esc_nrologr") & " " & objRSped1("esc_complemento") & " " & objRSped1("esc_bairro") & "</pontaAend>"
		strXML = strXML &	"<pontaAcontato>" & objRSped1("esc_contato")  & "</pontaAcontato>"
		strXML = strXML &	"<pontaAcid>" & objRSped1("cid_desc") &  "</pontaAcid>"
		strXML = strXML &	"<pontaAtelefone>" & objRSped1("esc_telefone") &  "</pontaAtelefone>"
		
		strXML = strXML &	"<pontaAemail>" & objRSped1("esc_email") &  "</pontaAemail>"
		strXML = strXML &	"<pontaAcnpj>" & TRIM(objRSped1("esc_CNPJ"))  & "</pontaAcnpj>"
		
		strXML = strXML &	"<velacessofisico>" & objRSped1("DescVelAcessoFis") &  "</velacessofisico>"
		strXML = strXML &	"<dataprevista>" & objRSped1("Ped_DtPrevistaAtendProv") &  "</dataprevista>"
		...
		strXML = strXML &	"<pontabnome>" &trim(objRSPontoB("cli_nome")) & "</pontabnome>"
		strXML = strXML &	"<pontabcid>" &trim(objRSPontoB("cid_desc")) & "</pontabcid>"
		strXML = strXML &	"<pontaBend>" &   trim(objRSPontoB("TPL_SIGLA")) & " " & trim(objRSPontoB("End_NomeLogr")) & " " & trim(objRSPontoB("End_NroLogr")) & " " & trim(objRSPontoB("Aec_Complemento")) & " " & trim(objRSPontoB("End_Bairro")) &  "</pontaBend>"
		strXML = strXML &	"<pontaBcontato>" & objRSPontoB("Aec_Contato") & "</pontaBcontato>"
		strXML = strXML &	"<pontaBtelefone>( " & mid(objRSPontoB("Aec_Telefone"),1,2) & " ) " & mid(objRSPontoB("Aec_Telefone"),3,len(objRSPontoB("Aec_Telefone")) - 2) & "</pontaBtelefone>"		
		
		strXML = strXML &	"<pontabcnpj>" &trim(objRSPontoB("aec_cnpj")) & "</pontabcnpj>"
		strXML = strXML &	"<pontabcontato>" &trim(objRSPontoB("aec_contato")) & "</pontabcontato>"
		'strXML = strXML &	"<pontabtelefone>" &trim(objRSPontoB("aec_telefone")) & "</pontabtelefone>"
		'strXML = strXML &	"<pontabemail>" &trim(objRSPontoB("aec_telefone")) & "</pontabemail>"
		
				
		'strXML = strXML &	"<previsaoentrega>" & trim(objRSPontoB("PED_DTPREVISAOENTREGA")) & "</previsaoentrega>"
		'strXML = strXML &	"<pontaBnome>" & trim(objRSped1("Cli_nome")) & "</pontaBnome>"
...
'' LPEREZ 13/12/2005		
		strXML = strXML &	"<observaçãop>" & trim(objRSped1("pED_obs")) & "</observaçãop>"
		strXML = strXML &	"<observaçãos>" & trim(objRSped1("SOL_obs")) & "</observaçãos>"
'' LP		
		...
		strXML = strXML &	"<emailgla>" & strEmailRepresent &  "</emailgla>"
		strXML = strXML &	"<telefonegla>" & strTelefone &  "</telefonegla>"
		
		strXML = strXML &	"<contatoebt>" & objRSPro("Cpro_ContratanteContato") & "</contatoebt>"
		strXML = strXML &	"<telefoneebt>" & objRSPro("Cpro_Contratantetelefone") & "</telefoneebt>"
