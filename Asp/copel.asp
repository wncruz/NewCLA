...
strParmProc = dblProId + "|" + dblPedId  + "|" + intTipoProcesso + "|" + strUserName 
	...
	if not isnull(objRSped1("esc_nomelogr")) then 
		...
		strXML = strXML &	"<arquivo>copel</arquivo>"
		...
		strXML = strXML &	"<servico_descricao>" & objRSped1("Ser_Desc") &  "</servico_descricao>"	
		...
		strXML = strXML &	"<cgc_cnpj_contratante>"& objRSPro("cPro_ContratanteCGC_CNPJ")  &"</cgc_cnpj_contratante>"
		...
		strXML = strXML &	"<clienterua>" & trim(objRSPontoB("TPL_SIGLA")) & " " & trim(objRSPontoB("End_NomeLogr")) &  "</clienterua>"
		strXML = strXML &	"<clientenumero>" & trim(objRSPontoB("End_NroLogr")) & "</clientenumero>"
		strXML = strXML &	"<clientecompl>" & trim(objRSPontoB("Aec_Complemento")) &  "</clientecompl>" 
		strXML = strXML &	"<clientebairro>" &  trim(objRSPontoB("End_Bairro")) & "</clientebairro>"
		
		strXML = strXML &	"<clientecidade>" & Trim(objRSPontoB("CID_DESC"))   &  "</clientecidade>"
		strXML = strXML &	"<clienteuf>"  & Trim(objRSPontoB("est_sigla")) & "</clienteuf>"
		strXML = strXML &	"<clientecep>" & Trim(objRSPontoB("end_Cep")) &  "</clientecep>"
		strXML = strXML &	"<clienteccta>" & Trim(objRSPontoB("Cli_CC")) & "</clienteccta>"
		strXML = strXML &	"<clientecnpj>" & Trim(objRSPontoB("Aec_CNPJ")) & "</clientecnpj>"
		strXML = strXML &	"<clienteiem>" & Trim(objRSPontoB("Aec_IM")) & "</clienteiem>"
		strXML = strXML &	"<clienteiee>" & Trim(objRSPontoB("Aec_IE")) & "</clienteiee>"
		strXML = strXML &	"<clientecontato>" & Trim(objRSPontoB("Aec_Contato")) & "</clientecontato>"
		...
		strXML = strXML &	"<pontabend_rua>" & trim(objRSped1("TPL_SIGLA")) & " " & trim(objRSped1("esc_nomelogr")) & "</pontabend_rua>"
		strXML = strXML &	"<pontabend_numero>" & trim(objRSped1("esc_nrologr")) & "</pontabend_numero>"
		strXML = strXML &	"<pontabend_complemento>" & trim(objRSped1("esc_complemento")) & "</pontabend_complemento>"
		strXML = strXML &	"<pontabend_bairro>" & trim(objRSped1("esc_bairro")) & "</pontabend_bairro>"
		
		
		strXML = strXML &	"<pontabcidade>" & trim(objRSped1("cid_desc")) & "</pontabcidade>"
		strXML = strXML &	"<pontabuf>" & Trim(objRSped1("Est_sigla")) & "</pontabuf>"
		strXML = strXML &	"<pontabcep>" & Trim(objRSped1("esc_cod_cep"))  & "</pontabcep>"
		strXML = strXML &	"<pontabcnpj>" & Trim(objRSped1("esc_CNPJ"))  & "</pontabcnpj>"
		strXML = strXML &	"<pontabie></pontabie>"
		strXML = strXML &	"<pontabtel>" & Trim(objRSped1("esc_telefone")) &  "</pontabtel>"
		strXML = strXML &	"<pontabcontato>" & Trim(objRSped1("esc_contato"))  & "</pontabcontato>"
		strXML = strXML &	"<pontabinterface>" & Trim(objRSPontoB("Acf_InterfaceEstEntregaFisico"))  & "</pontabinterface>"
...
'' LP		
		...
		dim strNomeGICL , strEmailGICL , strTelefoneGICL
		...
		do while not objRepresGLA.eof
			...
			if UCASE(objRepresGLA("AGE_DESC")) = "GICL" then
				  strNomeGICL   = objRepresGLA("USU_NOME") 
				  strEmailGICL	 = objRepresGLA("USU_EMAIL") 
				  strTelefoneGICL = objRepresGLA("USU_RAMAL")
			end if 
		...
		strXML = strXML &	"<nomeGICL>" & strNomeGICL &  "</nomeGICL>"
		strXML = strXML &	"<emailGICL>" & strEmailGICL &  "</emailGICL>"
		strXML = strXML &	"<TelefoneGICL>" & strTelefoneGICL &  "</TelefoneGICL>"
