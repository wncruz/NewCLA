...
if not objRSPag.Eof and not objRSPag.Bof then
	For intIndex = 1 to objRSPag.PageSize
		...
		<tr class=<%=strClass%>>
			...
			-->
			<td style="WORD-BREAK: break-all">&nbsp;<%=TratarAspasHtml(objRSPag("CPro_ContratadaEmail"))%></td>
			<td style="WORD-BREAK: break-all">&nbsp;<%=TratarAspasHtml(objRSPag("CPro_ContratadaContato"))%></td>
			<!-- CH-28482YPU - Fim -->
...
<%end if%>			
		<input type="button" class="button" name="btnLimpar" value=" Limpar " onClick="javascript:window.location.replace('provedorcontrato_main.asp')" accesskey="X" onmouseover="showtip(this,event,'Sair (Alt+X)');">
