...
Dim gravarDireto
	...
	If len(request("ID")) > 3 then
		nup = request("Id")
		if Trim(nup) <> "" then
			Set rs = db.execute("cla_sp_sel_snoa '"& mid(nup, 1, 2) &"',"& mid(nup, 4, 5) &","& mid(nup, 10, 4) & ",null,null,null,null")
	...
	Else
	 	...
	 	If request("numero") <> "" then
			...
			Set rs = db.execute("cla_sp_sel_snoa null,null,null," & strNroAcesso &",null,null,null")
			if Not rs.Eof and Not rs.bof then
				dblSolId = rs("Sol_Id")
				strAcaoPedido = rs("Tprc_Des")
				strDM = ucase(rs("Ped_Prefixo")) & "-" & right("00000" & rs("Ped_Numero"),5) & "/" & rs("Ped_Ano")
		...
		Else
 			...
 			If request("idFisico") <> "" then
				...
				Set rs = db.execute("cla_sp_sel_snoa null,null,null,null," & idFisico &",null,null")
				if Not rs.Eof and Not rs.bof then
					dblSolId = rs("Sol_Id")
					strAcaoPedido = rs("Tprc_Des")
					strDM = ucase(rs("Ped_Prefixo")) & "-" & right("00000" & rs("Ped_Numero"),5) & "/" & rs("Ped_Ano")
			...
			Else
 				...
 				If request("idSolicitacao") <> "" then
					...
					Set rs = db.execute("cla_sp_sel_snoa null,null,null,null,null," & idSolicitacao & ",null")
					if Not rs.Eof and Not rs.bof then
						dblSolId = rs("Sol_Id")
						strAcaoPedido = rs("Tprc_Des")
						dblAcfId = rs("Acf_id")
				...
				Else
	 				...
	 				If request("numeroSNOA") <> "" then
						...
						Set rs = db.execute("cla_sp_sel_snoa null,null,null,null,null,null, " & numeroSNOA )
						if Not rs.Eof and Not rs.bof then
							dblSolId = rs("Sol_Id")
							strAcaoPedido = rs("Tprc_Des")
							dblAcfId = rs("Acf_id")
...
<input type=hidden name=hdnPaginaOrig 	value="historicoPedidoSNOA_main.asp">

<table border=0 cellspacing="1" cellpadding="0" width="760"> 
	<th style="FONT-SIZE: 14px">&nbsp;•&nbsp;Dados da Solicitação</th>
	<!--
	<th align=rigth><a href="javascript:DetalharItem()"><font color=white>Mais...</font></a></th>
	-->
	<th align=rigth>
		<a href="javascript:DetalharItem()"> 
		<font color=white style="FONT-SIZE: 14px">&nbsp;Mais...</font></a>
	</th>
