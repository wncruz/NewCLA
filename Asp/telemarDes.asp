...
'set objRSPontoB = db.execute("CLA_sp_view_ponto null, " & dblPedId & ",null," & objRSped1("Sol_ID")) --Correção de erro uat plus
set objRSPontoB = db.execute("CLA_sp_view_ponto null, " & dblPedId & ",null," & dblSolId)
...
DIM   strXML
	...
	strXML = strXML & "<arquivo>TelemarDes</arquivo>"
...
''	strXML = strXML & "<icms></icms>" 
	...
	strXML = strXML & "<endereço>" & trim(objRSPontoB("TPL_SIGLA")) & " " & trim(objRSPontoB("End_NomeLogr")) & " Nº" & trim(objRSPontoB("End_NroLogr")) & " " & trim(objRSPontoB("Aec_Complemento")) & " " & trim(objRSPontoB("End_Bairro")) & "</endereço>"  
...
'' LPEREZ 13/12/2005
	...
	if cLng(objRSped1("Vel_Conversao")) >= 2048 then'SUPERLINK
	  strXML = strXML & "<frasedeclaracao>" & Ucase("DECLARO PARA TODOS OS FINS QUE ESTOU DE ACORDO COM TODAS AS CLAUSULAS DO CONTRATO DE ADESÃO DO SERVIÇO SUPERLINK") &  "</frasedeclaracao>"
	  strXML = strXML & "<fraseobs>DECLARO PARA TODOS OS FINS QUE ESTOU DE ACORDO COM TODAS AS CLAUSULAS DO CONTRATO DE ADESÃO DO SERVIÇO SUPERLINK E SEUS TERMOS RETIFICADORES (1º E 2º)</fraseobs>"
	else''ELID PADRAO
	  strXML = strXML & "<frasedeclaracao>" & Ucase("DECLARO PARA TODOS OS FINS QUE ESTOU DE ACORDO COM TODAS AS CLAUSULAS DO CONTRATO DE ADESÃO DO SERVIÇO EILD PADRÃO") & "</frasedeclaracao>"
	  strXML = strXML & "<fraseobs>DECLARO PARA TODOS OS FINS QUE ESTOU DE ACORDO COM TODAS AS CLAUSULAS DO CONTRATO DE ADESÃO DO SERVIÇO EILD PADRÃO E SEU ACORDO DE CONDIÇÕES ESPECÍFICAS ASSINADOS EM 31 DE MARÇO DE 2009</fraseobs>"
	end if
