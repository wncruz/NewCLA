...
'	- Sistema			: CLA
'	- Arquivo			: Tecnologia.asp
'	- Responsável		: Vital
'	- Descrição			: Cadastra/Altera Tecnologia
%>
...
if Trim(Request.Form("hdnAcao")) = "Gravar" then
	...
	Vetor_Campos(3)="adWChar,5,adParamInput,"&	Trim(Request.Form("txtSigla"))
	Vetor_Campos(4)="adWChar,10,adParamInput," & strloginrede 
	Vetor_Campos(5)="adInteger,2,adParamOutput,0"  

	Call APENDA_PARAM("CLA_sp_ins_newTecnologia",5,Vetor_Campos)
...
if dblId<> "" then
	...
	Set objRSSer = db.execute("CLA_sp_sel_newtecnologia " & dblId)

	'response.write "<script>alert('"&objRSSer("newtec_sigla")&"')</script>"

	if Not objRSSer.Eof and not objRSSer.Bof then
		strDesc				= TratarAspasHtml(objRSSer("newtec_nome"))
		strCodigo			= TratarAspasHtml(objRSSer("newtec_id"))
		strSigla			= TratarAspasHtml(objRSSer("newtec_sigla"))
...
<form action="tecnologia.asp" method="post" >
...
<tr>
	<th colspan=2><p align="center">Cadastro de Tecnologia</p></th>
</tr>
<!--
<tr class=clsSilver>
...
<tr class=clsSilver>
	...
	<td>
		<input type="text" class="text" name="txtSigla" value="<%=strSigla%>" maxlength="5" size="5">
	</td>
</tr>
...
<td colspan=2 align="center"><br>
	...
	<input type="button" class="button" name="btnGravar" value="Gravar" onclick="GravarTecnologia()" accesskey="I" onmouseover="showtip(this,event,'Incluir (Alt+I)');">&nbsp;
	...
	<input type="button" class="button" name="Voltar" value="Voltar" onClick="javascript:window.location.replace('tecnologia_main.asp')" accesskey="B" onmouseover="showtip(this,event,'Voltar (Alt+B)');">
