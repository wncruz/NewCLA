...
set objRSFacilidade = db.execute("CLA_sp_view_cartaprovedor " & dblPedId )
...
'Response.Write "CLA_sp_view_ponto null, " & dblPedId & ",null, "& objRSped1("Sol_ID")
'Response.End
...
strParmProc = dblProId + "|" + dblPedId  + "|" + intTipoProcesso + "|" + strUserName 
	...
	if not isnull(objRSped1("esc_nomelogr")) then 
		...
		strXML = strXML &	"<arquivo>brtelecom</arquivo>"
		...
		strXML = strXML &	"<data>" & formatdatetime(date(),2) &  "</data>"
		strXML = strXML &	"<pontaebt>" & objRSped1("Acf_NroAcessoPtaEbt") &  "</pontaebt>"
		strXML = strXML &	"<acao>" & strAcao &  "</acao>"
		strXML = strXML &	"<codacao>" & DblNdTipo &  "</codacao>"

		strXML = strXML &	"<qtde_equipamento>" & objRSped1("Acf_QtdEquip") &  "</qtde_equipamento>"
		strXML = strXML &	"<propriedade_equipamento>" & objRSped1("Acf_ProprietarioEquip") &  "</propriedade_equipamento>"
		
		
		
		If objRSped1("Acf_ProprietarioEquip") = "TER"  Then
			strXML = strXML &	"<existemodem>" & "SIM" &  "</existemodem>"
		Else
			If objRSped1("Acf_ProprietarioEquip") = "EBT" or objRSped1("Acf_ProprietarioEquip") = "CLI" Then
				strXML = strXML &	"<existemodem>" & "NAO" &  "</existemodem>"
			Else
				strXML = strXML &	"<existemodem>" & " " &  "</existemodem>"
			End IF
		End IF



		strXML = strXML &	"<tipocircuito>" & objRSped1("Acf_CCTOTipo") &  "</tipocircuito>"

		strXML = strXML &	"<numeroagregado>" & objRSPro("CPro_Numero_Agregado") &  " </numeroagregado>"
		'strXML = strXML &	"<numeroagregado> " &    " </numeroagregado>"
		
		
		
		If Trim(objRSPontoB("Sol_TipoAlteracao")) = "Velocidade"  Then
		
			strXML = strXML &	"<tipoAlteracao>" & "V" &  "</tipoAlteracao>"
			
		Else
		
			If Trim(objRSPontoB("Sol_TipoAlteracao")) = "Endereco"  Then
				strXML = strXML &	"<tipoAlteracao>" & "E" &  "</tipoAlteracao>"
			ElseIf Trim(objRSPontoB("Sol_TipoAlteracao")) = "Endereco+Velocidade"  Then
					strXML = strXML &	"<tipoAlteracao>" & "E+V" &  "</tipoAlteracao>"	
				
			ElseIf Trim(objRSPontoB("Sol_TipoAlteracao")) = "Velocidade+Facilidade"  Then  ' INCUIDO POR PSOUTO 24/04/2006  - ACERTO DE BO
					strXML = strXML &	"<tipoAlteracao>" & "E+V" &  "</tipoAlteracao>"	
		...
		If Trim(objRSPontoB("Sol_programacaoAlteracao")) = "Programada"  Then
			strXML = strXML &	"<ProgAlteracao>" & "P" &  "</ProgAlteracao>"
		Else
			If Trim(objRSPontoB("Sol_programacaoAlteracao")) = "Nao Programada"  Then
				strXML = strXML &	"<ProgAlteracao>" & "NP" &  "</ProgAlteracao>"
			End if 
		End IF
		...
		If trim(objRSped1("acf_portadora")) = "Constante" then
			strConstante = "x"
		else
			If trim(objRSped1("acf_portadora")) = "PseudoControlada" then
				strPSeudo = "x"
			End If
		End If
		
		strXML = strXML &	"<constante>" & strConstante &  " </constante>"
		strXML = strXML &	"<pseudo>" & strPSeudo &  " </pseudo>"



		strXML = strXML &	"<facilidadeEbt> " 
		
		if trim(objRSFacilidade("Sis_id")) > 0 then
		
		while not objRSFacilidade.eof  and   not objRSFacilidade.bof 
			
			If objRSFacilidade("Sis_id") = 1 then ' Deterministico
			
				StrFacilidade = objRSFacilidade("fac_desigTronco") & objRSFacilidade("FAC_Link") & objRSFacilidade("FAC_Dominio") & objRSFacilidade("FAC_No") & objRSFacilidade("FAC_Slot") & objRSFacilidade("FAC_Porta")
				StrFacilidadeProx = objRSFacilidade("fac_desigTronco") & objRSFacilidade("FAC_Link") & objRSFacilidade("FAC_Dominio") & objRSFacilidade("FAC_No") & objRSFacilidade("FAC_Slot") & objRSFacilidade("FAC_Porta")
				

				StrInfoProvedor = StrInfoProvedor &	" BRT: Designação: " & Trim(objRSFacilidade("fac_desigTronco")) 	
				StrInfoProvedor = StrInfoProvedor &	" Identificação: " & Trim(objRSFacilidade("Fac_IdentProvedor")) 

				strXML = strXML &	" EBT: Link: " & Trim(objRSFacilidade("FAC_Link"))
				strXML = strXML &	" Dominio: " & Trim(objRSFacilidade("FAC_Dominio"))
				strXML = strXML &	" No: " & Trim(objRSFacilidade("FAC_No"))
				strXML = strXML &	" Slot: " & Trim(objRSFacilidade("FAC_Slot"))
				strXML = strXML &	" Porta: " & Trim(objRSFacilidade("FAC_Porta"))
				strXML = strXML &   " TimeSlot: " & Trim(objRSFacilidade("FAC_TimeSlot")) & " - "
				strTimeslotini = Trim(objRSFacilidade("FAC_TimeSlot"))

			Else

				If objRSFacilidade("Sis_id") = 2  then 'Não Deterministico
				
					StrFacilidade = objRSFacilidade("FAC_Tronco") 
					StrFacilidadeProx = objRSFacilidade("FAC_Tronco") 
			
					strXML = strXML &	" Tronco: " & objRSFacilidade("FAC_Tronco") 
					strXML = strXML &	" Par: " 
					''strXML = strXML &	" Par Inicial: " & objRSFacilidade("Fac_Representacao")  & " - "
			...
			While not objRSFacilidade.eof and StrFacilidade = StrFacilidadeProx
			
				
				If objRSFacilidade("Sis_id") = 1 then ' Deterministico
					strfinal =  objRSFacilidade("FAC_TimeSlot")  
					strTSlot =  objRSFacilidade("FAC_TimeSlot")  
			
					objRSFacilidade.Movenext
			
					If not objRSFacilidade.eof Then
				...
				Else
					...
					If objRSFacilidade("Sis_id") = 2 then 'Não Deterministico
						...
						strfinal = strfinal & " - " & objRSFacilidade("Fac_Par")  
		...
		strXML = strXML &	"<facilidadeProv> " 
		
		'if trim(objRSFacilidade("Sis_id")) > 0 then
		
			strXML = strXML & StrInfoProvedor  & " TimeSlot: " & strTimeslotini & " - " & strTSlot
...
'***		
		...
		strXML = strXML &	"<cidadecontratante>" &   objRSPro("ContratanteCidSigla") & "</cidadecontratante>"
		...
		strXML = strXML &	"<chkendereco></chkendereco>"
		...
		strXML = strXML &	"<clientecidade>" & objRSPontoB("CID_SIGLA")   &  "</clientecidade>"
		...
		strXML = strXML &	"<AcessoLogico>" & trim(objRSPontoB("Acl_IDAcessoLogico")) & " </AcessoLogico>"
		''--Acl_IDAcessoLogico
		
		
		'@@ PontaB
		strXML = strXML &	"<pontabnome>EMPRESA BRASILEIRA DE TELECOMUNICAÇÔES S/A - EMBRATEL</pontabnome>"
		strXML = strXML &	"<cnpjcontratante>"& objRSPro("CPro_ContratanteCGC_CNPJ")  &"</cnpjcontratante>"
		...
		strXML = strXML &	"<pontabcidade>" & objRSped1("CID_SIGLA") & "</pontabcidade>"
		...
		strXML = strXML &	"<localdataembratel>" & objRSPro("ContratanteCid")   &", "& strData_Ext & " </localdataembratel>"
		strXML = strXML &	"<localdatacontratada> " & objRSPro("ContratadaCid") &", "& strData_Ext & " </localdatacontratada>"		
		strXML = strXML &	"<observacao>" & objRSped1("pED_obs")  &  "</observacao>"
'' LPEREZ 13/12/2005		
		strXML = strXML &	"<observacaop>" & objRSped1("pED_obs") & "</observacaop>"
		strXML = strXML &	"<observacaos>" & objRSped1("SOL_obs") & "</observacaos>"
'' LP		
		
		dim objRepresGLA 
		set objRepresGLA = db.execute("CLA_sp_view_agentesolicitacao " & dblSolId )
		dim strNomeRepresent , strEmailRepresent , strTelefone
		dim strNomeGIC, strEmailGIC, strTelefoneGIC
		...
		do while not objRepresGLA.eof
			...
			if UCASE(objRepresGLA("AGE_DESC")) = "GICL" then
				  strNomeGIC   = objRepresGLA("USU_NOME") 
				  strEmailGIC	 = objRepresGLA("USU_EMAIL") 
				  strTelefoneGIC = objRepresGLA("USU_RAMAL") 
			end if 
		...
		strXML = strXML &	"<telcontatoebt>" & objRSPro("Cpro_Contratantetelefone") & "</telcontatoebt>"
		strXML = strXML &	"<cargoebt>" & objRSPro("Cpro_ContratanteDepto") & "</cargoebt>"
		strXML = strXML &	"<contatocontratada>" & objRSPro("Cpro_ContratadaContato") & "</contatocontratada>"
		strXML = strXML &	"<cargocontatocontratada>" & objRSPro("Cpro_ContratadaDepto") & "</cargocontatocontratada>"

		strXML = strXML &	"<nomegic>" & strNomeGIC &  "</nomegic>"
		strXML = strXML &	"<emailgic>" & strEmailGIC &  "</emailgic>"
		strXML = strXML &	"<telefonegic>" & strTelefoneGIC &  "</telefonegic>"
