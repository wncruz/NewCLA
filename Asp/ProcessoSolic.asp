...
' Custom function: Recursively formats XML node as indented string
Function VisualXML(node, indent)
    Dim result, i, child, attr
    
    ' Start with opening tag and attributes
    result = indent & "<" & node.nodeName
    For Each attr In node.attributes
        result = result & " " & attr.nodeName & "='" & attr.nodeValue & "'"
    Next
    result = result & ">" & vbCrLf
    
    ' Recurse on child elements (skip text nodes for simplicity; adjust if needed)
    For Each child In node.childNodes
        If child.nodeType = 1 Then  ' NODE_ELEMENT = 1
            result = result & VisualXML(child, indent & "  ")  ' Indent children
        ElseIf child.nodeType = 3 Then  ' NODE_TEXT = 3, include text content
            result = result & indent & "  " & child.nodeValue & vbCrLf
        End If
    Next
...
Function FormatVBSDateTimeWithCenti(dt, formatPattern)
    Dim yearStr, monthStr, dayStr, hourStr, minStr, secStr
    yearStr = CStr(Year(dt))
    monthStr = Right("0" & Month(dt), 2)
    dayStr = Right("0" & Day(dt), 2)
    hourStr = Right("0" & Hour(dt), 2)  ' 24-hour: 00-23
    minStr = Right("0" & Minute(dt), 2)
    secStr = Right("0" & Second(dt), 2)
    
    ' Centiseconds via Timer() (fractional seconds)
    Dim totalSeconds : totalSeconds = Timer()
    Dim integerSeconds : integerSeconds = Int(totalSeconds)
    Dim fractional : fractional = totalSeconds - integerSeconds
    Dim centiSeconds : centiSeconds = Round(fractional * 100, 0)
    Dim centiStr : centiStr = Right("0" & centiSeconds, 2)  ' e.g., "12"
    
    Select Case formatPattern
        Case "yyyy/mm/dd hh:mm:ss.cc"  ' With centiseconds
            FormatVBSDateTimeWithCenti = yearStr & "/" & monthStr & "/" & dayStr & " " & _
                                         hourStr & ":" & minStr & ":" & secStr & "." & centiStr
        Case "yyyy-mm-dd hh:mm:ss.cc"  ' ISO for SQL parsing (recommended)
            FormatVBSDateTimeWithCenti = yearStr & "-" & monthStr & "-" & dayStr & " " & _
                                         hourStr & ":" & minStr & ":" & secStr & "." & centiStr
        Case Else
            FormatVBSDateTimeWithCenti = CStr(dt)
    End Select
End Function
  ...
  ''                    msNumeroFax, msMatriculaCS, msEmail, _
   ''                   msNomeContatoCom, msDDDTelefoneCom, _
	...
	if dblIdLogico <> "" then
		...
		End if
	End if
' Supondo que objXmlDadosForm.xml contém o XML que você deseja exibir

strXml = "<?xml version='1.0' ?>" & objXmlDadosForm.xml
xmlContent = strXml

filePath = "C:\temp\objXmlDadosForm.xml.txt" ' Altere para o caminho desejado
...
strXmlDataBase = "<?xml version='1.0'?>" & objXmlDadosDB.xml

Vetor_Campos(1) = "adLongVarWChar, -1, adParamInput, " & strXml  ' Unicode XML, unlimited
Vetor_Campos(2) = "adLongVarWChar, -1, adParamInput, " & strXmlDataBase  ' Unicode XML, unlimited

dim sqlProc
sqlProc = APENDA_PARAMSTR("CLA_sp_ins_new_solicitacaoAtivacao_good", 2, Vetor_Campos)

On Error Resume Next

set sqlProcx = createobject("ADODB.Recordset")
Set sqlProcx = db.Execute(sqlProc)


strXml = CStr(sqlProcx(0).value)

If Err.Number <> 0 Then
	Response.Write "<script type='text/javascript'>"
    Response.Write "alert('SP Call Error: " & Err.Description & "')"
	Response.Write "</script>"
    Response.End
End If
'filePath = "C:\temp\strXml.xml.txt" ' Altere para o caminho desejado
'xmlContent = strXml
' Criar um objeto FileSystemObject
'Set fsox = CreateObject("Scripting.FileSystemObject")

' Criar ou abrir o arquivo de texto
'Set txtFilex = fsox.CreateTextFile(filePath, True)

' Escrever o conteúdo XML no arquivo
'txtFilex.WriteLine xmlContent

' Fechar o arquivo
'txtFilex.Close

' Limpar objetos
'Set txtFilex = Nothing
'Set fsox = Nothing

Response.Write "<script language=javascript>var objXmlRet = new ActiveXObject(""Microsoft.XMLDOM"");objXmlRet.loadXML('" & strXML &"');parent.Messagenew(objXmlRet);</script>"
...
Function ResgatarGLA()
	...
	if VerificaRedir = 0 then
		Response.Write "<script language=javascript> parent.document.Form2.hdntxtGLA.value = """";</script>"
			response.end
	end if
...
Function ResgatarSev(dblNroSev)
	...
	if DBAction <> 0 then
		...
		'Response.Write "<script language=javascript>parent.strdivTecnologia.innerHTML = '" & strSolTec & "'; </script>"
	...
	Else
		If Not objRSCli.eof and  Not objRSCli.bof then
			...
			strSolSel = " <table cellspacing=1 cellpadding=0 border=0 width=760> "
			strSolSel = strSolSel & "					<tr class=clsSilver> "
			strSolSel = strSolSel & "						 <td width=321>Endereço Higienizado:  	<b> SIM  </b></td> "
			strSolSel = strSolSel & "					</TR>" 
			strSolSel = strSolSel & "		</table> "
...
''		Set objXmlDadosForm  = AdicionarNode("hdnPossuiGeren",objXmlDadosForm,Request.Form("hdnPossuiGeren"))
		...
		if objNode.length > 0 then
			...
			Next
		End if
Response.Write "<script language=javascript>alert('saiu');</script>"
	...
	Case "CheckEstacaoInstala"
		'response.write "<script>alert('"&Request.Form("txtCNLLocalInstala")&"')</script>"
		...
		DBAction = ObjCmd.Parameters("RET").value
		strEsc_ID = ObjCmd.Parameters("RET2").value
		'response.write "<script>alert('"&strEsc_ID&"')</script>"
