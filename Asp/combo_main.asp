<%
'	- Sistema			: CLA
'	- Arquivo			: combo_main.asp
'	- Responsável		: Edar
'	- Descrição			: Lista/Remove Combo
%>
<!--#include file="../inc/data.asp"-->
<%
If Trim(Request.Form("hdnAcao")) = "Excluir" then
	Call ExcluirRegistro("CLA_sp_del_Combo")
End if
%>
...
<tr>
	<th colspan=4><p align="center">Cadastro de Combo</p></th>
</tr>
<tr class=clsSilver>
  <td >Busca (Combo Ex.: Modelo EDD ou Fabricante EDD)&nbsp;&nbsp;</td>
  <td colspan=4>
...
if not objRSPag.Eof and not objRSPag.Bof then
	For intIndex = 1 to objRSPag.PageSize
		...
		%>
		<tr class="<%=strClass%>">
			<td ><a href="combo.asp?ID=<%=objRSPag("newcombo_id") %>">&nbsp;<%=TratarAspasHtml(objRSPag("newcombo_nome"))%></a> </td>
			<td> 
			<% strStatus=TratarAspasHtml(objRSPag("newcombo_ativo"))
			  if  strStatus = "S" then
			...
			<!--<td><input  type="checkbox" name="excluir" value="<%=objRSPag("newcombo_id")%>" onClick="AddSelecaoChk(this)"></td>-->
		</tr>
		<%
		intCount = intCount+1
		objRSPag.MoveNext
...
<tr>
	<td align=center>
		...
		<input type="button" class="button" name="Incluir" value="Incluir" onClick="javascript:window.location.replace('combo.asp')" accesskey="I" onmouseover="showtip(this,event,'Incluir (Alt+I)');">
