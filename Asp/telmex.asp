...
strParmProc = dblProId + "|" + dblPedId  + "|" + intTipoProcesso + "|" + strUserName 
	...
	if not isnull(objRSped1("esc_nomelogr")) then 
		...
		strXML = strXML &	"<arquivo>telmex</arquivo>"
		strXML = strXML &	"<titulo>SOLICITAÇÃO DE SERVIÇO TELMEX DO BRASIL</titulo>"
		...
		strXML = strXML &	"<chkativa>" & blnAtiva &  "</chkativa>"
		strXML = strXML &	"<chkdesativa>" & blnDestaivacao &  "</chkdesativa>"
		strXML = strXML &	"<chkcancela>" & blnCancelamento &  "</chkcancela>"
		strXML = strXML &	"<chkaltera>" & blnAltera &  "</chkaltera>"
		strXML = strXML &	"<dtdesativa>" & objRSped1("Acf_DtDesatAcessoFis") &  "</dtdesativa>"
		...
		if not isnull(objRSPontoB("Aec_Telefone")) then strXML = strXML &	"<clientetelefone>( " & mid(objRSPontoB("Aec_Telefone"),1,2) & " ) " & mid(objRSPontoB("Aec_Telefone"),3,len(objRSPontoB("Aec_Telefone")) - 2) & "</clientetelefone>"
		...
		strXML = strXML &	"<localdataebt>" & objRSPro("ContratanteCid") & " " & day(date) & " de " & retornaMes(month(date)) & " de " & year(date) & "</localdataebt>"
		strXML = strXML &	"<localdatacontratada>" & objRSPro("ContratadaCid") & " "  & day(date) & " de " & retornaMes(month(date)) & " de " & year(date) & "</localdatacontratada>"
		strXML = strXML &	"</ROOT>"
