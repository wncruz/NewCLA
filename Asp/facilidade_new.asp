...
dblStsID = Trim(Request.Form("hdnStsId"))
if dblStsID = "283" or dblStsID = "284" then
	blnLibera = "readonly"
	bbloqueia = "readonly"
else
	blnLibera = ""
...
if Trim(dblSolId) <> "" then
	...
	set ObjRSTProcesso = db.execute("select tprc_id from cla_solicitacao where sol_id = " & dblSolId ) 'Request.Form("hdnSolId"))
	if not ObjRSTProcesso.eof then
		tprc_id = ObjRSTProcesso("tprc_id")
	...
	set ObjRssnoaDes = db.execute(" select top 1 cla_acessofisico.Pro_ID,cla_tecnologia.Tec_Nome,cla_acessofisico.Acf_ID, cla_tecnologia.Tec_ID  from cla_solicitacao inner join cla_acessologico 	On cla_solicitacao.Acl_IDAcessoLogico = cla_acessologico.Acl_IDAcessoLogico  inner join cla_acessologicofisico 	on cla_acessologico.Acl_IDAcessoLogico = cla_acessologicofisico.Acl_IDAcessoLogico 	 inner join cla_acessofisico 		on cla_acessologicofisico.acf_id = cla_acessofisico.acf_id 	 left join cla_tecnologia 		on cla_acessofisico.tec_id = cla_tecnologia.tec_id 	 where tprc_id = 2   and sol_id =  " & dblSolId ) 'Request.Form("hdnSolId") )
	'<!-- OI-8064742-16 - Fim -->
	
	if not ObjRssnoaDes.eof then
		strTecnologia = ObjRssnoaDes("Tec_Nome")
		strTecId = ObjRssnoaDes("Tec_ID")
		DblAcf_ID = ObjRssnoaDes("Acf_ID")
		strProId = ObjRssnoaDes("Pro_ID")
	end if
	
	SET ObjRssnoa = Nothing
	SET ObjRssnoaDes = Nothing
	...
	If trim(strLoginRede) = "T3FRRP" then 'or  trim(strLoginRede) = "EDAR" Then
		...
		response.write "<script>alert('"&dblStsID&"')</script>"
...
<table border=0 cellspacing="1" cellpadding="1" width="760">
	...
	<Form name="Form2" method="Post">
		...
		<input type=hidden name=hdndblStsID value="<%=dblStsID%>">
		...
		<input type=hidden name=hdnUserGICL value="<%=trim(strLoginRede)%>">

		<!--Novo - Fim-->

		<!-- CH-83646VWR - Inicio -->
		<input type="hidden" name="hdnNumPedSnoaAnt"	    value="<%=strNumPedSnoaAnt%>">
		<input type="hidden" name="hdnACFid"	    value="<%=DblAcf_ID%>">
		<!-- CH-83646VWR - Fim -->

		<%
		If trim(strLoginRede) = "T3FRRP" then 'or  trim(strLoginRede) = "EDAR" Then
			response.write "<script>alert('" & blnLibera & "')</script>"
		end if
		%>
		...
		<tr><th colspan=6><p style="FONT-SIZE: 14px" align=center>Solicitação de Pedido SNOA</p></th></tr>
		...
		<tr class=clsSilver>
			...
			<td>&nbsp;
				...
				<% if bbloqueia="" then%>
					<select name="CboFornecedora" style="width:200px" onChange="ResgatarContrato(this.value);">
				<%else%>
					<select name="CboFornecedora" style="width:200px" onChange="ResgatarContrato(this.value);" disabled="disabled">
				<%end if%>
					<option value=""></option>
					<%	
			...
			<td colspan="3">&nbsp;
				...
				<% if bbloqueia="" then%>
					<select name="CboContrFornec" style="width:200px">
				<%else%>
					<select name="CboContrFornec" style="width:200px" disabled="=disabled">
				<%end if%>
					<option value=""></option>
					<%	
						...
						While not objRS.Eof
							...
							'Response.Write "<Option value=" & objRS("Bilhete_ContratoID") & strItemSel & ">" & objRS("Contrato_fornecedora") & "</Option>"
							objRS.MoveNext
						Wend
						strItemSel = ""
					%>
		...
		<tr class=clsSilver>
			...
			<td colspan=5>&nbsp;&nbsp;<textarea name='txtDescrEstudo' cols='89' rows='2'<%=bbloqueia%> ><%=strObs%></textarea></td>
		</tr>

		<!-- CH-83646VWR - Inicio -->
		<% 	if trim(strNumPedSnoaAnt) = "" then%>
			<tr class=clsSilver>
				<td nowrap width=170>&nbsp;Nº Pedido SNOA Anterior</td>
				<td colspan=5>&nbsp;&nbsp;<input type="text" class="text" name="txtNumPedSnoaAnt" value="<%=trim(strNumPedSnoaAnt)%>" size="89" maxlength="10" onKeyUp="ValidarTipo(this,0)">
				</td>
			</tr>
		<% else %>
		...
		if not objRS.Eof then
			...
			if objRS("Retorno") = "Verdadeiro" then%>
				<table cellpadding=0 cellspacing=1 width=760>
					<tr>
						<tr>
							<th style="FONT-SIZE: 14px" colspan=4>&nbsp;•&nbsp;Informações de Origem</th>
						</tr>
					<tr>
	...
	<tr>
		<td>
			<iframe	id			= "IFrmListaStatusSNOA"
				    ...
				    width       = "100%"
				    height      = "120px"
				    src			= "../inc/ListaStatusSNOA.asp?dblSolId=<%=dblSolId%>&dblPedId=<%=dblPedId%>&telaaceitar=<%=telaaceitar%>"
				    frameborder = "0"
				    scrolling   = "no"
				    align       = "left">
			</iFrame>
			...
			'if objRS("Retorno") = "Verdadeiro" then%>
				<table cellpadding=0 cellspacing=1 width=760>
					...
					<tr class=clsSilver>
						...
						<td colspan=5>&nbsp;
							<span id="solicPedSnoaTipoAcao">
								<select name="cboTipoAcao" style="width:250px">
								<!--<option value=""></option>-->
								<%	
									set objRS = db.execute("CLA_sp_sel_AcaoSnoa " & str_tprc_id & "," & dblSolId)
									While not objRS.Eof
										if objRS("acaoSnoa_id") = "6" then
											str_ACAOId = objRS("acaoSnoa_id")
										end if 
										strItemSel = ""
					...
					<tr class=clsSilver>
						...
						<td colspan=5>&nbsp; 
								<span id="spncboMotivoSNOA">
									<select name="cboMotivoSNOA" style="width:200px">
									<option value=""></option>
									<%	
						...
						<tr class=clsSilver>
							<td nowrap width=170>&nbsp;Designação da Fornecedora</td>
							<%
								'NOVAS PROCEDURES - SNOA - INICIO'
								Vetor_Campos(1)="adInteger,2,adParamInput,null"
								Vetor_Campos(2)="adInteger,2,adParamInput," & strNumPedSnoaAnt
								...
								if str_Designacao <> "" then
									bbloqueiaDes = "readonly"
								else
									bbloqueiaDes = ""
								end if
							%>
							...
							<td colspan=5>				
									&nbsp;
									<input type="text" class="text"name="txtDesignacaoFornecedora" size="60" maxlength="100"  onKeyPress="MaxLength(this,100)" value="<%=trim(str_Designacao)%>" <%= bbloqueiaDes %> />
							</td>
						</tr>
						<tr class=clsSilver>
							...
							<td colspan=5>				
									&nbsp;
									<textarea name="txtMotivo" cols="80" rows="2" onKeyPress="MaxLength(this,1000)"></textarea>
							</td>
						</tr>
					...
					<tr class="clsSilver">
						...
						<td>&nbsp;
							<input type="text" class="text" name="txtCproContratanteContato" value="<%=trim(str_Cpro_ContratanteContato)%>" maxlength="20" size="30" <%=bbloqueia%> >
						</td>
						<td>Email</td>
						<td>
							<input type="text" class="text" name="txtCproContratanteEmail" value="<%=trim(str_Cpro_ContratanteEmail)%>"	maxlength="50" size="27" > <!--onkeyUp="ValidarTipo(this,0)">-->
						</td>
					</tr>
					...
					<tr class="clsSilver">
						...
						<td>&nbsp;
							<input type="text" class="text" name="txtCproContratanteTelefone" value="<%=trim(str_Cpro_ContratanteTelefone)%>" maxlength="20" size="30" onkeyUp="ValidarTipo(this,0)">
						</td>
						<td>Fax</td>
						<td>
							<input type="text" class="text" name="txtCproContratanteFax" value="<%=trim(str_Cpro_ContratanteFax)%>"	maxlength="20" size="27" onkeyUp="ValidarTipo(this,0)">
						</td>
					</tr>
					...
					<tr class="clsSilver">
						...
						<td>&nbsp;
							<input type="text" class="text" name="txtCproContaCustomizadaPonta_a" value="<%=trim(str_Cpro_conta_customizada_ponta_a)%>" maxlength="50" size="30" <%=bbloqueia%> >
						</td>
						<td>Conta Customizada Ponta B</td>
						<td>
							<input type="text" class="text" name="txtCproContaCustomizadaPonta_b" value="<%=trim(str_Cpro_conta_customizada_ponta_b)%>" maxlength="50" size="27" <%=bbloqueia%> >
						</td>
					</tr>
					...
					<tr class=clsSilver>
						...
						<td colspan="5">&nbsp;
							<input type="text" class="text" name="txtNome_ReprComer" value="<%=trim(str_Nome_ReprComer)%>" size="88" maxlength="50" <%=bbloqueia%> >
						</td>
					</tr>
					...
					<tr class="clsSilver">
						<td width=170px>Endereço</td>
						<td colspan=3>&nbsp;
							<input type="text" class="text" name="txtEnd_ReprComer" value="<%=trim(str_End_ReprComer)%>" size="88" maxlength="50" <%=bbloqueia%> >
						</td>
					</tr>
					...
					<tr class="clsSilver">
						...
						<td>&nbsp;
							<select name="cboUFEnd_ReprComer" >
								<Option value=""></Option>
			            		<%
						...
						<td nowrap right>CEP&nbsp;</td>
						<td>
							<input type="text" class="text" name="txtCepEnd_ReprComer" value="<%=trim(str_End_CEP_ReprComer)%>" maxlength="9" size="12">
						</td>
					</tr>
					...
					<tr class="clsSilver">
						...
						<td nowrap colspan=3 >&nbsp;
							...
							onBlur="if (ValidarTipo(this,1)){ResgatarCidadeSnoa(document.forms[0].cboUFEnd_ReprComer,1,this)}">&nbsp;
							<input type=text size=27 readonly style="BACKGROUND-COLOR:#eeeeee" class=text
							name="txtEndDescCid_ReprComer" readonly value="<%=str_Cid_Desc_ReprComer%>" tabIndex=-1>
                		</td>
					</tr>
					...
					<tr class="clsSilver">
						<td>Email</td>
						<td nowrap colspan=3 >&nbsp;
							<input type="text" class="text" name="txtEmailTec_ReprComer" value="<%=strim(str_Email_ReprComer)%>" maxlength="50" size="88">&nbsp;
						</td>
					</tr>
					...
					<tr class="clsSilver">
						<td width=170px>Telefone</td>
						<td>&nbsp;
							<input type="text" class="text" name="txtTelefone_ReprComer" value="<%=trim(str_Telefone_ReprComer)%>" maxlength="20" size="30" <%=bbloqueia%> >
						</td>
						<td>Fax</td>
						<td>
							<input type="text" class="text" name="txtFax_ReprComer" value="<%=trim(str_Fax_ReprComer)%>"	maxlength="20" size="30" onkeyUp="ValidarTipo(this,0)">
						</td>
					</tr>
					<!-- Fim Endereco-->
					...
					<tr class=clsSilver>
						<td nowrap width=170>Taxa de Transmissão</td>
						<td>&nbsp;
			...
			-->
				...
				<table cellpadding=0 cellspacing=1 width=760>
					...
					<tr class=clsSilver>
						...
						<td colspan="5">&nbsp;
							<input type="text" class="text" name="txtUsuario_A" value="<%=str_Cliente%>" size="80" maxlength="50" <%=bbloqueia%> >
						</td>
					</tr>
					...
					<tr class=clsSilver>
						...
						<td colspan="4">
							&nbsp;
							<input type="text" class="text" name="txtLatEnd_A" onKeyUp="ValidarTipo(this,9)" value="<%=str_Latitude_A%>" size="20" maxlength="20" 
							<%=bbloqueia%> >
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							Longitude&nbsp;
							&nbsp;&nbsp;
							<input type="text" class="text" name="txtLongEnd_A" onKeyUp="ValidarTipo(this,9)" value="<%=str_Longitude_A%>" size="20" maxlength="20" 
							<%=bbloqueia%> >
							&nbsp;&nbsp;
						<% if bbloqueia="" then%>
							<input type=button name=btnProcurarCoord_A value="Localizar Coordenadas" 	
							class="button" onclick="Validar_LatLong('A')" tabindex=-1 
							onmouseover="showtip(this,event,'Procurar por CEP exato ou pelos 5 primeiros dígitos (Alt+D)');" accesskey="D">
						<%else%>
							<input type=button name=btnProcurarCoord_A value="Localizar Coordenadas"  class="button" tabindex=-1 disabled="disabled">
						<%end if%>
						</td>
					...
					<tr class="clsSilver">
						<td>&nbsp;Complemento</td>
						<td colspan=3>&nbsp;
							<input type="text" class="text" name="txtComplEnd_A"  value="<%=trim(str_Aec_Complemento_PTA)%>" maxlength="120" size="80"  <%=bbloqueia%> >
						</td>
					</tr>
					...
					<tr class="clsSilver">
						...
						<td colspan=3>&nbsp;
							<input type="text" class="text" name="txtPontoRefencia_A"  value="<%=trim(str_Ponto_Referencia_A)%>" maxlength="80" size="80" <%=bbloqueia%> >
						</td>
					</tr>
					...
					<tr class="clsSilver">
						...
						<td colspan = 3 >&nbsp;
							...
							value="<%=trim(str_End_Bairro_PTA)%>" maxlength="120" size="40" readonly="TRUE">&nbsp;
						</td>			
					</tr>
					...
					<tr class="clsSilver">
						...
						<td>&nbsp;
							<input type="text" class="text" name="txtContatoEnd_A" value="<%=rtrim(str_Aec_Contato_PTA)%>" maxlength="30" size="30"  >
						</td>
						...
						<td>
							...
							<input type="text" class="text" name="txtTelEnd_A" value="<%=rtrim(str_Aec_Telefone_PTA)%>"	maxlength="15" size="11" onkeyUp="ValidarTipo(this,0)">
					...
					<tr class="clsSilver">
	    				...
	    				<td>
	      					<input type="text" class="text" name="txtEmailTec_A"  value="<%=trim(str_Email_PTA)%>" maxlength="50" size="30" <%=bbloqueia%> >&nbsp;
	    				</td>
					</tr>
					<!-- Fim Endereco-->
					...
					<tr class=clsSilver>
						...
						<td>&nbsp;
							...
							<% if bbloqueia="" then%>
								<input type="radio" name="rdoEquip_A" onClick="javascript:document.Form1.hdnQinQ.value = 'S';" value="S"
								<%if str_Equipamento_A = "S" then%> checked <%end if%>>&nbsp; Sim
			
								<input type="radio" name="rdoEquip_A" onClick="javascript:document.Form1.hdnQinQ.value = 'N';" value="N"
								<%if str_Equipamento_A <> "S" then%> checked <%end if%>>&nbsp; Não

							<%else%>

								<input type="radio" name="rdoEquip_A" value="S" disabled="disabled"
								<%if str_Equipamento_A = "S" then%> checked <%end if%>>&nbsp; Sim
			
								<input type="radio" name="rdoEquip_A" value="N" disabled="disabled"
								<%if str_Equipamento_A <> "S" then%> checked <%end if%>>&nbsp; Não
						...
						<td>&nbsp;
							...
							<% if bbloqueia="" then%>
								<input type="radio" name="rdoCentroFios_A" onClick="javascript:document.Form1.hdnQinQ.value = 'S';" value="S"
								<%if str_CentroFios_A = "S" then%> checked <%end if%>>&nbsp; Sim
			
								<input type="radio" name="rdoCentroFios_A" onClick="javascript:document.Form1.hdnQinQ.value = 'N';" value="N"
								<%if str_CentroFios_A <> "S" then%> checked <%end if%>>&nbsp; Não
							<%else%>
								<input type="radio" name="rdoCentroFios_A" value="S" disabled="disabled"
								<%if str_CentroFios_A = "S" then%> checked <%end if%>>&nbsp; Sim
			
								<input type="radio" name="rdoCentroFios_A" value="N" disabled="disabled"
								<%if str_CentroFios_A <> "S" then%> checked <%end if%>>&nbsp; Não
							<%end if%>
							</span>
						</td>
					...
					<tr class=clsSilver>
						...
						<td colspan = 3 >&nbsp;
							...
							<% if bbloqueia="" then%>
								<input type="radio" name="rdoEntrCanalizada_A" 
							...
							<%else%>
								<input type="radio" name="rdoEntrCanalizada_A" value='S' disabled="disabled"
								<%if str_Entrega_Canalizado_A = "S" then%> checked <%end if%>>&nbsp; Sim
			
								<input type="radio" name="rdoEntrCanalizada_A" value='N' disabled="disabled"
								<%if str_Entrega_Canalizado_A <> "S" then%> checked <%end if%>>&nbsp; Não
							<%end if%>
							</span>
						</td>
					...
					<tr class=clsSilver>
						...
						<td>&nbsp;
							...
							<% if bbloqueia="" then%>
								<select name="cboInterfEletr_A" style="width:190px">
							<%else%>
								<select name="cboInterfEletr_A" style="width:190px" disabled="disabled">
							<%end if%>
								<option value=""></option>
								<%	
						...
						<td>&nbsp;
							...
							<% if bbloqueia="" then%>
								<select name="cboInterfFisica_A" style="width:190px">
							<%else%>
								<select name="cboInterfFisica_A" style="width:190px" disabled="disabled">
							<%end if%>
								<option value=""></option>
								<%	
				...
				<table cellpadding=0 cellspacing=1 width=760>
					...
					<tr class=clsSilver>
						<td width=170>&nbsp;Usuário</td>
						<td colspan="5">&nbsp;
							<input type="text" class="text" name="txtUsuario_B" value="EMBRATEL SA" size="80" 
							maxlength="50" <%=bbloqueia%> >
						</td>
					</tr>
					...
					<tr class=clsSilver>
						...
						<td colspan="4">
							&nbsp;
							<input type="text" class="text" name="txtLatEnd_B" onKeyUp="ValidarTipo(this,9)" value="<%=str_Latitude_B%>" size="20" maxlength="20" <%=bbloqueia%> >
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							Longitude&nbsp;
							&nbsp;&nbsp;
							<input type="text" class="text" name="txtLongEnd_B" onKeyUp="ValidarTipo(this,9)" value="<%=str_Longitude_B%>" size="20" maxlength="20" <%=bbloqueia%> >
							&nbsp;&nbsp;
						<% if bbloqueia="" then%>
							...
							onmouseover="showtip(this,event,'Procurar por CEP exato ou pelos 5 primeiros dígitos (Alt+D)');" accesskey="D">
						<%else%>
							<input type=button name=btnProcurarCoord_B value="Localizar Coordenadas " class="button" tabindex=-1 disabled="disabled">
						<%end if%>
						</td>
					...
					<tr class="clsSilver">
						<td>&nbsp;Complemento</td>
						<td colspan=3>&nbsp;
							<input type="text" class="text" name="txtComplEnd_B"  value="<%=trim(str_Esc_Complemento_PTB)%>" maxlength="120" size="80" <%=bbloqueia%> >
						</td>
					</tr>
					...
					<tr class="clsSilver">
						<td>&nbsp;Ponto de Referência</td>
						<td colspan=3>&nbsp;
							<input type="text" class="text" name="txtPontoRefencia_B"  value="<%=trim(str_Ponto_Referencia_B)%>" maxlength="80" size="80" <%=bbloqueia%> >
						</td>
					</tr>
					...
					<tr class="clsSilver">
						...
						<td colspan = 3 >&nbsp;
							...
							value="<%=trim(str_Esc_Bairro_PTB)%>" maxlength="120" size="40" readonly="TRUE">&nbsp;
						</td>			
					</tr>
					...
					<tr class="clsSilver">
						<td width=170px><font class="clsObrig">:: </font>Contato</td>
						<td>&nbsp;
							<input type="text" class="text" name="txtContatoEnd_B" value="<%=trim(str_Esc_Contato_PTB)%>" maxlength="30" size="30" <%=bbloqueia%> >
						</td>
						<td><font class="clsObrig">:: </font>Telefone</td>
						<td>
						<% if bbloqueia="" then%>						
							<input type="text" class="text" name="txtTelEnd_B" value="<%=rtrim(str_Esc_Telefone_PTB)%>" maxlength="15" size="11" onkeyUp="ValidarTipo(this,0)">
						<%else%>
							<input type="text" class="text" name="txtTelEnd_B" value="<%=rtrim(str_Esc_Telefone_PTB)%>" maxlength="15" size="11" readonly="true">
						<%end if%>
						</td>
					</tr>
					...
					<tr class="clsSilver">
						...
						<% if bbloqueia="" then%>
							...
							<%=bbloqueia%>
							size="16" onKeyUp="ValidarTipo(this,0)" value="<%=str_Esc_CNPJ_PTB%>" >&nbsp;(99999999999999)
						<%else%>
							<input type="text" class="text" name="txtCNPJ_B"  maxlength="14"
							<%=bbloqueia%>
							size="16" value="<%=str_Esc_CNPJ_PTB%>" >&nbsp;(99999999999999)
						<%end if%>
						</td>
						<td width=170px>&nbsp;</font>Email Contato Técnico</td>
	    				<td>
	      					<input type="text" class="text" name="txtEmailTec_B"  value="<%=trim(str_Esc_Email_PTB)%>" maxlength="50" size="30" <%=bbloqueia%> >&nbsp;
	    				</td>
					</tr>
					<!-- Fim Endereco-->
					...
					<tr class=clsSilver>
						...
						<td>&nbsp;
							...
							<% if bbloqueia="" then%>
								<input type="radio" name="rdoEquip_B" onClick="javascript:document.Form1.hdnQinQ.value = 'S';" value="S"
								<%if str_Equipamento_B = "S" then%> checked <%end if%>>&nbsp; Sim
			
								<input type="radio" name="rdoEquip_B" onClick="javascript:document.Form1.hdnQinQ.value = 'N';" value="N"
								<%if str_Equipamento_B <> "S" then%> checked <%end if%>>&nbsp; Não
							<%else%>
								<input type="radio" name="rdoEquip_B" value="S" disabled="disabled"
								<%if str_Equipamento_B = "S" then%> checked <%end if%>>&nbsp; Sim
			
								<input type="radio" name="rdoEquip_B" value="N" disabled="disabled"
								<%if str_Equipamento_B <> "S" then%> checked <%end if%>>&nbsp; Não
							<%end if%>
							</span>
						</td>
						...
						<td>&nbsp;
							...
							<% if bbloqueia="" then%>
								<input type="radio" name="rdoCentroFios_B" onClick="javascript:document.Form1.hdnQinQ.value = 'S';" value="S"
								<%if str_CentroFios_B = "S" then%> checked <%end if%>>&nbsp; Sim
			
								<input type="radio" name="rdoCentroFios_B" onClick="javascript:document.Form1.hdnQinQ.value = 'N';" value="N"
								<%if str_CentroFios_B <> "S" then%> checked <%end if%>>&nbsp; Não
							<%else%>
								<input type="radio" name="rdoCentroFios_B" value="S" disabled="disabled" 
								<%if str_CentroFios_B = "S" then%> checked <%end if%>>&nbsp; Sim
			
								<input type="radio" name="rdoCentroFios_B" value="N" disabled="disabled" 
								<%if str_CentroFios_B <> "S" then%> checked <%end if%>>&nbsp; Não
							<%end if%>
							</span>
						</td>
					...
					<tr class=clsSilver>
						...
						<td colspan = 3 >&nbsp;
							...
							<% if bbloqueia="" then%>
								<input type="radio" name="rdoEntrCanalizada_B" 
								onClick="javascript:document.forms[0].txtTimeSlot_B.disabled=false,document.forms[0].txtTimeSlot_B.value='', document.forms[0].txtE1Canalizado_B.disabled=false,document.forms[0].txtE1Canalizado_B.value='';" value="S"
								<%if str_Entrega_Canalizado_B = "S" then%> checked <%end if%>>&nbsp; Sim
			
								<input type="radio" name="rdoEntrCanalizada_B" 
								onClick="javascript:document.forms[0].txtTimeSlot_B.disabled=true,document.forms[0].txtTimeSlot_B.value='', document.forms[0].txtE1Canalizado_B.disabled=true,document.forms[0].txtE1Canalizado_B.value='';" value="N"
								<%if str_Entrega_Canalizado_B <> "S" then%> checked <%end if%>>&nbsp; Não
							<%else%>
								<input type="radio" name="rdoEntrCanalizada_B" value="S" disabled="disabled"
								<%if str_Entrega_Canalizado_B = "S" then%> checked <%end if%>>&nbsp; Sim
			
								<input type="radio" name="rdoEntrCanalizada_B" value="N" disabled="disabled"
								<%if str_Entrega_Canalizado_B <> "S" then%> checked <%end if%>>&nbsp; Não
							<%end if%>
							</span>
						</td>
					...
					<tr class=clsSilver>
						...
						<td>&nbsp;
							...
							<% if bbloqueia="" then%>
								<select name="cboInterfEletr_B" style="width:190px">
							<%else%>
								<select name="cboInterfEletr_B" style="width:190px" disabled="disabled">
							<%end if%>
								<option value=""></option>
								<%	
									set objRS = db.execute("CLA_sp_sel_InterfaceEletrica")
						...
						<td>&nbsp;
							...
							<% if bbloqueia="" then%>
								<select name="cboInterfFisica_B" style="width:190px">
							<%else%>
								<select name="cboInterfFisica_B" style="width:190px" disabled="disabled">
							<%end if%>
								<option value=""></option>
								<%	
									set objRS = db.execute("CLA_sp_sel_InterfaceFisica")
		...
		<td>
			<table width=100% border=0 align=center cellspacing=1 cellpadding=1>
				<tr class=clsSilver2>
					...
					<!--response.write "<script>alert('"&Trim(Request.QueryString("SolId"))&"')</script>" -->
					<!--
					<%if ( ( cint("0" & tprc_id) = 2 ) or (cint("0" & str_ACAOId ) = 6 ) ) then%>
						<td colspan=4 align=center >
							<input type="button" class="button"
							name="btnEmailPro" style="width:150px"
							value="Enviar e-mail para BDA/SNOA"
							onclick="EnviarEmailLiberacaoSnoa()"
							accesskey="M" style="width:150px;height:22px" onmouseover="showtip(this,event,'Enviar email para provedor(Alt+M)');">
						</td>
					<%End if%>
					-->
					<td colspan=4 align=center>
						<input type="button" class="button" name="btnOK" style="width:150px;height:22px" value="Enviar Solicitação" onclick=" GravarSolicPedSNOA1()" accesskey="I" onmouseover="showtip(this,event,'Solicitar Pedido SNOA (Alt+I)');">
					</td>
				</tr>
			</table>
		</td>
		...
		<td>
			<table width=100% border=0 align=center cellspacing=1 cellpadding=1>
				...
				<%if cint("0" & intTipoProcesso) = 4 then%>
					<td colspan=4 align=center>
						<input type="button" class="button" name="btnOK" style="width:150px;height:22px" value="Concluir Cancelamento" onclick="return GravarFacilidade()" accesskey="I" onmouseover="showtip(this,event,'Concluir cancelamento (Alt+I)');">
					</td>
				<%Else%>
					<%if (  intTipoProcesso = 3 and intTipoAcao = 3 ) then%>
						...
						<td colspan=4 align=center>
							<input type="button" class="button" name="btnOK" style="width:150px;height:22px" value="Enviar Solicitação" onclick="return GravarFacilidade()" accesskey="I" onmouseover="showtip(this,event,'Alocar Facilidade(s)(Alt+I)');">
						</td>
...
function EnviarEmailLiberacaoSnoa()
{
	...
	//alert(document.Form2.hdnSol_Id.value)
	//alert(document.Form2.hdnUsuID.value)
	strXML = "<root>"
	...
	xmlDoc.loadXML(strXML);
	xmlhttp.Open("POST", "RetornaCartaliberacaoSnoa.asp?solid=" + document.Form2.hdnSol_Id.value, false);
	xmlhttp.Send(xmlDoc.xml);
