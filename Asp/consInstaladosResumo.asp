...
'	- Sistema			: CLA
'	- Arquivo			: consInstaladosResumo.asp
'	- Responsável		: Vital
...
<tr>
<td >
<form name="f" method="post" action="consInstaladosResumo.asp" onSubmit="return false">
<table border="0" cellspacing=1 cellpadding=0 width=760>
<tr>
	<th colspan=2><p align="center">Acessos Instalados por Período (Resumo)</p></th>
</tr>
<tr class=clsSilver>
...
<%
if ( ( Trim(request("cboProvedor")) <> ""  or Trim(request("cboHolding")) <> "" ) and Trim(request("txtDataFim")) <> "" ) then

'**************************************************************************
'*** COLETA PARA BLOQUEIO DE CONSULTAS POR MOTIVO DE PERFORMANCE DO CLA ***
'**************************************************************************
db.execute("insert into newcla.tab_temp2(Valor) values('Acessos Instalados por Período (Resumo);' + CAST(CONVERT(varchar(19),getDate(),126) as varchar) + ';" & trim(strLoginRede) & ";" & request("txtDataInicio") & ";" & request("txtDataFim") & "')")
...
strSql = APENDA_PARAMSTRSQL("CLA_sp_cons_InstaladosResumo",7,Vetor_Campos)
...
<input type="hidden" name="hdnProvedor" value="<%=Request.Form("hdnProvedor")%>">
<input type="hidden" name="hdnDataInicio" value="<%=Request.Form("txtDataInicio")%>">
<input type="hidden" name="hdnDataFim" value="<%=Request.Form("txtDataFim")%>">
...
function Consultar()
{
	with (document.forms[0])
	{
		...
		action = "consInstaladosResumo.asp"
...
function AbrirExcel()
{
	with (document.forms[0])
	{
		BreakItUp()
		target = "_blank"
		action = "excelConsInstaladosResumo.asp"
		submit()
	}
}
