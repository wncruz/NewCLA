...
<form name=Form1 Method=post action="ProcessoMonitoracao.asp">
...
'' LPEREZ 13/12/2005
	...
	Vetor_Campos(18)="adInteger,10,adParamInput,"	&  holding
	
	
strSql = APENDA_PARAMSTRSQL("CLA_sp_view_monitoracao",18,Vetor_Campos)
'SqlXls = strSql
...
%>
	...
	'			Response.Write "<input type=button class=button name=btnAddAtivacao value=Adicionar onclick=""AtualizarStatus('pendenteativacao','" & Trim(objRSPag("Sol_ID")) & "', '" & Trim(objRSPag("Ser_ID")) & "','" & Trim(objRSPag("Ser_Desc")) & "','" & Trim(objRSPag("Acl_IDAcessoLogico")) & "','" & Trim(objRSPag("Sts_ID")) & "','" & Trim(strPropAcesso) & "','" & Trim(objRSPag("Tprc_ID")) & "','" & Trim(objRSPag("orisol_id")) & "')""></span></td></tr>"
	'	End Select
	'End if	
	...
	response.write "<center><table><tr><td align=center valign=center width=100% height=20 ><font color=red><center>&nbsp;•&nbsp;Registro(s) não encontrado(s).</font></td></tr></table>"
End if
%>
