...
'	Response.Write "dblPedId=" & dblPedId & "<BR>"
'	Response.Write "dblSolId=" & dblSolId & "<BR>"
'	Response.Write "DblNdTipo=" & DblNdTipo & "<BR>"
'	Response.Write "intTipoProcesso=" & intTipoProcesso & "<BR>"
...
case 4 
	...
	set ObjPendencia = db.execute ("select top 1 AprovPend_id, Aprovisi_id, Acl_IDAcessoLogico ,  Observacao from cla_aprovPendencia where Acl_IDAcessoLogico = " & objRSped1("Acl_IDAcessoLogico") & " order by 1 desc " )
								
	if not ObjPendencia.eof then  						
		  var_obsSaip = ObjPendencia("Observacao")
	end if
...
DIM   strXML
	...
	strXML = strXML & "<arquivo>FOETHERNET</arquivo>"
	...
	strXML = strXML & "<acfid>"  & objRSPontoB("acf_id") & "</acfid>"
...
''	strXML = strXML & "<icms></icms>" 
	...
	set ObjCombo = db.execute ("select distinct cla_aprovisionador.sol_id, cla_aprovisionador.acao,cla_aprovisionador.id_tarefa_can, cla_aprovisionador.ser_desc, cla_aprovisionador.acl_designacaoservico, cla_aprovisionador.vel_desc , cla_aprovisionador.data_recebimento , cla_aprovisionador.aprovisi_dtRecebimentoNotif from cla_acessologico WITH (NOLOCK) inner join cla_aprovisionador WITH (NOLOCK) on cla_acessologico.id_acesso = cla_aprovisionador.id_acesso where orisol_descricao = 'ASMS' and cla_acessologico.Acl_IDAcessoLogico = " & objRSped1("Acl_IDAcessoLogico") & " order by 1 desc " )
								
	if not ObjCombo.eof then
		  do While not ObjCombo.Eof	 
		  		dim exibe_servico_carta		  		 
		  		if isnull(ObjCombo("sol_id")) then
		  			 	exibe_servico_carta = True
		  		else
		  			 set ObjTemp = db.execute ("select cla_statussolicitacao.sts_id from cla_solicitacao WITH (NOLOCK) inner join cla_statussolicitacao WITH (NOLOCK) on cla_statussolicitacao.stssol_id = cla_solicitacao.stssol_id where cla_solicitacao.sol_id=" & ObjCombo("sol_id"))
		  			 if ObjCombo("acao") = "CAN" then
								set ObjTempCan = db.execute ("select distinct cla_pedido.ped_numero from cla_acessologicofisico " &_
										"inner join cla_acessofisico on cla_acessofisico.acf_id=cla_acessologicofisico.acf_id " &_
										"inner join cla_pedidofisico on cla_pedidofisico.acf_id=cla_acessofisico.acf_id " &_
										"inner join cla_pedido on cla_pedido.ped_id=cla_pedidofisico.ped_id " &_
										"inner join cla_solicitacao on cla_solicitacao.acl_idacessologico=cla_acessologicofisico.acl_idacessologico " &_
										"where cla_pedido.ped_numero=" & objRSped1("Ped_Numero") & " and ped_ano=" & objRSped1("Ped_Ano") & " and cla_solicitacao.sol_id=" & ObjCombo("sol_id"))											 	
																					 	
		  			 				if  ObjTempCan.Eof then		  			 					 
		  			 					  exibe_servico_carta = False		  			 					 
		  			 				else
		  			 					  exibe_servico_carta = true	
		  			 				end if
					 elseif ObjCombo("acao") = "ALT" or ObjCombo("acao") = "DES" then
					 		if ObjTemp("sts_id") ="122" then
		  			 			exibe_servico_carta = false	
		  			 		else
								if ObjCombo("aprovisi_dtRecebimentoNotif") <>  "" then
									exibe_servico_carta = false
								else
		  			 				exibe_servico_carta = true	
								end if
		  			 		end if
		  			 elseif ObjCombo("acao") = "ATV" then
		  			 		if ObjTemp("sts_id") ="55" or ObjTemp("sts_id") ="122" then
		  			 			exibe_servico_carta = false	
		  			 		else
		  			 			exibe_servico_carta = true	
		  			 		end if
		  		...
		  		if exibe_servico_carta = True then
		  			strXML = strXML & "<Combo>"
		 				 strXML = strXML & "<DesignacaoServico>" & ObjCombo("acl_designacaoservico") & "</DesignacaoServico>"
		 				 strXML = strXML & "<DescVelAcessoLog>" & ObjCombo("vel_desc") & "</DescVelAcessoLog>"
		 				 strXML = strXML & "<serviço>" & ObjCombo("ser_desc") & "</serviço>"			
		 				 if ObjCombo("acao") = "CAN" then
		 				 		 set ObjTemp = db.execute ("select acao from cla_aprovisionador WITH (NOLOCK) where id_tarefa = " & ObjCombo("id_tarefa_can"))  		
		 						 if ObjTemp("acao") = "ATV" then
           					strXML = strXML & "<strServico>" & "Cancelamento de ativação" & "</strServico>"			
          			 elseIf ObjTemp("acao") = "ALT" then
           					strXML = strXML & "<strServico>" & "Cancelamento de alteração" & "</strServico>"			
           			 end if
		 				 else
		 			 			 if ObjCombo("acao") = "ATV" then 
		 			 			 		strXML = strXML & "<strServico>" & "Ativação" & "</strServico>"	
		 			 			 elseIf ObjCombo("acao") = "ALT" then
		 						 		strXML = strXML & "<strServico>" & "Alteração" & "</strServico>"	
		 						 elseIf ObjCombo("acao") = "DES" then
		 						 		strXML = strXML & "<strServico>" & "Retirada" & "</strServico>"	
		 						 end if					 
		 			 	end if		
						strXML = strXML & "<data>" & ObjCombo("data_recebimento") & "</data>"
		 		 	 strXML = strXML & "</Combo>" 
		  		end if
		  	ObjCombo.movenext
		  loop	
		 ...
		 strXML = strXML & "<Combo>"
		 	...
		 	strXML = strXML & "<serviço>" & objRSped1("ser_desc") & "</serviço>"		
		 	strXML = strXML & "<strServico>" & strServico & "</strServico>"								
		 strXML = strXML & "</Combo>"
	end if
	...
	set ObjContrato = db.execute (" select designacaoContrato from cla_ontVlan where Acl_IDAcessoLogico = " & objRSped1("Acl_IDAcessoLogico") & " order by 1 desc" )
			
	dim designacaoContrato					
	if not ObjContrato.eof then
		designacaoContrato = ObjContrato("designacaoContrato")
	...
	strXML = strXML & "<designacao>" & designacaoContrato & "</designacao>"
...
'' LPEREZ 13/12/2005
	...
	'' PRSSILV - 05/03/2009 - Correção causa raiz OBS.
		...
		strXML = strXML &	"<observacaoSaip>" & var_obsSaip & "</observacaoSaip>"
'' PRSSILV
'' LP
	strXML = strXML & "</telemar>" 
	strXML = strXML & "</root>"
