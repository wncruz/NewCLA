...
Function AprovarAcesso()
   ...
   dblSolId = Request.Form("hdnSolId")
	...
	Vetor_Campos(4)="adWChar,1,adParamInput,"  'Atualiza Data Devolução p/ GIC. Caso S, atualiza com data atual. Caso "N", a data é removida(Null).
...
on error resume next
	...
	Vetor_Campos(2)="adWChar,1,adParamInput,N"  'Atualiza Data Inicio Avaliacao (S/N)
	Vetor_Campos(3)="adWChar,1,adParamInput,N"  'Atualiza Data Fim Avaliacao (S/N)
	Vetor_Campos(4)="adWChar,1,adParamInput,N"  'Atualiza Data Devolução p/ GIC p/ Null
	Vetor_Campos(5)="adInteger,4,adParamOutput,0"
...
if err.number <> 0 then
	...
	If DBAction <> 0 Then
		Response.Write "<script language=javascript>alert('Ola');</script>"
		Response.Write "<script language=javascript>parent.resposta(" & Cint("0" & DBAction) & ",'');</script>"
...
'Response.Write "<script language=javascript>alert('Acao: " & Trim(Request.Form("hdnAcao")) & "..');</script>"
