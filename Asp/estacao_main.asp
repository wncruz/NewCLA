...
'	- Sistema			: CLA
'	- Arquivo			: Estacao_Main.asp
'	- Responsável		: Vital
'	- Descrição			: Lista/Remove estação
%>
<!--#include file="../inc/data.asp"-->
<%
If Trim(Request.Form("hdnAcao"))="Excluir" then
	Call ExcluirRegistro("CLA_sp_del_estacao")
End if
%>
...
<tr>
	<td width=100%>
		<table border="0" cellspacing="1" cellpadding=0 width=760 >
			<tr>
				<th colspan=2><p align="center">Cadastro de Estação</p></th>
			</tr>
			<tr class=clsSilver>
...
<tr>
	...
	<th width="310">Endereço</th>
	<%'@@JKNUP: Adicionado.%>
	<th width="55">Cód SAP</th>
	<th width="105">Contato</th>
...
if Trim(Request.Form("txtBusca")) <> ""  then
	...
	if Ubound(objAryParam) > 0 then
		strSql = "CLA_sp_sel_estacao 0,'" & TratarAspasSql(objAryParam(0)) & "','" & TratarAspasSql(objAryParam(1)) & "'"
	Else
		strSql =  "CLA_sp_sel_estacao 0,'" & TratarAspasSql(objAryParam(0)) & "'"
	End if	
'Else
'	strSql = "CLA_sp_sel_estacao 0"
End if
...
if not objRSPag.Eof and not objRSPag.Bof then
	For intIndex = 1 to objRSPag.PageSize
		...
		<tr class=<%=strClass%>>
<%''@@LPEREZ - 25/05/2006%>			
			<td><a href="manter_estacao.asp?TpProc=ALTER&strEst=<%=objRSPag("Esc_id") %>">&nbsp;<%=TratarAspasHtml(objRSPag("Cid_Sigla"))%></a></td>
<%'@@LP%>	
			<td>&nbsp;<%=TratarAspasHtml(objRSPag("Esc_Sigla"))%></td>
			<td onmouseover="showtip(this,event,'<%
				if not isnull(objRSPag("Tpl_Sigla")) then response.write TratarAspasHtml(objRSPag("Tpl_Sigla")) & " " & TratarAspasHtml(objRSPag("Esc_NomeLogr")) & " nº " & TratarAspasHtml(objRSPag("Esc_NroLogr")) end if
				%>');">&nbsp;<%
				if not isnull(objRSPag("Tpl_Sigla")) then response.write TratarAspasHtml(objRSPag("Tpl_Sigla")) & " " & TratarAspasHtml(objRSPag("Esc_NomeLogr")) & " nº " & TratarAspasHtml(objRSPag("Esc_NroLogr")) end if
			%></td>
			<%'@@JKNUP: Adicionado.%>
			<td>&nbsp;<%=TratarAspasHtml(objRSPag("Esc_COD_SAP"))%></td>
			<td>&nbsp;<%=TratarAspasHtml(objRSPag("Esc_Contato"))%></td>
			<td>&nbsp;<%=TratarAspasHtml(objRSPag("Esc_Telefone"))%></td>
			<td onmouseover="showtip(this,event,'<%=TratarAspasHtml(objRSPag("Esc_Email"))%>');">&nbsp;<%if len(TratarAspasHtml(objRSPag("Esc_Email"))) > 18 then response.write left(TratarAspasHtml(objRSPag("Esc_Email")),18) & "..."%></td>
			<%'@@JKNUP: Comentado.%>
			<!-- <td><input type="checkbox" name="excluir" value="< %=objRSPag("Esc_ID")%>" onClick="AddSelecaoChk(this)"></td> -->
		</tr>
		<%
		intCount = intCount+1
		objRSPag.MoveNext
...
<tr>
	<td align=center>
		...
		<!--	LP<input type="button" class="button" name="btnExcluir" value="Excluir" onclick="ExlcuirRegistro()"  accesskey="R" onmouseover="showtip(this,event,'Excluir (Alt+R)');">-->
		<input type="button" class="button" name="btnLimpar" value=" Limpar " onClick="javascript:window.location.replace('estacao_main.asp')" accesskey="L" onmouseover="showtip(this,event,'Limpar (Alt+L)');">
