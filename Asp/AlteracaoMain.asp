...
function ResgatarAcessoLogico()
{
	with (document.forms[0])
	{
		...
		hdnTipoProcesso.value = 4 //Lista todas as solicitações em aberto
			...
			switch (hdnOrigem.value)
			{
				...
				case "can":
					if (confirm("Confirma o cancelamento do serviço?"))
					{
						...
						if (parseInt(intRet) == 146 || parseInt(intRet) == 145 || parseInt(intRet) == 117 || parseInt(intRet) == 2 || parseInt(intRet) == 124  || parseInt(intRet) == 1)
						{
							spnSolId.innerHTML = objAryRet[1] 
							ResgatarAcessoLogico() 
						}
...
<input type=hidden name="hdnAlteracaoAcesso" value="Alteracao">
...
<tr><th colspan=2>&nbsp;•&nbsp;Modificações de Informações do Acesso</th></tr>
<tr class=clsSilver>
<td>
	Pedido de Acesso
</td>
...
<tr class=clsSilver>
	...
	<td><input type="text" name="txtFacID" size=10 class=text onKeyUp="ValidarTipo(this,0)" maxlength=10></td>
</tr>
<!-- LP -->
<tr class=clsSilver>
