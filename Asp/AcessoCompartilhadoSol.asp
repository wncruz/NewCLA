...
<tr >
	...
	<th width=10% nowrap>&nbsp;Vel Fis</th>
	<th width=35%>&nbsp;Facilidade</th>
	<th width=35%>&nbsp;Tecnologia</th>
...
intEnd = parent.Request.QueryString("intEnd") 
...
strSql = APENDA_PARAMSTR("CLA_sp_sel_comp_AcessoFisico",10,Vetor_Campos)
'Response.Write strSql
...
if Not objRSSolic.Eof and Not objRSSolic.Bof then 
	While Not objRSSolic.Eof 
		...
		if (strOrisol_id = "9") then
			'set RSTemp = db.execute("select Tec_ID,Acf_DtAceite from cla_acessofisico WITH (NOLOCK) where acf_id=" & objRSSolic("acf_id"))
			if (  IsNull(objRSSolic("Acf_DtAceite")) and  in_array(objRSSolic("tec_id"), tecnologiaRIP) ) then
				exibir_compart = "N"
				'qtd_reg = qtd_reg + 1
				
			end if			
			
			if ( IsNull(objRSSolic("tec_id")) and ( objRSSolic("pro_id") <> "145" ) )then
				exibir_compart = "N"
				'qtd_reg = qtd_reg + 1
				
			end if			
			
			if ( in_array(objRSSolic("tec_id"), tecnologia ) ) then
				exibir_compart = "N"
				'qtd_reg = qtd_reg + 1
				
			end if
			
			if (objRSSolic("tec_id") = 6 and IsNull(objRSSolic("ont_id")) ) then
				exibir_compart = "N"
			end if 
			
		else
			if (IsNull(objRSSolic("Acf_DtAceite"))  ) then
				'response.write "<script>alert('" & objRSSolic("Acf_DtAceite") & "')</script>"
				exibir_compart = "N"
				'qtd_reg = qtd_reg + 1
			else
				'response.write "<script>alert('" & objRSSolic("sis_id") & "')</script>"
				if (  in_array(objRSSolic("pro_id"), provedor)  )	then
					'response.write "<script>alert('" & objRSSolic("sis_id") & "')</script>"
					'response.write "<script>alert('" & objRSSolic("sis_id") & "')</script>"
					exibir_compart = "N"
					'qtd_reg = qtd_reg + 1
				...
				if ( in_array( objRSSolic("sis_id") , sistema ) ) then
					exibir_compart = "N"
					'qtd_reg = qtd_reg + 1
				end if 
				
				'response.write "<script>alert('"&objRSSolic("tec_id")&"')</script>"
				'response.write "<script>alert('"&objRSSolic("acf_id")&"')</script>"
				'response.write "<script>alert('"&objRSSolic("ont_id")&"')</script>"
 ...
 if (exibir_compart = "S") then
	...
	%>
		<tr class="<%=strClass%>" width=100% >
			...
			%>
				<td width=3%><input type=radio name=rdoIDFis<%=intEnd%> value="" Aec_IdFis="<%=objRSSolic("Aec_IdFis")%>" prop="<%=objRSSolic("Acf_Proprietario")%>" onClick="parent.SelIDFisComp_(this,<%=intEnd%>,<%=objRSSolic("Acf_ID")%>)" <%if Request.Form("hdnIdAcessoFisico") = objRSSolic("Acf_IDAcessoFisico") then Response.Write " checked " End if%>></td>
			<% else
			%>
				<td width=3%><input type=radio name=rdoIDFis<%=intEnd%> value="<%=objRSSolic("Acf_IDAcessoFisico")%>" Aec_IdFis="<%=objRSSolic("Aec_IdFis")%>" prop="<%=objRSSolic("Acf_Proprietario")%>" onClick="parent.SelIDFisComp_(this,<%=intEnd%>,<%=objRSSolic("Acf_ID")%>)" <%if Request.Form("hdnIdAcessoFisico") = objRSSolic("Acf_IDAcessoFisico") then Response.Write " checked " End if%>></td>
			<% end if %>	
			...
			<td width=10%><%=objRSSolic("Vel_Desc")%>&nbsp;<%=TipoVel(objRSSolic("Acf_TipoVel"))%></td>
			<td width=35%><%=objRSSolic("newfac_nome")%></td>
			<td width=35%><%=objRSSolic("newtec_nome")%></td>
