...
''Response.Write "<script language=javascript>alert('Lendo Solicitação .');</script>"
...
Dim strNomeFantasia 	'Nome fantasia
...
Dim strUserNameAVL		'UserName do Avaliador
Dim strNomeAVL			'Nome do Avaliador
Dim strRamalAVL			'Ramal do Avaliador
...
Set objRSPedido		= db.execute("Select ped_id from cla_pedido where sol_id = " & dblSolId)


'@@ Davif - Avaliador
'Grava o Usuário responsável por Avaliar o Acesso
'
dblPedId = objRSPedido("Ped_Id")
dblSolId = Request.Form("hdnSolId")
strAvaliador = "S"

'Verifica se Existe Avaliador Alocado para Solicitação

Vetor_Campos(1)="adInteger,4,adParamInput," & dblSolId
strSqlRet = APENDA_PARAMSTRSQL("CLA_sp_Sel_Avaliador",1,Vetor_Campos)
Set objDicProp = Server.CreateObject("Scripting.Dictionary")


Set ObjRSAvaliador = db.Execute(strSqlRet)
if ObjRSAvaliador.EOF or ObjRSAvaliador.BOF then
	...
	Vetor_Campos(5)="adWChar,1,adParamInput," & strAvaliador

	Call APENDA_PARAM("CLA_sp_AlocacaoCarteira",5,Vetor_Campos)
	...
	If DBAction <> "178" Then
		Response.End
	End If
End If


'Verifica se Precisa atualizar data inicio de avaliacao
...
If not ObjRSAvaliacao.EOF and not ObjRSAvaliacao.BOF and isnull(ObjRSAvaliacao("Sol_DtInicioAvaliacao")) then

	'Grava Data de inicio do Processo de Avaliacao de Acesso
	Vetor_Campos(1)="adInteger,4,adParamInput," & dblSolId
	Vetor_Campos(2)="adWChar,1,adParamInput," & "S"
	Vetor_Campos(3)="adWChar,1,adParamInput," & "N"
	Vetor_Campos(4)="adWChar,1,adParamInput," & "N"
	...
	If DBAction = 1 or DBAction = 2 Then
		...
		Response.Write "<script language=javascript>alert('Erro ao Gravar Data Início de Avaliação. Contate o Hdesk');window.location.replace('main.asp');</script>"
...
strSqlRet = APENDA_PARAMSTRSQL("CLA_SP_VIEW_PONTO",4,Vetor_Campos)

Set objRSFis = db.Execute(strSqlRet)
Set objDicProp = Server.CreateObject("Scripting.Dictionary")

if Not objRSFis.EOF and not objRSFis.BOF then
    strDtPrevEntrAcesProv   = Formatar_Data(Trim(objRSFis("Ped_DtPrevistaAtendProv"))) 'Incluido PRSS 13/12/2006 - Planilha_Testes_916x_06dez.xls
...
''Para o Processo de Avaliação não será necessário alocar o GLA
''O GLA continuará a ser alocado na Fase de Alocação de Facilidades
strObrigaGla = 0
...
function CarregarLista()
{
	...
	if ('<%=intAcesso%>' != ''){
		//bjXmlGeral.loadXML("<%=strXmlAcesso%>") //OLD PRSS 05/12/2006
		objXmlGeral.loadXML("<%=replace(strXmlAcesso,"""","'")%>") //ALTERADO PRSS 05/12/2006
	}else{
...
<table cellspacing="1" cellpadding="0" border=0 width="760">
	<tr >
		...
		<th nowrap>&nbsp;Avaliação de Informações do Acesso</th>
		<th nowrap>&nbsp;Acesso Lógico&nbsp;:&nbsp;<%=dblIdLogico%></th>
		<th >&nbsp;Data&nbsp;:&nbsp;<%=strDtPedido%></th>
	</tr>
</table>
...
<!-- ''@@ -->
	<tr class="clsSilver">
		...
		<td colspan="3" >
			<input type="text" class="text" name="txtRazaoSocial"  maxlength="55" size="55" value="<%=strRazaoSocial%>">
...
<table  border=0 cellspacing="1" cellpadding="0" width="760" >
	...
	<tr class="clsSilver">
		<td width="170px"><font class="clsObrig">:: </font>UserName Avaliador</td>
		<td colspan=5>
			<table rules="groups" bgcolor="#eeeeee" cellspacing="0" cellpadding="2" bordercolorlight="#003388" bordercolordark="#ffffff" width="20%" >
				<tr>
					<td class="lightblue">&nbsp;
					<span id=spnUserName><%=strUserNameAVL%>
					</td>
				</tr>
			</table>
		</td>
	...
	<tr class="clsSilver">
		<td width="170px">&nbsp;&nbsp;&nbsp;&nbsp;Nome Avaliador</td>
		<td width=355px>
			<table rules="groups" bgcolor="#eeeeee" cellspacing="0" cellpadding="2" bordercolorlight="#003388" bordercolordark="#ffffff" width="100%" >
				<tr><td class="lightblue">&nbsp;
					<span id=spnNomeAVL><%=strNomeAVL%></span>
				</td></tr>
			</table>
		</td>
		<td align=right >Ramal&nbsp;</td>
		<td colspan=3 align=left >
			<table rules="groups" bgcolor="#eeeeee" cellspacing="0" cellpadding="2" bordercolorlight="#003388" bordercolordark="#ffffff" width="100px" >
				<tr><td class="lightblue">&nbsp;
					<span id=spnRamalAVL><%=strRamalAVL%></span>
				</td></tr>
			</table>
		</td>
	</tr>
...
<span id=spnGLA>
			<%if (strPropAcessoFisico = "TER" or strPropAcessoFisico = "CLI")  then      ''and (trim(strUserNameGLA) <> "" or trim(strUserNameGLA) <> null)
...
<table border=0 cellspacing="1" cellpadding="0" width="760" >
	<tr>
		<th colspan="6" >&nbsp;•&nbsp;Informações de Avaliação</th>
	</tr>

	<tr class="clsSilver">
		<td width="170px">&nbsp;&nbsp;&nbsp;&nbsp;Data Início Avaliação</td>
		<td width="120px">
			<table rules="groups" bgcolor="#eeeeee" cellspacing="0" cellpadding="2" bordercolorlight="#003388" bordercolordark="#ffffff" width="100%" >
				<tr><td class="lightblue">&nbsp;
					<span id=spnNomeAVL><%=strDtInicioAvaliacao%></span>
				</td></tr>
			</table>
		</td>
		<td align=right >Data Fim Avaliação&nbsp;</td>
		<td colspan=3 align=left >
			<table rules="groups" bgcolor="#eeeeee" cellspacing="0" cellpadding="2" bordercolorlight="#003388" bordercolordark="#ffffff" width="100px" >
				<tr><td class="lightblue">&nbsp;
					<span id=spnRamalAVL><%=strDtFimAvaliacao%></span>
				</td></tr>
			</table>
		</td>
	</tr>
...
<table  border=0 cellspacing="1" cellpadding="0" width="760" >
	<tr >
		<td align=center>
			<input type="button" class="button" name="btnAlterar" value="Alterar"  onclick="AprovarAlteracao(<%=dblSolId%>)" accesskey="I" onmouseover="showtip(this,event,'Alterar a solicitação (Alt+I)');" >&nbsp;
			<%
			if dblStsId <> 204 and dblStsId <> 200 then 'devolvida ao GIC pelo avaliador
				strdesabilita = ""
			else
				strdesabilita = "disabled = true"
			end if%>
			<input type="button" <%=strdesabilita%> class="button" name="btnAprovar" value=" Aprovar " onClick="AprovarAvaliacao(<%=dblSolId%>)" style="width:100px" accesskey="A" onmouseover="showtip(this,event,'Aprovar (Alt+A)');">									
			<input type="button" <%=strdesabilita%> class="button" name="btnDevolver" value=" Devolver p/ GIC " onClick="DevolverParaGIC(<%=dblSolId%>)" style="width:100px" accesskey="D" onmouseover="showtip(this,event,'Devolver p/ GIC (Alt+D)');">
