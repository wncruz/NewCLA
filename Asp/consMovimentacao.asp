...
<tr>
<td >
<form name="f" method="post" action="consMovimentacao.asp" onSubmit="return false">
<table border="0" cellspacing=1 cellpadding=0 width=760>
<tr>
	<th colspan=2><p align="center">Consulta de Movimentação de Acessos (Lista)</p></th>
</tr>
<tr class=clsSilver>
...
'**************************************************************************
db.execute("insert into newcla.tab_temp2(Valor) values('Consulta de Movimentação de Acessos (Lista);' + CAST(CONVERT(varchar(19),getDate(),126) as varchar) + ';" & trim(strLoginRede) & ";" & request.form("txtdatainicio") & ";" & request.form("txtdatafim") & "')")

strSql = APENDA_PARAMSTRSQL("CLA_sp_cons_Movimentacao",6,Vetor_Campos)
...
if not objRSPag.Eof or not objRSPag.Bof then
	...
	Response.Write	"<table border=0 width=760><tr><td colspan=2 align=right>" & _
					...
					"<a href='javascript:TelaImpressao(800,600,""Consulta de Movimentações (Lista) - " & date() & " " & Time() & " "")' onmouseover=""showtip(this,event,'Tela de Impressão...')""><img src='../imagens/impressora.gif' border=0></a></td></tr>" & _ 
					"</table>"

	strHtml = "<table border=0 cellspacing=1 cellpadding=0 >"
	strHtml = strHtml  & "<tr >"
	strHtml = strHtml  & "	<td colspan=15>" & strNomeProvedor & "  " & Formatar_Data(strDataInicio) & " - " & Formatar_Data(strDataFim)   & "</td>"
	strHtml = strHtml  & "</tr>"
	strHtml = strHtml  &  "<tr>"
	...
	For intI = 1 to objRSPag.PageSize
		...
		strXls = strXls  &  "<td nowrap>&nbsp;" & Formatar_Data(objRSPag("PrazoRestante")) & "</td>"
		strXls = strXls  &  "<td nowrap>&nbsp;" & objRSPag("Tct_Desc") & "</td>"
...
<input type=hidden name=hdnXls>
<input type=hidden name=hdnXls>
<input type=hidden name=hdnNomeCons value="ConsMovimentação">
