...
'set NdAcfid = objXmlDoc.selectSingleNode("//Acf")
dblPedId  = ndPed.Text
dblSolId  = ndSol.Text
...
Set objRSPro =  nothing 
	...
	if dblPedId <> "" then
		...
		if Not ped1.Eof and Not ped1.Bof then		
			...
			ASSUNTO = AcaoPedidoEmail(ucase(ped1("tprc_id"))) & "  -  " & trim(ped("Cli_nome")) & "  -  " & ucase(ped1("Ped_Prefixo")) & "-" & right("00000" & ped1("Ped_Numero"),5) & "/" & ped1("Ped_Ano")
			...
			textohtml = textohtml & "<br>A</br>"


			textohtml = textohtml & strProNome
			textohtml = textohtml & "<br><br>"
			textohtml = textohtml & "Assunto: Solicitacao de Servico" & "<br>"
			textohtml = textohtml & "Acao   : <u>"& AcaoPedidoEmail(ucase(ped1("tprc_id"))) & "</u>"
			...
			if trim(strNroAcessoPtaEbt) <> "" and not isnull(strNroAcessoPtaEbt) then
				textohtml = textohtml & "<br>Numero de Acesso: " & strNroAcessoPtaEbt
				if Trim(strCctoProvedor) <> "" then
					textohtml = textohtml & "<br>CCTO Provedor: " & strCctoProvedor
				End if 
			end if
			textohtml = textohtml & "<br><br>"
			textohtml = textohtml & "<font size=2 face=Arial color=#FF0000>Numero do Pedido:"& ucase(ped1("Ped_Prefixo")) & "-" & right("00000" & ped1("Ped_Numero"),5) & "/" & ped1("Ped_Ano") & "</font><br><br>"
						...
						Case 1
							if  Not objDic.Exists(Cstr(strRepresentacao)) then
								...
								if intCount = 1 then
									...
									strHtmlFac = strHtmlFac & "<th colspan=7>&nbsp;•&nbsp;Informacoes da Facilidade</th>"
									strHtmlFac = strHtmlFac & "</tr>"
						...
						Case 2	
							...
							if  Not objDic.Exists(Cstr(strRepresentacao)) then
								...
								if intCount = 1 then
									...
									strHtmlFac = strHtmlFac & "<table border=0 cellspacing=1 cellpadding=1 width=100% >"
									strHtmlFac = strHtmlFac & "	<tr >"
									strHtmlFac = strHtmlFac & "		<th colspan=5>&nbsp;•&nbsp;Informacoes da Facilidade</th>"
									strHtmlFac = strHtmlFac & "	</tr>"
									strHtmlFac = strHtmlFac & "	<tr class=clsSilver2>"
						...
						Case 3	
							...
							if  Not objDic.Exists(Cstr(strRepresentacao)) then
								...
								if intCount = 1 then
									...
									strHtmlFac = strHtmlFac & "<table border=0 cellspacing=1 cellpadding=1 width=100% >"
									strHtmlFac = strHtmlFac & "	<tr>"
									strHtmlFac = strHtmlFac & "		<th colspan=6>&nbsp;•&nbsp;Informacoes da Facilidade</td>"
									strHtmlFac = strHtmlFac & "	</tr>"
									strHtmlFac = strHtmlFac & "	<tr class=clsSilver2>"
			...
			textohtml = textohtml & "<br>Solicitamos providenciar as medidas cabiveis conforme acao acima."
			textohtml = textohtml & "<br><br>"
			...
			textohtml = textohtml & "<tr><td>Inscricao Estadual</td><td>" & ped("Aec_IE") & "</td></tr>"
			...
			if not isNull(ped1("Reg_Id")) then
				...
				if not objRS.Eof and Not objRS.Bof then
					textohtml = textohtml & "<tr><td>Regime de Contratacao</td><td>" & objRS("Tct_Desc") & "</td></tr>"
				Else
					textohtml = textohtml & "<tr><td>Regime de Contratacao</td><td></td></tr>"	
				End if		
			Else	
				textohtml = textohtml & "<tr><td>Regime de Contratacao</td><td></td></tr>"	
			End if
			...
			textohtml = textohtml & "<center><u><b>LOCAL DE INSTALACAO</b></u></center><br>"
			...
			textohtml = textohtml & "<td width=180>Endereco</td><td>" & strEndereco & "</td></tr>"
			...
			if not objRSEndPto.Eof and not objRSEndPto.bof then
				...
				textohtml = textohtml & "<table><tr>"
				textohtml = textohtml & "<td width=180>Endereco</td><td>" & objRSEndPto("Tpl_Sigla") & " " & objRSEndPto("End_NomeLogr") & " nº " & objRSEndPto("End_NroLogr") & " " & objRSEndPto("Aec_Complemento") & "</td></tr>"
...
'' LPEREZ  13/12/2005			
			textohtml = textohtml & "<table><tr><td colspan=2>Observacao</td></tr>"
...
'LP
			...
			'else
				...
				'Set objRSResp = db.execute("CLA_sp_sel_provedoremail " & dblProId & "") 
				
				If Not objRSResp.Eof and Not objRSResp.bof then
					textohtml = textohtml & "<br><br>" & Trim(objRSResp("cPRo_ContratanteContato"))
					textohtml = textohtml & "<br>" & Trim(objRSResp("CPro_ContratanteDepto"))
				Else
					textohtml = textohtml & "<br><br>" & " Ruben Nalin Filho"
					textohtml = textohtml & "<br>Gerente de Implantacao de Acessos."
				End if
			'End if
			
			'textohtml = textohtml & "<br>Gerente de Implantacao de Acessos."
			'textohtml = textohtml & "<br>" & Trim(objRSResp("CPro_ContratanteDepto"))
			textohtml = textohtml & "<br><br><hr>Embratel - Empresa Brasileira de Telecomunicacoes S.A."
	...
	'5''strButton = "<form name = ""Envio"" action=""enviaemailRadioIP.asp"" method=""post"" target=""IFrmEmail"">"  & chr(10)
	'5''strButton = "<table width=100% border=0>" & chr(10)

        strButton = "<form name = ""Envio"" action=""enviaemailRadioIP.asp"" method=""post"" target=""IFrmEmail""> <table width=100% border=0>" & chr(10)
	...
	'strfunction = strfunction & "		alert(1)" & chr(10)
	'strfunction = strfunction & " 		with (document.forms[1]) "
	'strfunction = strfunction & " 		{ "
	
	strfunction = strfunction & "			dim retorno" & chr(10)
	strfunction = strfunction & "			document.Envio.hdnstrXML.value = document.form1.InnerHTML" & chr(10)
	...
	strfunction = strfunction & "		Envio.btnEnviar.disabled=true" & chr(10)
