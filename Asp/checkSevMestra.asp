...
<%
	...
	dblNroSev 		= Trim(Request.Form("hdnNroSev"))
	
	pro_id 	  		= Trim(Request.Form("hdnProvedor"))  	
	origemSol 	  	= Trim(Request.Form("hdnOrigemSol"))
	AcaoAPG   		= Trim(Request.Form("hdnTipoAcaoAPG"))
	Segmento		= Trim(Request.Form("hdnSegmento"))
	Porte			= Trim(Request.Form("hdnPorte"))
	Tecnologia		= Trim(Replace(Request.Form("cboTecnologia"), "'", "\'")) 'Request.Form("hdnTecnologia") 		
	Prop			= Trim(Request.Form("hdnPropIdFisico"))
	
	CliCC			= Trim(Request.Form("hdnConta"))

	AcfId			= Trim(Request.Form("hdnAcfId"))
	
	if Acfid = "" then
	   AcfId = Trim(Request.Form("hdnStrAcfId"))
	...
	'if ( ltrim(rtrim(origemSol)) <> "3" or ltrim(rtrim(origemSol)) = "" ) then
		...
		Vetor_Campos(7)="adWChar,20,adParamInput,"	& Prop 'obrig
		
		Vetor_Campos(8)="adInteger,4,adParamInput," & origemSol 'obrig
		Vetor_Campos(9) ="adWChar,20,adParamInput,"	& CNLSiglaCentroCliDest 'obrig
		Vetor_Campos(10)="adWChar,20,adParamInput,"	& ComplSiglaCentroCliDest 'obrig
