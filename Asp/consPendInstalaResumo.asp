...
'	- Sistema			: CLA
'	- Arquivo			: ConsPendInstalaResumo.asp
'	- Responsável		: Vital
...
<tr>
<td >
<form name="f" method="post" action="consPendInstalaResumo.asp" onSubmit="return false">
<table border="0" cellspacing=1 cellpadding=0 width=760>
<tr>
	<th colspan=2><p align="center">Pendentes de Instalação (Resumo)</p></th>
</tr>
<tr class=clsSilver>
...
<tr class=clsSilver>
	...
	<td><input type="text" class="text" name="txtDataFim" size="10"  maxlength="10" value="<%if request("txtDataFim") <> "" then response.write request("txtDataFim") else response.write strDataAtual end if %>" onKeyPress="OnlyNumbers();AdicionaBarraData(this)"></td>
</tr>
<tr>	
...
strDataFim = inverte_data(Request.Form("txtDataFim"))

Vetor_Campos(1)="adInteger,4,adParamInput," & Request.Form("cboProvedor")
Vetor_Campos(2)="adInteger,4,adParamInput," & Request.Form("cboAcao")
Vetor_Campos(3)="adInteger,4,adParamInput," & Request.Form("cboCef")
Vetor_Campos(4)="adWChar,2,adParamInput,"	& Request.Form("cboUF")
Vetor_Campos(5)="adWChar,10,adParamInput,"	& strDataFim

'**************************************************************************
'*** COLETA PARA BLOQUEIO DE CONSULTAS POR MOTIVO DE PERFORMANCE DO CLA ***
'**************************************************************************
db.execute("insert into newcla.tab_temp2(Valor) values('Pendentes de Instalação (Resumo);' + CAST(CONVERT(varchar(19),getDate(),126) as varchar) + ';" & trim(strLoginRede) & ";" & request("txtdatafim") & "')")


strSql = APENDA_PARAMSTRSQL("CLA_sp_cons_PendInstalacaoResumo",5,Vetor_Campos)
...
if not objRSPag.Eof or not objRSPag.Bof then
	...
	Response.Write	"<table border=0 width=300 align=center><tr><td colspan=2 align=right>" & _
					...
					"<a href='javascript:TelaImpressao(800,600,""Consulta de Pendentes de Instalação (Resumo) - " & date() & " " & Time() & " "")' onmouseover=""showtip(this,event,'Tela de Impressão...')""><img src='../imagens/impressora.gif' border=0></a></td></tr>" & _ 
					"</table>"
	...
	strHtml = strHtml  & "<tr >"
	strHtml = strHtml  & "	<td colspan=2>" & Request.Form("hdnProvedor") & "   " & Request.Form("txtDataFim")    & "</td>"
	strHtml = strHtml  & "</tr>"
	strHtml = strHtml  & "<tr class=clsSilver>"
	strHtml = strHtml  & "	<th colspan=2>&nbsp;Acessos Pendentes</th>"
	strHtml = strHtml  & "</tr>"
	strHtml = strHtml  & "<tr class=clsSilver>"
	...
	strHtml = strHtml  & "</tr>"
	strHtml = strHtml  & "<tr class=clsSilver >"
	strHtml = strHtml  & "	<td width=150px >&nbsp;Até 20</td>"
	...
	strHtml = strHtml  & "</tr>"
	strHtml = strHtml  & "<tr class=clsSilver>"
	strHtml = strHtml  & "	<td>&nbsp;Acima de 91</td>"
	strHtml = strHtml  & "	<td align=right>" & int90Mais & "&nbsp;</td>"
	strHtml = strHtml  & "</tr>"
...
function Consultar()
{
	with (document.forms[0])
	{
		...
		action = "ConsPendInstalaResumo.asp"
