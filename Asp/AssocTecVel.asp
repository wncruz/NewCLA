...
'	- Sistema			: CLA
'	- Arquivo			: AssocTecVel.asp
'	- Responsável		: Vital
'	- Descrição			: Associação de tecnologia e velocidade
%>
...
if Trim(dblID) = "" then
	dblID = Server.HTMLEncode(Request.Form("hdnId"))
End if	
...
If Request.Form("hdnAcao")="Gravar" then
	...
	Vetor_Campos(2)="adInteger,2,adParamInput,"& Request.Form("cboTecnologia") 
	Vetor_Campos(3)="adInteger,2,adParamInput," & Request.Form("cboVel") 

	Vetor_Campos(4)="adInteger,2,adParamOutput,0"  
	
	Call APENDA_PARAM("CLA_sp_ins_AssocTecVeloc",4,Vetor_Campos)
...
If dblID <> "" then
	Set objRSVel = db.execute("CLA_sp_sel_AssocTecVeloc " & dblID)
End if
%>
<!--#include file="../inc/header.asp"-->
<form action="AssocTecVel.asp" method="post" >
<SCRIPT LANGUAGE="JavaScript">
function checa(f) 
{
	if (!ValidarCampos(f.cboTecnologia,"Tecnologia")) return false;
	if (!ValidarCampos(f.cboVel,"Velocidade")) return false;
	return true;
}
...
<tr>
	<td>
		<table border="0" cellspacing="1" cellpadding=0 width="760">
			<tr>
				<th colspan=2><p align="center">Associação de Tecnologia com Velocidade</p></th>
			</tr>
			<tr class=clsSilver>
				<td>
				...
				<td>
					<select name="cboTecnologia">
						<option value=""></option>
						<% set objRS = db.execute("CLA_sp_sel_tecnologia") 
							If Trim(dblID)<> "" then
								dblIDAtual = objRSVel("Tec_Id")
							Else
								dblIDAtual = Request.Form("cboTecnologia") 
							End if

							While Not objRS.Eof
								strSel = ""
								if Cdbl("0" & objRS("Tec_Id")) = Cdbl("0" & dblIDAtual) then strSel = " selected "
								Response.Write "<Option value="& Trim(objRS("Tec_Id")) & strSel & ">" & Trim(objRS("Tec_Nome")) & "</Option>"
								objRS.MoveNext
							Wend
							Set objRS = Nothing
						%>
		...
		<tr>
			<td colspan=2 align="center"><br>
				...
				<input type="button" class="button" name="btnLimpar" value="Limpar" onclick="document.forms[0].hdnId.value = '';LimparForm();setarFocus('cboTecnologia');" accesskey="L" onmouseover="showtip(this,event,'Limpar (Alt+L)');">&nbsp;
				<input type="button" class="button" name="Voltar" value="Voltar" onClick="javascript:window.location.replace('AssocTecVel_main.asp')" accesskey="B" onmouseover="showtip(this,event,'Voltar (Alt+B)');" >
