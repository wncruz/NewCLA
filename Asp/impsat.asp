...
dim strServico , strXML, strAcao, strCodAcao
dim ndPed
dim ndSol
dim ndProv
dim ndEsc
...
''response.write "PEdido:" + dblPedID
...
strParmProc = dblProId + "|" + dblPedId  + "|" + intTipoProcesso + "|" + strUserName
	...
	if not isnull(objRSped1("esc_nomelogr")) then
		...
		strXML = strXML &	"<arquivo>impsat</arquivo>"
		...
		If not isnull(objRSped1("ACL_DtDesejadaEntregaAcessoServico")) then
			strXML = strXML &	"<dataprevistaentraga>" & formatdatetime(objRSped1("ACL_DtDesejadaEntregaAcessoServico"),2) &  "</dataprevistaentraga>"
		else
			strXML = strXML &	"<dataprevistaentraga> </dataprevistaentraga>"
		End if

		strXML = strXML &	"<pontaebt>" & objRSped1("Acf_NroAcessoPtaEbt") &  "</pontaebt>"
		strXML = strXML &	"<acao>" & strAcao &  "</acao>"
		strXML = strXML &	"<codacao>" & strCodAcao &  "</codacao>"
		...
		strXML = strXML &	"<pontabinterface>" & objRSPontoB("Acf_InterfaceEstEntregaFisico") &	"</pontabinterface>"
		...
		strXML = strXML &	"<observação>" & objRSped1("pED_obs")  &  "</observação>"
