...
If Trim(Request.Form("hdnAcao"))="Excluir" then
	Call ExcluirRegistro("CLA_sp_del_Empreiteira")
End if
%>
...
<tr>
	<td width=100%>
		<table border="0" cellspacing="1" cellpadding=0 width=760 >
			<tr>
				<th colspan=2><p align="center">Cadastro de Empreiteiras</p></th>
			</tr>
			<tr class=clsSilver>
				<td >Busca &nbsp;&nbsp;</td>
				<td >
...
if Trim(Request.Form("txtBusca")) <> ""  then
	...
	if Ubound(objAryParam) > 0 then
		strSql = "CLA_sp_sel_Empreiteira 0,'" & TratarAspasSql(objAryParam(0)) & "','" & TratarAspasSql(objAryParam(1)) & "'"
	Else
		strSql =  "CLA_sp_sel_Empreiteira 0,'" & TratarAspasSql(objAryParam(0)) & "'"
	End if
Else
	strSql = "CLA_sp_sel_Empreiteira 0"
End if
...
if not objRSPag.Eof and not objRSPag.Bof then
	For intIndex = 1 to objRSPag.PageSize
		...
		%>
		<tr class=<%=strClass%>>
			<td>&nbsp;<a href="cad_Empreiteira.asp?ID=<%=objRSPag("Font_ID") %>"><%=TratarAspasHtml(objRSPag("Font_Nome"))%></a></td>
...
<tr>
	<td align=center>
		<input type="button" class="button" name="Incluir" value="Incluir" onClick="javascript:window.location.replace('cad_Empreiteira.asp?TpProc=INC')" accesskey="I" onmouseover="showtip(this,event,'Incluir (Alt+I)');">
<!--
@@ LPEREZ - 25/05/2006
